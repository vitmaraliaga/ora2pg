...
PROCEDURE        IUDP_PERSONA_NATURAL_PARENT (P_Modo IN int, P_ID_PERSONA IN int, P_ID_PARIENTE IN int, P_ID_TIPOPARENTESCO IN int, P_COMENTARIO IN varchar2 DEFAULT NULL) AS

	S_COMENTARIO VARCHAR2(50);
  S_CANTIDAD number:=0;
BEGIN
  IF P_Modo = 0 THEN
		S_COMENTARIO := TRIM(NVL(P_COMENTARIO, ''));

    IF P_ID_PERSONA IS NOT NULL AND P_ID_PARIENTE IS NOT NULL THEN

    	SELECT COUNT(*) INTO S_CANTIDAD FROM moises.PERSONA_NATURAL_PARENTESCO WHERE ID_PERSONA = P_ID_PERSONA AND ID_PARIENTE = P_ID_PARIENTE; 
