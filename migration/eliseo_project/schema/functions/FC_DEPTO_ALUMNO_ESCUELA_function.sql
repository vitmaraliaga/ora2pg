...
CREATE OR REPLACE FUNCTION eliseo.fc_depto_alumno_escuela (P_ID_CLIENTE bigint, P_ID_DEPTO text) RETURNS varchar AS $body$
DECLARE
...
BEGIN
  ...
  if l_contar > 0 then
    ...
    SELECT coalesce(ID_DEPTO_GENERAL,ID_DEPTO) INTO STRICT l_depto
    FROM ORG_SEDE_AREA
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.fc_depto_alumno_escuela (P_ID_CLIENTE bigint, P_ID_DEPTO text) FROM PUBLIC;
