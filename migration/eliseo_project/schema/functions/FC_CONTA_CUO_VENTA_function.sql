...
CREATE OR REPLACE FUNCTION eliseo.fc_conta_cuo_venta (P_ID_VENTA bigint, p_id_entidad bigint, p_id_anho bigint, p_id_mes bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
	L_CONTAR := 0;
		...
		IF L_CONTAR > 0 THEN
            ...
            SELECT COUNT(1), min(lote) INTO STRICT L_CONTAR, L_NOMBRE FROM VW_CONTA_DIARIO d 
            where
...
;
	...
	;
            ...
            IF L_CONTAR = 0 THEN 
                SELECT COUNT(1), MAX(A.CUO) INTO STRICT L_CONTAR, L_NOMBRE FROM CONTA_DIARIO_CUO_AUX A
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.fc_conta_cuo_venta (P_ID_VENTA bigint, p_id_entidad bigint, p_id_anho bigint, p_id_mes bigint) FROM PUBLIC;
