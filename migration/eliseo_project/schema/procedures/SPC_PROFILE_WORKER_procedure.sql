...
CREATE OR REPLACE PROCEDURE eliseo.spc_profile_worker ( p_id_persona integer, OUT CURSOR REFCURSOR ) AS $body$
DECLARE
    ...
    l_email varchar(100);
    l_fec_misionero varchar(100);
    l_sueldo_depositado decimal(38,2);
    l_doc_number varchar(50);
...
;
    ...
    SELECT max(NOMBRE) INTO STRICT l_TIPO_STATUS FROM TIPO_ESTATUS
    ...
    select max(num_telefono) into STRICT l_telefono from persona_telefono
          where id_persona = p_id_persona;

    SELECT to_char(max(fec_misionero),'dd/mm/yyyy') into STRICT l_fec_misionero FROM APS_EMPLEADO
    WHERE ID_PERSONA = p_id_persona
;

    
    select max(NUM_DOCUMENTO) into STRICT l_doc_number from VW_PERSONA_NATURAL
    where ID_PERSONA = p_id_persona
;

    select max(DIRECCION) into STRICT l_email from persona_virtual
          where id_persona = p_id_persona
          and ID_TIPOVIRTUAL = 1;

     CALL spc_account_statement_by_id(l_ID_ENTIDAD,l_ID_ANHO,l_ID_MES,p_id_persona,l_doc_number,l_sueldo_depositado);

    RAISE NOTICE 'en profile worker';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.spc_profile_worker ( p_id_persona integer, OUT CURSOR REFCURSOR ) FROM PUBLIC;
