...
CREATE OR REPLACE PROCEDURE eliseo.spc_user_session_login_remad ( -- spc_user_session_login_portal_padre_mobile
    -- LOGIN PARA LA AUTENTICACIÓN DEL SISTEMA REMAD
 p_id_persona integer, p_token text, p_token_oauth text, p_id_entidad OUT bigint, p_error OUT bigint, p_msjerror OUT text, p_caduca text default 'S') AS $body$
DECLARE
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.spc_user_session_login_remad (  p_id_persona integer, p_token text, p_token_oauth text, p_id_entidad OUT bigint, p_error OUT bigint, p_msjerror OUT text, p_caduca text default 'S') FROM PUBLIC;
