...
CREATE OR REPLACE PROCEDURE eliseo.spc_sta_ingresos_rt ( p_id_entidad integer, p_id_entidad_assinet integer, p_id_anho integer, p_id_mes integer, p_id_persona integer, p_id_ctacte text, OUT CURSOR REFCURSOR ) AS $body$
DECLARE
...
BEGIN
    ...
    SELECT TO_CHAR(fecha,'DD/MM/YY') AS fecha, comentario, sum(valor) as valor from (SELECT 
                ...
                AND ID_TIPOPLANILLA in (98626) -- SOLO ingresos de la boleta de pagos (tipo de planilla) contra_cheque
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.spc_sta_ingresos_rt ( p_id_entidad integer, p_id_entidad_assinet integer, p_id_anho integer, p_id_mes integer, p_id_persona integer, p_id_ctacte text, OUT CURSOR REFCURSOR ) FROM PUBLIC;
