...
CREATE OR REPLACE PROCEDURE eliseo.spc_user_session_login_secre ( p_id_persona integer, p_token text, p_token_oauth text, p_error OUT bigint, p_msjerror OUT text, p_caduca text default 'S') AS $body$
DECLARE
...
BEGIN
    ...
    SELECT COUNT(1) into STRICT l_contar FROM APS_EMPLEADO A WHERE A.ID_PERSONA=p_id_persona;
    IF l_contar = 0 THEN
        l_error := 1;
...
--         goto salida_rapida;
    ...
    SELECT COUNT(1) into STRICT l_contar FROM APS_EMPLEADO A
    ...
    UPDATE esdras.ENTIDAD_RELIGIOSA_USUARIO SET estado = '1'
     ...
     -- Validar que la persona tenga una entidad asignada por defecto en lamb secretaría
        SELECT 
            ...
            into STRICT l_contar, l_entidad, l_dpto
        FROM esdras.ENTIDAD_RELIGIOSA_USUARIO eru
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.spc_user_session_login_secre ( p_id_persona integer, p_token text, p_token_oauth text, p_error OUT bigint, p_msjerror OUT text, p_caduca text default 'S') FROM PUBLIC;
