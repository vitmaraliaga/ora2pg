...
CREATE OR REPLACE PROCEDURE eliseo.spc_account_statement_sal_rt ( p_id_entidad integer, p_id_entidad_assinet integer, p_id_anho integer, p_id_mes integer, p_id_persona integer, p_id_cta_cte text, OUT CURSOR REFCURSOR ) AS $body$
DECLARE

    l_ingresos_total decimal(38,2);
   	l_ingresos decimal(38,2);
   	l_ingresos_ayudas decimal(38,2);

    l_descuentos_total decimal(38,2);
    l_descuentos decimal(38,2);
    l_descuentos_sueldo decimal(38,2);
    l_descuentos_adelantos decimal(38,2);
    l_descuentos_ayudas decimal(38,2);

    l_depositado decimal(38,2);
   	l_sueldo_depositado decimal(38,2);
   	l_ayudas_depositado decimal(38,2);
...
BEGIN
	...
	select 
    coalesce(sum(p.COS_VALOR),0)  into STRICT l_descuentos_sueldo
    from
    VW_APS_PLANILLA p, APS_CONCEPTO_PLANILLA a
    where p.ID_CONCEPTOAPS=a.ID_CONCEPTOAPS
    and p.ID_ENTIDAD=p_id_entidad
...
------------------------ AYUDAS INFORMADOS------------------------
	SELECT abs(coalesce(sum(valor),0)) into STRICT l_ingresos_ayudas FROM (
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.spc_account_statement_sal_rt ( p_id_entidad integer, p_id_entidad_assinet integer, p_id_anho integer, p_id_mes integer, p_id_persona integer, p_id_cta_cte text, OUT CURSOR REFCURSOR ) FROM PUBLIC;
