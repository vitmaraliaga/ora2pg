...
CREATE OR REPLACE PROCEDURE eliseo.spc_mobile_user_data ( p_id_persona integer, OUT CURSOR REFCURSOR ) AS $body$
DECLARE


    l_total_diezmo decimal(38,2);
    l_total_gasto decimal(38,2);
    l_tipo_admin decimal(38,2);
    l_tienepptodepartamentos varchar(10);
    l_tienepptoviajes varchar(10);
...
BEGIN
    ...
    ),'N') into STRICT l_tienepptodepartamentos;
    ...
    SELECT    id_persona as id, 
              ...
              (select coalesce((
                ...
                ),'N')  ) as tienepptoviajes
      from (select id_persona, num_documento, nom_persona, id_entidad
    from VW_APS_EMPLEADO
    where id_persona = p_id_persona
    AND coalesce(FEC_TERMINO::text, '') = '' 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.spc_mobile_user_data ( p_id_persona integer, OUT CURSOR REFCURSOR ) FROM PUBLIC;
