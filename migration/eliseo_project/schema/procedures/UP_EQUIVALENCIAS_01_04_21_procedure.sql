...
CREATE OR REPLACE PROCEDURE eliseo.up_equivalencias_01_04_21 () AS $body$
DECLARE
...
BEGIN
	...
	FOR ITEM IN (
		SELECT * FROM (SELECT ID_CUENTAEMPRESARIAL AS CODE_AASSI,COUNT(ID_CUENTAAASI) AS CANT_AASSI FROM (SELECT DISTINCT ID_CUENTAEMPRESARIAL,ID_CUENTAAASI 
			FROM AAA_CONTA_EMPRESA_CTA_ASSINET) alias1  GROUP BY ID_CUENTAEMPRESARIAL) T1
		LEFT JOIN(SELECT ID_CUENTAEMPRESARIAL AS CODE_LAMB,COUNT(ID_CUENTAAASI) AS CANT_LAMB FROM (SELECT DISTINCT ID_CUENTAEMPRESARIAL,ID_CUENTAAASI
		FROM CONTA_EMPRESA_CTA cec
		WHERE ID_EMPRESA =207 AND ID_ANHO = 2020) alias4 GROUP BY ID_CUENTAEMPRESARIAL)T2 ON T2.CODE_LAMB=T1.CODE_AASSI 
		WHERE T1.CANT_AASSI<>T2.CANT_LAMB
		ORDER BY T1.CODE_AASSI)
	LOOP
		-- AGREGAR EQUIVALENCIAS QUE NO ESTAN EN LAMB
		SELECT COUNT(1) INTO STRICT L_COUNT FROM (SELECT DISTINCT ID_CUENTAEMPRESARIAL,ID_CUENTAAASI
				...
				FROM CONTA_EMPRESA_CTA cec
				WHERE ID_EMPRESA =207 AND ID_ANHO = 2020 AND ID_CUENTAEMPRESARIAL=ITEM.CODE_AASSI) ) alias9;
		IF L_COUNT>0 THEN
			FOR ITEM1 IN (
			...
			LOOP
				SELECT ID_RESTRICCION,ID_TIPOPLAN INTO STRICT L_ID_RESTRICION,L_ID_TIPOPLAN FROM CONTA_CTA_DENOMINACIONAL ccd WHERE ID_CUENTAAASI =ITEM1.ID_CUENTAAASI;
				IF (L_ID_RESTRICION IS NOT NULL AND L_ID_RESTRICION::text <> '') AND (L_ID_TIPOPLAN IS NOT NULL AND L_ID_TIPOPLAN::text <> '') THEN
		...
		SELECT COUNT(1) INTO STRICT L_COUNT FROM (SELECT DISTINCT ID_CUENTAEMPRESARIAL,ID_CUENTAAASI
				...
				FROM AAA_CONTA_EMPRESA_CTA_ASSINET aceca
				WHERE ID_CUENTAEMPRESARIAL=ITEM.CODE_AASSI) ) alias2;
		IF L_COUNT>0 THEN
			FOR ITEM1 IN (
				SELECT DISTINCT ID_CUENTAEMPRESARIAL,ID_CUENTAAASI
				FROM CONTA_EMPRESA_CTA cec
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.up_equivalencias_01_04_21 () FROM PUBLIC;
