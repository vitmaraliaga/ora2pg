...
VALOR bigint;
IMPORTE_AYUDA bigint;
ID_CVARIABLE bigint;
ID_CONCEPTOAPS varchar(50);
ES_CONCEPTOAPS_PERSONA varchar(1):='0';
CONTAR bigint :=0;
BEGIN
  BEGIN
...
--	EL calculo se hace desde laravel
	...
	IF (NEW.ES_CONCEPTOAPS_PERSONA IS NOT NULL AND NEW.ES_CONCEPTOAPS_PERSONA::text <> '') THEN
		ES_CONCEPTOAPS_PERSONA := NEW.ES_CONCEPTOAPS_PERSONA;
	END IF;
	IF (ID_CONCEPTOAPS IS NOT NULL AND ID_CONCEPTOAPS::text <> '') THEN
		IF ES_CONCEPTOAPS_PERSONA = '1' THEN
			SELECT COUNT(1),MAX(ICP.TIENE_VARIABLE),MAX(ICP.TIPO_VALOR),MAX(ICP.VALOR) INTO STRICT CONTAR,TIENE_VARIABLE,TIPO_VALOR,VALOR
		...
		END IF;
		IF CONTAR = 0 THEN
			SELECT TIENE_VARIABLE,TIPO_VALOR,VALOR INTO STRICT TIENE_VARIABLE,TIPO_VALOR,VALOR
			FROM  ELISEO.INFORME_CONCEPTOAPS_CONFIG 
			WHERE ID_CONCEPTOAPS =NEW.ID_CONCEPTOAPS;
		END IF;
		IF TIENE_VARIABLE='1' THEN
			IF (ID_CVARIABLE IS NOT NULL AND ID_CVARIABLE::text <> '')
			THEN 
				SELECT VALOR INTO STRICT VALOR FROM ELISEO.CONCEPTO_CONFIG_VARIABLE
