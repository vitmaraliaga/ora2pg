...
CREATE OR REPLACE VIEW vw_orders_pending (id_entidad, id_depto, id_anho, id_mes, id_pedido, id_persona, pid_pedido, id_tipopedido, nombre_tipopedido, id_actividad, nombre_actividad, id_areaorigen, nombre_areaorigen, id_areadestino, nombre_areadestino, numero, acuerdo, codigo, fecha_pedido, fecha_entrega, motivo, estado_porcentaje, accion, revisado, id_paso_actual, estado, estado_run, llave, llave_nombre, estado_pedido, id_registro, fecha_registro, usuario, id_departamento_origen, tipo_articulo) AS SELECT A.ID_ENTIDAD,
         ...
         -- PKG_ORDERS.FC_NOMBRE_TIPOPEDIDO (A.ID_TIPOPEDIDO) AS NOMBRE_PEDIDO,
          (SELECT coalesce(X.NOMBRE,'') AS NOMBRE_PEDIDO FROM TIPO_PEDIDO X WHERE X.ID_TIPOPEDIDO = A.ID_TIPOPEDIDO) AS NOMBRE_PEDIDO,
          A.ID_ACTIVIDAD,
         -- PKG_ORDERS.FC_NOMBRE_ACTIVIDAD (A.ID_ACTIVIDAD) AS NOMBRE_ACTIVIDAD,
          (SELECT coalesce(X.NOMBRE,'') AS NOMBRE_ACTIVIDAD FROM PSTO_ACTIVIDAD X WHERE X.ID_ACTIVIDAD = A.ID_ACTIVIDAD) AS NOMBRE_ACTIVIDAD,
          A.ID_AREAORIGEN,
          --PKG_ORDERS.FC_NOMBRE_AREA (A.ID_AREAORIGEN) NOMBRE_AREAORIGEN,
          (SELECT  coalesce(Y.ID_DEPTO||' - '||X.NOMBRE ,'') AS AREA FROM ORG_AREA X, ORG_SEDE_AREA Y WHERE X.ID_AREA = Y.ID_AREA AND Y.ID_SEDEAREA = A.ID_AREAORIGEN) AS NOMBRE_AREAORIGEN,
          A.ID_AREADESTINO,
          --PKG_ORDERS.FC_NOMBRE_AREA (A.ID_AREADESTINO) NOMBRE_AREADESTINO,
          (SELECT  coalesce(Y.ID_DEPTO||' - '||X.NOMBRE ,'') AS AREA FROM ORG_AREA X, ORG_SEDE_AREA Y WHERE X.ID_AREA = Y.ID_AREA AND Y.ID_SEDEAREA = A.ID_AREADESTINO) AS NOMBRE_AREADESTINO,
          A.NUMERO,
          A.ACUERDO,
          C.CODIGO,
          ...
          ( SELECT PROCESS_COMPONENTE.LLAVE AS ACCION FROM PROCESS_PASO_RUN INNER JOIN PROCESS_COMPONENTE_PASO
            ...
            WHERE PROCESS_PASO_RUN.ID_REGISTRO=B.ID_REGISTRO AND PROCESS_PASO_RUN.ID_PASO=B.ID_PASO_ACTUAL  AND PROCESS_PASO_RUN.ESTADO='0'   LIMIT 1) AS ACCION,
    ...
    ON PROCESS_PASO_RUN.ID_PASO=PROCESS_COMPONENTE_PASO.ID_PASO
    INNER JOIN PROCESS_COMPONENTE ON PROCESS_COMPONENTE_PASO.ID_COMPONENTE=PROCESS_COMPONENTE.ID_COMPONENTE
  WHERE PROCESS_PASO_RUN.ID_REGISTRO=B.ID_REGISTRO AND PROCESS_PASO_RUN.ID_PASO=B.ID_PASO_ACTUAL  AND PROCESS_PASO_RUN.ESTADO='0'   LIMIT 1)
          ...
          (SELECT B.ID_DEPTO AS AREA
            ...
            WHERE A.ID_AREA = B.ID_AREA AND B.ID_SEDEAREA = A.ID_AREAORIGEN)
             AS ID_DEPARTAMENTO,
             (CASE WHEN A.ID_AREADESTINO = 45 THEN (SELECT DISTINCT PKG_INVENTORIES.FC_ARTICULO(X.ID_ARTICULO) FROM PEDIDO_DETALLE X WHERE X.ID_PEDIDO = A.ID_PEDIDO AND FC_CODIGO_ARTICULO_CODIGO(X.ID_ARTICULO,A.ID_ANHO) = '32-001' )||'|'||
