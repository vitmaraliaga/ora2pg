...
CREATE OR REPLACE VIEW vw_order_purchases (id_entidad, id_depto, id_anho, id_mes, id_pedido, id_persona, pid_pedido, id_tipopedido, nombre_pedido, nombre_actividad, id_areaorigen, nombre_areaorigen, id_areadestino, nombre_areadestino, numero, acuerdo, fecha_pedido, fecha_entrega, motivo, estado, fecha_registro, email, id_departamento, tipo_articulo, id_dexec, id_paso_actual, id_paso_next, codigo_paso, serie, nro_comprobante, importe, importe_me, id_proveedor, provedor, tramite_pago, id_actividad, ruc, comentario, id_tramite_pago, id_pcompra, id_compra, user_paso, fecha_paso, estado_voucher_compra, arreglo, id_task, color_task, codigo_task, id_proceso, es_ret_det, es_servicio, estado_compra, estado_compra_orden, id_orden, pendiente, fecha_pecompra, id_pbancaria, codigo_tramite_pago) AS SELECT A.ID_ENTIDAD,
          ...
          A.ID_TIPOPEDIDO,
          (SELECT coalesce(X.NOMBRE, '') AS NOMBRE_PEDIDO
             FROM TIPO_PEDIDO X
            WHERE X.ID_TIPOPEDIDO = A.ID_TIPOPEDIDO)
             AS NOMBRE_PEDIDO,
          (SELECT coalesce(X.NOMBRE, '') AS NOMBRE_ACTIVIDAD
             FROM PSTO_ACTIVIDAD X
            WHERE X.ID_ACTIVIDAD = A.ID_ACTIVIDAD)
             AS NOMBRE_ACTIVIDAD,
          A.ID_AREAORIGEN,
          (SELECT coalesce(Y.ID_DEPTO || ' - ' || X.NOMBRE, '') AS AREA
          ...
          A.ACUERDO,
          (CASE
              WHEN (G.ID_PEDIDO IS NOT NULL AND G.ID_PEDIDO::text <> '') THEN G.FECHA_PEDIDO
              ELSE A.FECHA_PEDIDO
           END)
             AS FECHA_PEDIDO,
          (CASE
              WHEN (G.ID_PEDIDO IS NOT NULL AND G.ID_PEDIDO::text <> '') THEN G.FECHA_ENTREGA
          ...
          (SELECT Y.CODIGO
            ...
            WHERE X.ID_DEXEC = A.ID_DEXEC)
             AS CODIGO_PASO,
          coalesce(E.SERIE, D.SERIE) AS SERIE,
          coalesce(E.NUMERO, D.NUMERO) AS NRO_COMPROBANTE,
          coalesce(
             (CASE
                 WHEN (F.ID_PEDIDO IS NOT NULL AND F.ID_PEDIDO::text <> '') THEN D.IMPORTE
                 WHEN (E.ID_COMPRA IS NOT NULL AND E.ID_COMPRA::text <> '') THEN E.IMPORTE
                 ELSE D.IMPORTE
              END),
             0)
             AS IMPORTE,
          coalesce(
             (CASE
                 WHEN (F.ID_PEDIDO IS NOT NULL AND F.ID_PEDIDO::text <> '') THEN D.IMPORTE_ME
                 WHEN (E.ID_COMPRA IS NOT NULL AND E.ID_COMPRA::text <> '') THEN E.IMPORTE_ME
                 ELSE D.IMPORTE_ME
              END),
             0)
             AS IMPORTE_ME,
          coalesce(
             (CASE
                 WHEN (G.ID_PROVEEDOR IS NOT NULL AND G.ID_PROVEEDOR::text <> '') THEN G.ID_PROVEEDOR
                 WHEN (E.ID_PROVEEDOR IS NOT NULL AND E.ID_PROVEEDOR::text <> '') THEN E.ID_PROVEEDOR
                 ELSE D.ID_PROVEEDOR
              END),
             0)
             AS ID_PROVEEDOR,
          (SELECT UPPER(coalesce(X.PATERNO || ' ', '')|| coalesce(X.MATERNO || ', ', ''))|| X.NOMBRE --COALESCE (X.NOMBRE, X.PATERNO, X.MATERNO)
             FROM MOISES.PERSONA X
            WHERE X.ID_PERSONA =
                     coalesce(
                        (CASE
                            WHEN (G.ID_PROVEEDOR IS NOT NULL AND G.ID_PROVEEDOR::text <> '')
                            THEN
                               G.ID_PROVEEDOR
                            WHEN (E.ID_PROVEEDOR IS NOT NULL AND E.ID_PROVEEDOR::text <> '')
                            THEN
                               E.ID_PROVEEDOR
                            ELSE
                               D.ID_PROVEEDOR
...
UNION ALL
            ...
            ) X WHERE X.ID_PERSONA = coalesce(
                            (CASE WHEN (G.ID_PROVEEDOR IS NOT NULL AND G.ID_PROVEEDOR::text <> '')
                                THEN G.ID_PROVEEDOR
                                WHEN (E.ID_PROVEEDOR IS NOT NULL AND E.ID_PROVEEDOR::text <> '')
                                THEN E.ID_PROVEEDOR
                                ELSE D.ID_PROVEEDOR
          ...
          coalesce(A.ID_PBANCARIA, D.ID_PBANCARIA) AS ID_PBANCARIA,
