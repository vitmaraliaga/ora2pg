...
SELECT 
        ...
        A.ID_MES,
        --B.ID_COMPRA,
        coalesce((SELECT ID_SALDO FROM COMPRA_SALDO X WHERE B.ID_COMPRA = X.ID_COMPRA  AND A.ID_ANHO = X.ID_ANHO AND A.ID_PROVEEDOR = X.ID_PROVEEDOR),B.ID_COMPRA) AS ID_COMPRA,
        A.ID_PROVEEDOR,
        B.ID_COMPROBANTE,
        A.ID_MONEDA,
        A.ID_TIPOORIGEN,
...
SELECT A.ID_ENTIDAD,
          ...
          0 AS CORRELATIVO,
          A.ID_USER,
             CASE WHEN A.ID_MEDIOPAGO='001' THEN  'TLC.-' WHEN A.ID_MEDIOPAGO='007' THEN  'CHQ.-' WHEN A.ID_MEDIOPAGO='008' THEN  'EFE.-' END 
          || A.NUMERO
             AS OPER,
             NULL AS TIPOCAMBIO
     FROM CAJA_PAGO A
