...
CREATE OR REPLACE PROCEDURE eliseo.pkg_sales_school_sp_generar_asiento_col_anu (P_ID_VENTA bigint, P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
     ...
     --DETALLE
      ...
      WHILE cdet%FOUND LOOP
            ...
            if l_es_mensualidad_colegios = 'S' then 
                SELECT coalesce(max(total),0) into STRICT l_importe_cuota FROM jose.VSCH_CONTRATO_CRONOGRAMA
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_school_sp_generar_asiento_col_anu (P_ID_VENTA bigint, P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
