...
CREATE OR REPLACE FUNCTION eliseo.pkg_sales_industry_fc_nota (P_ID_VENTA bigint) RETURNS text AS $body$
DECLARE
...
BEGIN  
	...
	open VENTA FOR  
		SELECT DISTINCT CASE WHEN S_TIPODOC='07' THEN 'CC'  ELSE 'CD' END ||
			'|'||'0101'||
			...
			--'|'||REGEXP_REPLACE (nvl(TRIM(eliseo.pkg_sales_facturacion_fc_cliente_direccion(nvl(A.ID_CLIENTE,A.ID_CLIENTE_LEGAL))),'Sin Direccion'),'-','')|| 
			'|'||REGEXP_REPLACE(A.ID_TIPODOCUMENTO_CLI,'[A-Za-z]*', 'g')||
			...
			'|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(A.GRATUITA,0) = 0 THEN '0.00' WHEN A.GRATUITA BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(A.GRATUITA,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(A.GRATUITA,'9999999990D99')),',','.') END)  ELSE (CASE  WHEN A.GRATUITA_ME = 0 THEN '0.00' WHEN A.GRATUITA_ME BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(A.GRATUITA_ME,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(A.GRATUITA_ME,'9999999990D99')),',','.') END) END ||
			'|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(A.VALOR_EXPORTACION,0) = 0 THEN '0.00' WHEN A.VALOR_EXPORTACION BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(A.VALOR_EXPORTACION,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(A.VALOR_EXPORTACION,'9999999999D99')),',','.') END)  ELSE (CASE  WHEN A.VALOR_EXPORTACION_ME = 0 THEN '0.00' WHEN A.VALOR_EXPORTACION_ME BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(A.VALOR_EXPORTACION_ME,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(A.VALOR_EXPORTACION_ME,'9999999999D99')),',','.') END) END ||
			'|'||'0.00'|| --27
			'|'||'0.00'|| --28
			'|'||'0.00'|| --29
			'|'||'0.00'|| --30
			'|'||'0.00'|| --31
	       	'|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0) = 0 THEN '0.00' WHEN coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0) BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0),'9999999990D99')),',','.') END)  ELSE (CASE  WHEN coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME,0) = 0 THEN '0.00' WHEN coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME,0) BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME,0),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME,0),'9999999990D99')),',','.') END) END || 
	    	'|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0)+coalesce(A.IGV,0) = 0 THEN '0.00' WHEN coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0)+coalesce(A.VALOR_EXPORTACION,0)+coalesce(A.IGV,0) BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.IGV,0),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0)+coalesce(A.IGV,0),'9999999999D99')),',','.') END)  ELSE (CASE  WHEN coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME+A.IGV_ME,0) = 0 THEN '0.00' WHEN coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME+A.IGV_ME,0) BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME+A.IGV_ME,0),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME+A.IGV_ME,0),'9999999999D99')),',','.') END) END || 
...
UNION ALL
 		...
 		SELECT  CASE WHEN S_TIPODOC='07' THEN 'DC'  ELSE 'DD' END ||
 			'|'||ROWNUM||
			...
			'|'||'NIU'|| 
			'|'||trim(both to_char(b.cantidad,'9999999990D99'))|| 
            ...
            '|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(B.PRECIO,0) = 0 THEN '0.00' WHEN B.PRECIO BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(B.PRECIO,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.PRECIO,'9999990D99999')),',','.') END)  ELSE (CASE  WHEN B.PRECIO_ME = 0 THEN '0.00' WHEN B.PRECIO_ME BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(B.PRECIO_ME,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.PRECIO_ME,'9999990D99999')),',','.') END) END ||
			...
			'|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(B.IGV,0) = 0 THEN '0.00' WHEN coalesce(B.IGV,0) BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(coalesce(B.IGV,0),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(coalesce(B.IGV,0),'9999999999D99')),',','.') END)  ELSE (CASE  WHEN coalesce(B.IGV_ME,0) = 0 THEN '0.00' WHEN coalesce(B.IGV_ME,0) BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(coalesce(B.IGV_ME,0),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(coalesce(B.IGV_ME,0),'9999999999D99')),',','.') END) END ||
			'|'||'0.00'|| 
			'|'||'0.00'|| 
			'|'||'0.00'|| 
			'|'||''|| 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.pkg_sales_industry_fc_nota (P_ID_VENTA bigint) FROM PUBLIC;
