...
CREATE OR REPLACE PROCEDURE eliseo.pkg_inventories_sp_insert_detalle_receta_tr (P_ID_MOVIMIENTO bigint, P_ID_DINAMICA bigint,P_CANTIDAD bigint,P_COSTO bigint, P_ERROR OUT bigint,P_MSN_ERROR OUT text) AS $body$
DECLARE
    ...
    SELECT 
    B.ID_ARTICULO,round((P_CANTIDAD)::numeric,2),round((P_COSTO)::numeric,2)--C.COSTO
...
BEGIN
        SELECT ID_RECETA,ID_ANHO INTO STRICT L_ID_RECETA,L_ID_ANHO FROM INVENTARIO_MOVIMIENTO
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_inventories_sp_insert_detalle_receta_tr (P_ID_MOVIMIENTO bigint, P_ID_DINAMICA bigint,P_CANTIDAD bigint,P_COSTO bigint, P_ERROR OUT bigint,P_MSN_ERROR OUT text) FROM PUBLIC;
