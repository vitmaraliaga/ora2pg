...
CREATE OR REPLACE PROCEDURE eliseo.pkg_purchases_sp_compra_ple_82_txt ( P_ID_EMPRESA bigint, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_ID_MES bigint, P_ID_ANHO bigint, OUT CURSOR REFCURSOR ) AS $body$
BEGIN
    ...
    SELECT
					...
					trim(both serie)||'|'||  
					trim(both numdoc)||'|'||  
					trim(both CASE WHEN importe_e>0 THEN importe_e ELSE importe END -CASE WHEN otros_e>0 THEN otros_e ELSE otros END )||'|'||  
					trim(both CASE WHEN otros_e>0 THEN otros_e ELSE otros END)||'|'||
					trim(both CASE WHEN importe_e>0 THEN importe_e ELSE importe END)||'|'||
					trim(both tipo_doc_parent)||'|'||
					trim(both serie_parent)||'|'||
					trim(both anio_parent)||'|'||
					trim(both numero_parent)||'|'||
					trim(both igv_total)||'|'||
					...
					''||'|'||
					ruc||'|'||
					trim(both num_doc_benficiario)||'|'||
					trim(both nombre_benficiario)||'|'||
					trim(both cod_pais_benficiario)||'|'||
					''||'|'||
					''||'|'||
					''||'|'||
					''||'|'||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_purchases_sp_compra_ple_82_txt ( P_ID_EMPRESA bigint, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_ID_MES bigint, P_ID_ANHO bigint, OUT CURSOR REFCURSOR ) FROM PUBLIC;
