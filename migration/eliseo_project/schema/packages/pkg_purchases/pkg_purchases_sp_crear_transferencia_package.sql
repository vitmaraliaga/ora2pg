...
BEGIN
            ...
            AND ID_PROVEEDOR = case when coalesce(P_ID_PROVEEDOR::text, '') = '' then ID_PROVEEDOR else P_ID_PROVEEDOR end  -- Si el valor de P_ID_PROVEEDOR es null entonces no validamos el proveedor (ello ocurre en los casos especiales de la Alma Mater) atte Ame del Perú
            ...
            EXCEPTION
            WHEN OTHERS THEN
                P_MSN := 'ERROR AL REGISTRAR EL AJUSTE: '||SQLSTATE||' -ERROR- '||SQLERRM||''||SQLSTATE;
                P_ERROR := 1;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_purchases_sp_crear_transferencia (P_ID_ENTIDAD bigint,P_ID_DEPTO text,P_ID_ANHO bigint,P_ID_MES bigint,P_ID_PERSONA bigint,P_ID_COMPRA bigint, ID_DINAMICA bigint,P_ID_VOUCHER bigint,P_FECHA timestamp(0), P_IMPORTE bigint,P_IMPORTE_ME bigint,P_DC text, P_ID_AJUSTE OUT bigint,P_ERROR OUT bigint,P_MSN OUT text,P_ESTADO text default '1',P_ID_PROVEEDOR bigint DEFAULT NULL) FROM PUBLIC;
