...
CREATE OR REPLACE FUNCTION eliseo.pkg_purchases_fc_autorizador (P_ID_REGISTRO bigint) RETURNS varchar AS $body$
DECLARE

        NOM_PERSONA varchar(100);

BEGIN
        SELECT EMAIL INTO STRICT NOM_PERSONA
        FROM (
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.pkg_purchases_fc_autorizador (P_ID_REGISTRO bigint) FROM PUBLIC;
