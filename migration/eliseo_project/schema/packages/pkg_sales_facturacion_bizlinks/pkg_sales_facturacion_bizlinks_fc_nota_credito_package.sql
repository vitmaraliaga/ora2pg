...
CREATE OR REPLACE FUNCTION eliseo.pkg_sales_facturacion_bizlinks_fc_nota_credito (P_ID_VENTA bigint) RETURNS varchar AS $body$
DECLARE
    ...
    SELECT 'CC'||'|'||'0101'||
        ...
        '|'||(CASE WHEN SUBSTR(a.serie_ref,1,1) = 'F' THEN '6'
              WHEN LENGTH(FC_DOCUMENTO_CLIENTE(coalesce(A.ID_CLIENTE_LEGAL,A.ID_CLIENTE))) =11 THEN '6'
              WHEN LENGTH(FC_DOCUMENTO_CLIENTE(coalesce(A.ID_CLIENTE_LEGAL,A.ID_CLIENTE))) =8 THEN '1'
              ELSE '0'
              END)
        || 
    ...
    '|'||FC_NOMBRE_PERSONA(coalesce(A.ID_CLIENTE_LEGAL,A.ID_CLIENTE))||
		'|'||REGEXP_REPLACE(coalesce(trim(both pkg_sales_facturacion_bizlinks_fc_cliente_direccion(coalesce(A.ID_CLIENTE_LEGAL,A.ID_CLIENTE))),'Sin Direccion'),'-','', 'g')||
		'|'||''|| 
...
UNION ALL
 ...
 SELECT  'DC'||	'|'||ROWNUM||
     ...
     /*   SELECT 
                ...
                INITCAP(FC_NOMBRE_PERSONA(A.ID_CLIENTE))||'|'||pkg_sales_facturacion_bizlinks_fc_cliente_direccion(A.ID_CLIENTE)||'|'||FC_PHONO(A.ID_CLIENTE)||'|'||FC_EMAIL(A.ID_CLIENTE)||'||||||'||
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.pkg_sales_facturacion_bizlinks_fc_nota_credito (P_ID_VENTA bigint) FROM PUBLIC;
