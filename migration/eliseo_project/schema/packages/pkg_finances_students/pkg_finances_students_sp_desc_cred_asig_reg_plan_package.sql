...
CREATE OR REPLACE PROCEDURE eliseo.pkg_finances_students_sp_desc_cred_asig_reg_plan (P_ID_ALUMNO_CONTRATO bigint,P_ID_CRITERIO_SEMESTRE bigint,P_TIPO_ALUMNO text,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
BEGIN
    ...
    INTO STRICT l_id_semestre_programa,l_id_plan_programa
...
--       goto salida_var_regular;
    ...
    SELECT  coalesce(IMPORTE,0) into STRICT  l_importe  FROM MAT_PLAN_COSTO a
    WHERE a.id_plan=l_id_plan
    AND A.ID_SEMESTRE_PROGRAMA=l_id_semestre_programa
    AND a.ID_CRITERIO=l_id_criterio;
    --=====
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_finances_students_sp_desc_cred_asig_reg_plan (P_ID_ALUMNO_CONTRATO bigint,P_ID_CRITERIO_SEMESTRE bigint,P_TIPO_ALUMNO text,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
