...
CREATE OR REPLACE PROCEDURE eliseo.pkg_finances_students_sp_desc_inst_educ (P_ID_ALUMNO_CONTRATO bigint,P_ID_CRITERIO_SEMESTRE bigint,P_TIPO_ALUMNO text,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
BEGIN
    ...
    IF l_contar>0 THEN
      ...
      SELECT COALESCE(ENSENANZA,0),TIPO_ENSE INTO STRICT l_importe,l_tipo_valor FROM FIN_ALUMNO_DESCUENTO_VICE
      WHERE ID_PERSONA=l_id_persona
      and tipo_dscto='I'
      AND ESTADO='1';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_finances_students_sp_desc_inst_educ (P_ID_ALUMNO_CONTRATO bigint,P_ID_CRITERIO_SEMESTRE bigint,P_TIPO_ALUMNO text,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
