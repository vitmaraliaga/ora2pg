...
CREATE OR REPLACE PROCEDURE eliseo.pkg_finances_students_sp_generar_asiento_matricula (P_ID_ALUMNO_CONTRATO bigint,P_ID_ENTIDAD bigint,P_ID_VENTA bigint,P_APLAZADO text,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
BEGIN
      ...
      SELECT (row_number() OVER ( ORDER BY A.ID_CRITERIO_SEMESTRE ASC ))  AS items,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_finances_students_sp_generar_asiento_matricula (P_ID_ALUMNO_CONTRATO bigint,P_ID_ENTIDAD bigint,P_ID_VENTA bigint,P_APLAZADO text,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
