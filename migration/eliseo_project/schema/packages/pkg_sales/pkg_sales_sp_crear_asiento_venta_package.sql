...
BEGIN
            L_ERROR :=0;
            SELECT ID_CLIENTE INTO STRICT L_ID_CLIENTE FROM VENTA WHERE ID_VENTA = P_ID_VENTA;
            IF P_MODO = '1' THEN
                OPEN casi;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_sp_crear_asiento_venta (P_ID_VENTA bigint,P_ID_DINAMICA bigint,P_ID_CUENTAAASI text,P_ID_RESTRICCION text,P_ID_CTACTE text,P_ID_FONDO bigint,P_ID_DEPTO text,P_IMPORTE bigint,P_IMPORTE_ME bigint,P_DESCRIPCION text, P_EDITABLE text, P_DC text,P_AGRUPA text,P_MODO text,P_ERROR OUT bigint, P_MSN OUT text) FROM PUBLIC;
