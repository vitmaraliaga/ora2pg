...
CREATE OR REPLACE PROCEDURE eliseo.pkg_presupuesto_sp_conservatorio_proceso (P_ID_ENTIDAD bigint,P_ID_DEPTO_PADRE text,P_ID_ANHO bigint,P_ID_AUXILIAR bigint,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
     ...
     SELECT COALESCE(MAX(ID_PREGRADO_PROCESO),0) INTO STRICT l_contar FROM PSTO_PREGRADO_PROCESO;
      ...
      FROM (
        ...
        WHEN  MATCHED THEN UPDATE SET  C.TOTAL_ENSE_I=t.ENSE_I,  C.TOTAL_ENSE_II=t.ENSE_II,C.TOTAL_MAT_I=t.MAT_I,C.TOTAL_MAT_II=t.MAT_II;
         ...
         CALL pkg_presupuesto_sp_conservatorio_concepto(P_ID_ENTIDAD,P_ID_DEPTO_PADRE,P_ID_ANHO,P_ID_AUXILIAR,l_error,l_msgerror);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_presupuesto_sp_conservatorio_proceso (P_ID_ENTIDAD bigint,P_ID_DEPTO_PADRE text,P_ID_ANHO bigint,P_ID_AUXILIAR bigint,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
