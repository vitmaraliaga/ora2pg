...
CREATE OR REPLACE PROCEDURE eliseo.pkg_sales_finances_sp_resultado_costos_sl (P_ID_ENTIDAD bigint,P_ID_ANHO bigint,P_ID_MES bigint,P_ID_USER bigint) AS $body$
DECLARE
        ...
        L_CU decimal(10,2);
        L_CU_SEDE decimal(10,2);
        L_IMP decimal(10,2);
        L_IMP_SEDE decimal(10,2);
        L_ID_PROGRAMA_ESTUDIO varchar(20);
        L_RESULTADO_SEDE decimal(10,2);
        L_RESULTADO decimal(10,2);
        L_NIVEL_CAMPUS bigint;
        L_CANTIDAD_CAMPUS bigint;
        L_IMPORTE_SEDE decimal(10,2);
        ...
        L_CODIGO_CUENTA varchar(20) := 'SSSL';
        L_NIVEL_ESCUELA bigint;
        L_NIVEL_DEPTO bigint;

        
        
BEGIN 
            SELECT ID_NIVEL INTO STRICT L_NIVEL_DEPTO FROM CONTA_NIVEL WHERE ID_ENTIDAD = P_ID_ENTIDAD AND ID_TIPOREPORTE = 3 AND CODIGO = 'C01';
            ...
            SELECT (CASE WHEN P_ID_MES BETWEEN 1 AND 6 THEN 1 ELSE 2 END) AS CICLO INTO STRICT L_CICLO;
             ...
             --CANTIDAD ALUMNOS
            SELECT SUM(A.CANTIDAD) INTO STRICT L_CANTIDAD_CAMPUS
            ...
            --SOPORTE LOCAL SEDE
            SELECT SUM(A.IMPORTE) INTO STRICT L_RESULTADO_SEDE
            ...
            OPEN C_NIVELES;
                ...
                WHILE C_NIVELES%FOUND LOOP
                    ...
                    BEGIN 
                        ...
                        IF L_NIVEL_ESCUELA <> L_NIVEL_CAMPUS THEN
                            ...
                            IF L_NIVEL_CAMPUS <> 1 THEN
                                IF L_NIVEL_CAMPUS = 5 THEN
                                    SELECT ID_NIVEL INTO STRICT L_NIVEL_DEPTO FROM CONTA_NIVEL WHERE ID_ENTIDAD = P_ID_ENTIDAD AND ID_TIPOREPORTE = 3 AND CODIGO = 'C02';
                                END IF;
                                IF L_NIVEL_CAMPUS = 6 THEN
                                    SELECT ID_NIVEL INTO STRICT L_NIVEL_DEPTO FROM CONTA_NIVEL WHERE ID_ENTIDAD = P_ID_ENTIDAD AND ID_TIPOREPORTE = 3 AND CODIGO = 'C03';
                                END IF;
                                --CANTIDAD ALUMNOS
                                SELECT SUM(A.CANTIDAD) INTO STRICT L_CANTIDAD_CAMPUS
                                FROM CONTA_NIVEL_ALUMNOS A JOIN CONTA_NIVEL B ON A.ID_NIVEL = B.ID_NIVEL JOIN CONTA_NIVEL C ON B.ID_PARENT = C.ID_NIVEL
                        ...
                        IF L_CANT > 0 THEN
                            IF (coalesce(L_RESULTADO_SEDE,0)+coalesce(L_RESULTADO,0)) <> 0 THEN
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_finances_sp_resultado_costos_sl (P_ID_ENTIDAD bigint,P_ID_ANHO bigint,P_ID_MES bigint,P_ID_USER bigint) FROM PUBLIC;
