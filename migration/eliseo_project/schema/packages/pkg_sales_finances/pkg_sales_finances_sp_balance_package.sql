...
CREATE OR REPLACE PROCEDURE eliseo.pkg_sales_finances_sp_balance (P_ID_ENTIDAD bigint,P_ID_ANHO bigint,P_ID_MES bigint,P_NIVEL bigint) AS $body$
DECLARE
        ...
        FROM ELISEO.CONTA_CUENTA A WHERE ID_CUENTA IN (64,65,66,67,68)
  UNION ALL
...
BEGIN 
            DELETE FROM EF_BALANCE_PATRIMONIAL WHERE ID_ENTIDAD = P_ID_ENTIDAD AND ID_ANHO = P_ID_ANHO AND ID_MES = P_ID_MES AND ID_NIVEL IN (SELECT ID_NIVEL FROM CONTA_NIVEL WHERE ID_TIPOREPORTE = 4 AND NIVEL = P_NIVEL);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_finances_sp_balance (P_ID_ENTIDAD bigint,P_ID_ANHO bigint,P_ID_MES bigint,P_NIVEL bigint) FROM PUBLIC;
