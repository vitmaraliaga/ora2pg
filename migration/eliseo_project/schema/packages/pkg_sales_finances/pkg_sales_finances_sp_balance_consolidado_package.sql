...
CREATE OR REPLACE PROCEDURE eliseo.pkg_sales_finances_sp_balance_consolidado (P_ID_ENTIDAD bigint,P_ID_ANHO bigint,P_ID_MES bigint) AS $body$
DECLARE
...
BEGIN 
            DELETE FROM EF_BALANCE_PATRIMONIAL WHERE ID_ENTIDAD = P_ID_ENTIDAD AND ID_ANHO = P_ID_ANHO AND ID_MES = P_ID_MES AND ID_NIVEL IN (SELECT ID_NIVEL FROM CONTA_NIVEL WHERE ID_TIPOREPORTE = 4 AND NIVEL = 0);

            SELECT ID_PARENT,ID_NIVEL,NOMBRE,NOMBRE INTO STRICT L_ID_NIVEL_PARENT,L_ID_NIVEL,L_NOMBRE_NIVEL_PARENT,L_NOMBRE_NIVEL
            FROM CONTA_NIVEL WHERE ID_TIPOREPORTE = 4 AND NIVEL = 0 AND (ID_PARENT IS NOT NULL AND ID_PARENT::text <> '');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_finances_sp_balance_consolidado (P_ID_ENTIDAD bigint,P_ID_ANHO bigint,P_ID_MES bigint) FROM PUBLIC;
