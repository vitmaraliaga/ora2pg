...
DECLARE
    ...
    curdet CURSOR FOR 
    SELECT 
    (PARAMETRO)::numeric  as id_sedearea,
    (VALOR)::numeric  as importe,
    VALOR1 as glosa
    from ELISEO.PAY_ORDEN_PARAM
    where id_orden=P_ID_ORDEN
    order by ID_ORDEN_PARAM;

    l_id_depto_ead varchar(10):='';
...
BEGIN
      ...
      BEGIN
        ...
        l_id_depto_ead:=null;
        select count(1) into STRICT l_contar from ORG_SEDE_AREA where id_sedearea=cur.id_sedearea;
        if l_contar>0 then
          select id_depto into STRICT l_id_depto_ead from ORG_SEDE_AREA where id_sedearea=cur.id_sedearea;
        end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_pay_sp_ventas_alumnoead ( P_ID_ORDEN bigint, P_ID_PERSONA bigint, P_OPERACION text, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_ID_ANHO bigint, P_ID_MES bigint, P_IMPORTE bigint, P_IP text, P_ID_DINAMICA bigint, P_ID_CLIENTE bigint, P_ID_COMPROBANTE text, P_ID_TIPOVENTA bigint, P_ID_MONEDA bigint, P_TC bigint, P_IMPORTE_ME bigint, P_CANTIDAD bigint DEFAULT NULL, P_VENTAS OUT text, P_VENTAS_IMP out text, P_ERROR OUT bigint, P_MSGERROR out text ) FROM PUBLIC;
