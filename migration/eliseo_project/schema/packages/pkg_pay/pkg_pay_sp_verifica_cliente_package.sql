...
DECLARE
    ...
    l_id bigint;
    l_id_emp bigint;
    l_nombre_visa varchar(100);

BEGIN
  
        l_id_dinamica:= (P_COD_TRANSACCION)::numeric;
...
;
        ...
        select count(1) into STRICT l_contar from CONTA_DINAMICA where id_dinamica=l_id_dinamica;
        if l_contar=0 then
...
--             GOTO salida_validadniid;
        ...
        select nombre into STRICT l_glosa from CONTA_DINAMICA where id_dinamica=l_id_dinamica;
...
UNION ALL
            ...
            IF l_contar>0 THEN
                select count(1) into STRICT l_contar from moises.persona a, moises.persona_juridica b where a.id_persona=b.id_persona and b.id_ruc = P_RUC;
                if l_contar > 0 then
                    select a.id_persona, a.nombre into STRICT l_id_persona,l_razonsocial
