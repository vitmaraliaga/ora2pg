...
CREATE OR REPLACE PROCEDURE eliseo.pkg_pay_sp_deposito_alumnoiddni_visa ( P_ID_ORDEN bigint, P_IMPORTE bigint, P_IP text, P_ID OUT text, P_ERROR OUT bigint, P_MSGERROR out text ) AS $body$
DECLARE
...
BEGIN
        ...
        SELECT COUNT(1) INTO STRICT l_contar FROM JONAS.EVENTO_EVENTO_PARTICIPANTE WHERE ID_EVENTO_PARTICIPANTE = P_ID_ORIGEN;
        IF l_contar >= 1 THEN
            SELECT coalesce(CANTIDAD,1) INTO STRICT L_CANTIDAD FROM JONAS.EVENTO_EVENTO_PARTICIPANTE WHERE ID_EVENTO_PARTICIPANTE = P_ID_ORIGEN;
        END IF;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_pay_sp_deposito_alumnoiddni_visa ( P_ID_ORDEN bigint, P_IMPORTE bigint, P_IP text, P_ID OUT text, P_ERROR OUT bigint, P_MSGERROR out text ) FROM PUBLIC;
