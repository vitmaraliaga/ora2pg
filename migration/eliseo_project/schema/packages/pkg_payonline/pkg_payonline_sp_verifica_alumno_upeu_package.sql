...
CREATE OR REPLACE PROCEDURE eliseo.pkg_payonline_sp_verifica_alumno_upeu ( P_ID_PERSONA bigint, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_COD_TRANSACCION text, OUT CURSOR REFCURSOR ) AS $body$
DECLARE
...
;
        ...
        CALL pkg_payonline_sp_valida_deposito(P_ID_ENTIDAD,P_ID_DEPTO,P_COD_TRANSACCION,l_id_anho,l_id_mes,l_id_user,l_id_dinamica,l_glosa, l_error,l_msgerror);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_payonline_sp_verifica_alumno_upeu ( P_ID_PERSONA bigint, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_COD_TRANSACCION text, OUT CURSOR REFCURSOR ) FROM PUBLIC;
