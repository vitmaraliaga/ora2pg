...
CREATE OR REPLACE FUNCTION eliseo.pkg_sales_facturacion_fc_venta (P_ID_VENTA bigint) RETURNS varchar AS $body$
DECLARE
...
UNION ALL
                ...
                SELECT  'DB'||	
                    ...
                    '|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(B.DESCUENTO,0) = 0 THEN '0.00' WHEN B.DESCUENTO BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(B.DESCUENTO,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.DESCUENTO,'9999999990D99')),',','.') END)  ELSE (CASE  WHEN B.DESCUENTO_ME = 0 THEN '0.00' WHEN B.DESCUENTO_ME BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(B.BASE_ME,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.BASE_ME,'9999999990D99')),',','.') END) END ||
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.pkg_sales_facturacion_fc_venta (P_ID_VENTA bigint) FROM PUBLIC;
