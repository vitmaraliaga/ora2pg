...
FUNCTION        FC_OBT_MESES_MISMO_RUC(P_ID_ENTIDAD NUMBER, P_ID_PERSONA NUMBER, FECHA_INI DATE, FECHA_FIN DATE) RETURN NUMBER
IS
l_contar int:=0;
...
BEGIN
		...
		IF l_contar > 0 THEN
			IF l_contar = 1 THEN
				SELECT MONTHS_BETWEEN(RECORDS.SALIDA + 1, RECORDS.INGRESO) INTO total_meses
				FROM (
			...
			END IF;
		END IF;
	RETURN TRUNC(mod(total_meses, 12));
END;
