...
FUNCTION        FC_CONTA_CUO_CORRELATIVO(P_ID_TIPOORIGEN NUMBER, P_ID_MOV NUMBER, p_id_entidad NUMBER, p_id_anho NUMBER, p_id_mes number) RETURN VARCHAR2 AS
...
BEGIN
	...
	IF P_ID_TIPOORIGEN = 1 THEN 
	    ...
	    SELECT max('M'||d.NUM_AASI) INTO L_NOMBRE FROM VW_CONTA_DIARIO d
	...
	ELSE 
		...
		SELECT count(1), max('M'||d.NUM_AASI), max(cod_aasi) INTO L_CONTAR, L_NOMBRE, L_COD_AASI FROM VW_CONTA_DIARIO d
		WHERE trim(to_char(P_ID_TIPOORIGEN,'999'))||'-'||trim(to_char(P_ID_MOV,'9999999999999999999')) = d.OBSERVACION
		AND d.ID_ENTIDAD = p_id_entidad
		AND d.ID_ANHO = p_id_anho
		AND d.ID_MES = p_id_mes
	   ...
	   SELECT
		...
		IF L_CONTAR = 0 THEN 
                SELECT MAX('M'||A.CUO_CORRELATIVO ) INTO L_NOMBRE FROM CONTA_DIARIO_CUO_AUX A
