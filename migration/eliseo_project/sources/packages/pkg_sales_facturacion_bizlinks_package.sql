...
CREATE OR REPLACE PACKAGE PKG_SALES_FACTURACION_BIZLINKS AS 
	 ...
	 PROCEDURE SP_VENTA_ELECTRONICA_BIZLINKS(P_ID_VENTA IN NUMBER);
...
END PKG_SALES_FACTURACION_BIZLINKS;


CREATE OR REPLACE PACKAGE BODY PKG_SALES_FACTURACION_BIZLINKS AS
    ...
    --SELECT ID_COMPROBANTE,ID_TIPOVENTA INTO S_TIPODOC, L_ID_TIPOVENTA FROM VENTA WHERE ID_VENTA=P_ID_VENTA;
        ...
        IF S_TIPODOC='03' THEN -- BOLETA
                ...
                SELECT distinct'CB'||'|'||'0101'||
                    ...
                    '|'||REGEXP_REPLACE (nvl(TRIM(PKG_SALES_FACTURACION_BIZLINKS.FC_CLIENTE_DIRECCION(nvl(A.ID_CLIENTE_LEGAL,A.ID_CLIENTE))),'Sin Direccion'),'-','')|| 
                    '|'||''|| 
                ...
                SELECT  'DB'||	
                    ...
                    '|'||DECODE(A.ID_MONEDA, '7',(CASE  WHEN nvl(B.DESCUENTO,0) = 0 THEN '0.00' WHEN B.DESCUENTO BETWEEN 0.01 AND 0.99 THEN REPLACE(TRIM(TO_CHAR(B.BASE,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.BASE,'9999999990D99')),',','.') END),(CASE  WHEN B.DESCUENTO_ME = 0 THEN '0.00' WHEN B.DESCUENTO_ME BETWEEN 0.01 AND 0.99 THEN REPLACE(TRIM(TO_CHAR(B.BASE_ME,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.BASE_ME,'9999999990D99')),',','.') END))||
...
open VENTA FOR  
 ...
 SELECT  'DC'||	'|'||ROWNUM||
     ...
     /*   SELECT 
                ...
                INITCAP(FC_NOMBRE_PERSONA(A.ID_CLIENTE))||'|'||PKG_SALES_FACTURACION_BIZLINKS.FC_CLIENTE_DIRECCION(A.ID_CLIENTE)||'|'||FC_PHONO(A.ID_CLIENTE)||'|'||FC_EMAIL(A.ID_CLIENTE)||'||||||'||
...
FUNCTION FC_NOTA_DEBITO(P_ID_VENTA IN NUMBER) RETURN VARCHAR2 IS
 ...
 SELECT  'DD'||	'|'||ROWNUM||
    ...
    END;
     ...
     PROCEDURE SP_VENTA_ELECTRONICA_BIZLINKS(P_ID_VENTA IN NUMBER) IS
        L_ID_ENTIDAD NUMBER;
    ...
    BEGIN  
        ...
        --IF 7 = 7 THEN 
            ...
            IF L_COD_ELECTRONICO = '03' OR L_COD_ELECTRONICO = '01' THEN
                SELECT TO_CLOB(PKG_SALES_FACTURACION_BIZLINKS.FC_VENTA(P_ID_VENTA)) INTO L_DET FROM DUAL;
            END IF;
            IF L_COD_ELECTRONICO = '07' THEN 
                SELECT TO_CLOB(PKG_SALES_FACTURACION_BIZLINKS.FC_NOTA_CREDITO(P_ID_VENTA)) INTO L_DET FROM DUAL;
            END IF;
            IF L_COD_ELECTRONICO = '08' THEN 
                SELECT TO_CLOB(PKG_SALES_FACTURACION_BIZLINKS.FC_NOTA_DEBITO(P_ID_VENTA)) INTO L_DET FROM DUAL;
            END IF;
            --FACTURACION ELECTRONICA LAMB
                        
            IF L_TIPOEMISORID = 'BIZLINKS' THEN
               
                     INSERT INTO VENTA_ELECTRONICA_BIZLINKS(EMISORID,ORIGENID,FECHA,TIPO,NROCOMPROBANTE,COMPROBANTE,NOMBREARCHIVO, PROCESADO, ENVIADO)
    ...
    END SP_VENTA_ELECTRONICA_BIZLINKS;
    ...
    BEGIN  
        ...
        IF L_TIPOEMISORID = 'BIZLINKS' THEN
            ...
            ELISEO.PKG_SALES_FACTURACION_BIZLINKS.SP_VENTA_ELECTRONICA_BIZLINKS(P_ID_VENTA);
