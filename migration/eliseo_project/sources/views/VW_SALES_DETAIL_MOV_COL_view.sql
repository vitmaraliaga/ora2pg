...
CREATE OR REPLACE VIEW vw_sales_detail_mov_col (id_entidad, id_depto, id_anho, id_mes, id_voucher, id_venta, id_articulo, id_cliente, id_sucursal, id_comprobante, id_moneda, serie, numero, documento_venta, serie_mov, numero_mov, documento_mov, fecha, glosa, detalle, importe, importe_me, id_tipotransaccion, id_depto_ccosto, id_usuario) AS SELECT ID_ENTIDAD,ID_DEPTO,ID_ANHO,ID_MES,ID_VOUCHER,ID_VENTA,ID_ARTICULO,ID_CLIENTE,ID_SUCURSAL,ID_COMPROBANTE,ID_MONEDA,SERIE,NUMERO,DOCUMENTO_VENTA,SERIE_MOV,NUMERO_MOV,DOCUMENTO_MOV,FECHA,GLOSA,DETALLE,IMPORTE,IMPORTE_ME,ID_TIPOTRANSACCION,ID_DEPTO_CCOSTO,ID_USUARIO FROM (  
  ...
  SELECT A.ID_ENTIDAD,
          ...
          A.GLOSA,
          B.DETALLE,
          B.IMPORTE - (COALESCE(N.IMPORTE, 0)),
          B.IMPORTE_ME,
   ...
   LEFT OUTER JOIN (
   SELECT X.ID_ANHO, X.ID_VENTA, X.ID_PARENT, Y.ID_ARTICULO, Y.IMPORTE FROM VENTA X INNER JOIN VENTA_DETALLE Y ON X.ID_VENTA = Y.ID_VENTA
   WHERE X.ID_COMPROBANTE IN ('07','08') 
   			AND X.ID_TIPONOTA IN ('01','02', '06', '07')
   			AND X.ESTADO = 1
   			AND X.TOTAL <> 0
   ) N ON N.ID_PARENT = A.ID_VENTA AND N.ID_ARTICULO = B.ID_ARTICULO AND N.ID_ANHO = A.ID_ANHO
...
UNION ALL 
   ...
   AND A.ID_TIPONOTA NOT IN ('01','02', '06', '07')
   AND A.ESTADO = 1
   UNION ALL 
   SELECT A.ID_ENTIDAD,
          A.ID_DEPTO,
   ...
   AND A.ID_TIPONOTA IN ('01','02', '06', '07')
   AND A.ESTADO = 1
   UNION ALL
   SELECT A.ID_ENTIDAD,
          A.ID_DEPTO,
