...
CREATE OR REPLACE VIEW vw_payments_det (id_pago, id_entidad, id_depto, id_anho, id_mes, id_user, id_voucher, id_ctabancaria, id_chequera, id_tipoorigen, id_origen, numero, fecha, id_compra, id_proveedor, importe, importe_me, detalle, tipo) AS SELECT A.ID_PAGO,
      A.ID_ENTIDAD,
      A.ID_DEPTO,
      A.ID_ANHO,
      A.ID_MES,
...
SELECT 
    ...
    A.NRO_OPERACION,
    A.FECHA_EMISION,
    NVL(B.ID_COMPRA,C.ID_VENTA) AS ID_COMPRA,
    A.ID_PROVEEDOR,
    NVL(B.IMPORTE,C.IMPORTE) AS IMPORTE,
    NVL(B.IMPORTE_ME,C.IMPORTE_ME) AS IMPORTE_ME,
    NVL(B.DETALLE,C.DETALLE) AS DETALLE,
    'D' AS TIPO
