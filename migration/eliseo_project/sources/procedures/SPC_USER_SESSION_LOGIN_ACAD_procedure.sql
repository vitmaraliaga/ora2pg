...
PROCEDURE        spc_user_session_login_acad (
    p_id_persona       int,
    p_token       varchar2,
...
BEGIN
   ...
   SELECT COUNT(1), max(X.id_entidad) into l_contar, l_entidad FROM CONTA_ENTIDAD_USUARIO X WHERE X.ID_ENTIDAD = 7124 AND X.ID_PERSONA = p_id_persona
...
--	 END IF ;
    ...
    IF l_contar = 0 THEN
    	  ...
    	  insert into CONTA_ENTIDAD_USUARIO
                (id_entidad, id_persona, estado) 
            values  (7124,p_id_persona,1);   
              SELECT COUNT(1), max(X.id_entidad) into l_contar, l_entidad FROM CONTA_ENTIDAD_USUARIO X WHERE X.ID_ENTIDAD = 7124 AND X.ID_PERSONA = p_id_persona;-- AND X.ESTADO = 1;
    END IF;
   ...
   -- ELSIF l_contar > 1 THEN
    ...
    SELECT COUNT(1) into l_contar FROM MOISES.TRABAJADOR WHERE ID_PERSONA = p_id_persona AND id_entidad= 7124 AND ID_SITUACION_TRABAJADOR = '1';
    IF l_contar > 0 THEN
        
        SELECT ID_SEDEAREA INTO L_ID_SEDEAREA FROM MOISES.TRABAJADOR WHERE ID_PERSONA = p_id_persona AND id_entidad= 7124 AND ID_SITUACION_TRABAJADOR = '1';
        SELECT COUNT(1) into l_contar FROM ORG_SEDE_AREA WHERE ID_SEDEAREA = L_ID_SEDEAREA;
        IF l_contar > 0 THEN
            SELECT SUBSTR(ID_DEPTO,1,1) INTO l_dpto FROM ORG_SEDE_AREA WHERE ID_SEDEAREA = L_ID_SEDEAREA;
            IF l_dpto IN ('0','2','3','4') THEN
                l_dpto := '1';
            END IF;
        ELSE
            SELECT A.ID_DEPTO into  l_dpto
                FROM LAMB_USERS_DEPTO A INNER JOIN CONTA_ENTIDAD_DEPTO B ON A.ID_ENTIDAD = B.ID_ENTIDAD AND A.ID_DEPTO = B.ID_DEPTO
            WHERE A.ID = p_id_persona
            AND A.ID_ENTIDAD = l_entidad
            AND A.ESTADO = 1
    ...
    ELSE --PARA ALUMNOS
    	...
    	FROM ( 
            ...
            AND a.ESTADO = '1'
            ORDER BY a.ID_SEDE ASC) x
            WHERE ROWNUM = 1;
    	
        IF l_contar > 0 THEN
        	
        	SELECT 
			(CASE x.ID_SEDE WHEN 1 THEN '1' WHEN 2 THEN '5' WHEN 3 THEN '6' WHEN 4 THEN '8' ELSE NULL END)  INTO l_dpto
			FROM ( 
				SELECT 
				 a.id_alumno_plan, a.id_persona, a.id_plan, a.id_sede
	            FROM DAVID.VW_ALUMNO_PLAN_PROGRAMA a
