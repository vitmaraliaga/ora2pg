	...
	SELECT nvl (
		...
		IF S_ID_PERSONANATURAL = 0 THEN
		      IF P_ID_PERSONA > 0  THEN 
		        ...
		        VALUES (P_ID_PERSONA, P_ID_ESTADOCIVIL, NVL(P_ID_TIPOTRATAMIENTO, 1), NULLIF(P_ID_PAIS, 0), NULLIF(P_ID_NACIONALIDAD, 0), NULLIF(P_ID_TIPOSANGRE, 0), P_SEXO, to_date(P_FEC_NACIMIENTO,'yyyy-mm-dd'), to_date(P_FEC_DEFUNCION,'yyyy-mm-dd"T"HH24:MI:SS'));
		      END IF;
	      ELSE
		      UPDATE Persona_Natural SET
		        ...
		        ID_TIPOPAIS            = NVL(NULLIF(P_ID_PAIS, 0), ID_TIPOPAIS),
		        ID_NACIONALIDAD    = NVL(NULLIF(P_ID_NACIONALIDAD, 0), ID_NACIONALIDAD),
