...
CREATE OR REPLACE PROCEDURE enoc.pkg_planilla_proc_sp_r_afp_prima_seguro (P_ID_PLANILLA_TRAB text,P_ID_CONFIG_PLANILLA_CONCEPTO bigint, P_IMPORTE OUT bigint,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
--           goto salida;
      ...
      select round((seguro/100.00)::numeric,3) into STRICT  l_porc from enoc.plla_comision_pensionaria
      where id_regimen_pensionaria=l_regimen_pensionaria
      and id_anho=l_id_anho
      and id_mes=l_id_mes;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE enoc.pkg_planilla_proc_sp_r_afp_prima_seguro (P_ID_PLANILLA_TRAB text,P_ID_CONFIG_PLANILLA_CONCEPTO bigint, P_IMPORTE OUT bigint,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
