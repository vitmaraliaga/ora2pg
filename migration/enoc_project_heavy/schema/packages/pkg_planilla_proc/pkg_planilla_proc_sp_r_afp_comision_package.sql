...
CREATE OR REPLACE PROCEDURE enoc.pkg_planilla_proc_sp_r_afp_comision (P_ID_PLANILLA_TRAB text,P_ID_CONFIG_PLANILLA_CONCEPTO bigint, P_IMPORTE OUT bigint,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
--         goto salida;
    ...
    if l_tipo='PR' then
      select count(*) into STRICT  l_contar from enoc.PLLA_REG_PENS_COMISION 
...
--           goto salida;
      ...
      select round((COMISION_FLUJO/100.00)::numeric,3) into STRICT  l_porc from enoc.PLLA_REG_PENS_COMISION 
      where id_tipo_comision_pens= l_id_tipo_comision_pens
      and id_comision_pensionaria in (
        SELECT id_comision_pensionaria from enoc.plla_comision_pensionaria
        where id_regimen_pensionaria=l_regimen_pensionaria
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE enoc.pkg_planilla_proc_sp_r_afp_comision (P_ID_PLANILLA_TRAB text,P_ID_CONFIG_PLANILLA_CONCEPTO bigint, P_IMPORTE OUT bigint,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
