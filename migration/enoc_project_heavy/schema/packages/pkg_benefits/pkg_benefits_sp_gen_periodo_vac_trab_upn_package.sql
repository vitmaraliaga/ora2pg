...
CREATE OR REPLACE PROCEDURE enoc.pkg_benefits_sp_gen_periodo_vac_trab_upn (P_ID_TRABAJADOR bigint, P_ID_PERIODO_VAC bigint,P_ITEM bigint,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
        ...
        l_meses_cal  decimal(10,2):=0;
       	...
       	l_es_vacacion_interna bigint:=0; --Esto es para los misiones y empleados
BEGIN
          select fecha_inicio, fecha_fin_previsto,id_tipo_tiempo_trabajo,id_condicion_laboral
         	into STRICT l_fecha_inicio,l_fecha_fin_previsto,l_id_tipo_tiempo_trabajo,l_id_condicion_laboral 
          ...
          if l_contar=0 and l_contar1=0 then   
            if coalesce(l_fecha_fin_previsto::text, '') = '' then
              ...
              select (l_inicio + interval '1' year)-1 into STRICT l_fin; -- to_date(to_char(add_months(l_inicio,12)-1 ,'DD-MM')||'-'||to_char(l_anho_inicio+1),'dd/mm/yyyy');
...
--                  GOTO salida_vactrab; 
              ...
              */
              select to_date(to_char(clock_timestamp(),'DD-MM-YYYY'),'dd/mm/yyyy') - l_fecha_inicio into STRICT l_dias;
              if l_dias<360 THEN
...
--                  GOTO salida_vactrab;
              ...
              if l_dias>=360 then
                l_inicio := to_date(to_char(l_fecha_inicio,'DD-MM')||'-'||l_anho_inicio::text,'dd/mm/yyyy');
                select l_inicio - l_fecha_inicio into STRICT l_dias;
                if l_dias<360 then
                   l_error:=1;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE enoc.pkg_benefits_sp_gen_periodo_vac_trab_upn (P_ID_TRABAJADOR bigint, P_ID_PERIODO_VAC bigint,P_ITEM bigint,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
