...
CREATE OR REPLACE PROCEDURE enoc.spc_user_session_login ( p_id_persona integer, p_token text, p_token_oauth text, p_error OUT bigint, p_msjerror OUT text, p_caduca text default 'S', p_device_source text default 'WEB') AS $body$
DECLARE
...
BEGIN
   ...
   -- Validar que la persona tenga una entidad asignada por defecto en el LAMB.
    SELECT COUNT(1), max(X.id_entidad) into STRICT l_contar, l_entidad FROM ELISEO.CONTA_ENTIDAD_USUARIO X WHERE X.ID_PERSONA = p_id_persona AND X.ESTADO = 1;
...
--         goto salida_rapida;
   ...
   --Validar si el usuario es trabajador activo de la entidad consultada al inicio. --last update 30/04/2021
   	SELECT COUNT(1) into STRICT l_contar FROM MOISES.TRABAJADOR T WHERE T.ID_PERSONA = p_id_persona AND T.ID_ENTIDAD = l_entidad AND T.ID_SITUACION_TRABAJADOR !='0';
  	IF l_contar = 0 THEN
  	...
  	IF p_device_source='APP_UPN' OR p_device_source='PORTAL_UPN' THEN 	
  		SELECT  COUNT(1),max(ID_ENTIDAD) into STRICT l_contar, l_entidad FROM (SELECT T.ID_TRABAJADOR,T.ID_ENTIDAD,SA.ID_DEPTO_PADRE AS ID_DEPTO FROM MOISES.TRABAJADOR T
		...
		ORDER BY T.FECHA_INICIO DESC) alias2 LIMIT 1;
	ELSE
		SELECT  COUNT(1),max(ID_ENTIDAD) into STRICT l_contar, l_entidad FROM (SELECT * FROM MOISES.TRABAJADOR T 
		WHERE T.ID_PERSONA = p_id_persona AND T.ID_SITUACION_TRABAJADOR !='0'
		ORDER BY T.FECHA_INICIO DESC ) alias2 LIMIT 1;
	END IF;
		IF l_contar = 0 THEN
...
-- 	        goto salida_rapida;
   ...
   IF p_device_source='APP_UPN' OR p_device_source='PORTAL_UPN' THEN
   		SELECT  COUNT(1),min(ID_DEPTO) into STRICT l_contar,l_dpto FROM (SELECT T.ID_TRABAJADOR,T.ID_ENTIDAD,SA.ID_DEPTO_PADRE AS ID_DEPTO FROM MOISES.TRABAJADOR T 
		INNER JOIN ELISEO.VW_SEDE_AREA SA ON SA.ID_SEDEAREA = T.ID_SEDEAREA
		WHERE T.ID_PERSONA = p_id_persona AND T.ID_SITUACION_TRABAJADOR !='0'
		ORDER BY T.FECHA_INICIO DESC) alias2 LIMIT 1;
   ELSE
...
--         goto salida_rapida;*/
    ...
    SELECT count(1) into STRICT l_contar FROM ELISEO.LAMB_USUARIO_ROL WHERE ID_PERSONA = p_id_persona AND ID_ENTIDAD = l_entidad;
...
--         goto salida_rapida;
    ...
    if l_error=0 then
        ...
        SELECT COUNT(1) INTO STRICT l_cant FROM ELISEO.LAMB_MODULO M 
        ...
        ELSE
            insert into ELISEO.USERS_SESSION(TOKEN,ID_USER,ID_ENTIDAD,ID_DEPTO,DATE_CREATE,LAST_UPDATE,STATUS,TOKEN_OAUTH,CADUCA)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE enoc.spc_user_session_login ( p_id_persona integer, p_token text, p_token_oauth text, p_error OUT bigint, p_msjerror OUT text, p_caduca text default 'S', p_device_source text default 'WEB') FROM PUBLIC;
