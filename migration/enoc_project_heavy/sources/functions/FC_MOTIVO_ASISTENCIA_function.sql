...
function      FC_MOTIVO_ASISTENCIA(P_ID_TRABAJADOR NUMBER,P_ID_TIPO_HORARIO NUMBER,P_ID_SEDEAREA NUMBER, P_FECHA date) RETURN VARCHAR2
IS
l_dato VARCHAR(10):='A';
l_contar number:=0;
...
BEGIN
   ...
   IF l_dato='A' then
    SELECT count(*) into l_contar FROM ENOC.PLLA_TIPO_HORARIO A,ENOC.PLLA_TIPO_HORARIO_TRAB B ,ENOC.PLLA_TIPO_CONFIG_HORARIO c 
    WHERE A.ID_TIPO_HORARIO=B.ID_TIPO_HORARIO 
