...
FUNCTION       FT_CODIGO_ADM( V_ID_PERSONA_INSCRIPCION IN NUMBER ) RETURN VARCHAR2 AS 

p_codigo varchar2(20);
p_cantidad number;
BEGIN
   select codigo, cantidad into p_codigo , p_cantidad 
   from (
        select UPPER(SUBSTR(nvl(p.nombre,'Z') ,1,1)) || UPPER(SUBSTR(nvl(p.paterno,'Z') ,1,1)) || to_char(sysdate,'DDMMYY') || '0' || SUBSTR(nvl(ac.nombre,'0'),-1,1) as codigo,
