...
CREATE OR REPLACE FUNCTION caleb.ft_matricula (p_id_persona_inscripcion text) RETURNS bigint AS $body$
DECLARE

  cant_persona_ins bigint;
...
BEGIN
	...
	FROM (
		...
		WHERE 
		pd.ES_ACTIVO = '1' 
		AND  (pd.NUM_DOCUMENTO IS NOT NULL AND pd.NUM_DOCUMENTO::text <> '')
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION caleb.ft_matricula (p_id_persona_inscripcion text) FROM PUBLIC;
