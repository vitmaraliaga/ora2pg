...
CREATE OR REPLACE VIEW vw_colp_ventas_credito_dep (id_venta, id_almacen, id_entidad, id_depto, id_campanha, id_anho, id_voucher, id_cliente, id_comprobante, serie, numero, nro_comprobante, glosa, fecha, total_venta, diezmo, total_libros, amortizado, anulado, pagado) AS SELECT
	...
	lista.AMORTIZADO,
	lista.ANULADO,
	(CASE WHEN lista.ANULADO != 'OK' AND round((lista.TOTAL_VENTA)::numeric,2) = ROUND(lista.AMORTIZADO) THEN '1' ELSE '0' END) AS PAGADO
FROM
	(
		SELECT
			V.ID_VENTA,
			...
			(
				SELECT coalesce(SUM(CDD.IMPORTE), 0)
			...
			coalesce((SELECT CASE WHEN X.ESTADO='1' THEN 'S' WHEN X.ESTADO='2' THEN 'OK'  ELSE 'N' END
				FROM ARREGLO X 
