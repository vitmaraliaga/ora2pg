...
CREATE OR REPLACE FUNCTION eliseo.fc_saldo_alumno_antes_fecha (P_ID_PERSONA integer, P_FECHA text,P_ID_DEPTO text) RETURNS bigint AS $body$
DECLARE
...
BEGIN
        ...
        select  sum(z.importe) into STRICT SALDO
        ...
        where c.id_entidad = 7124
          ...
          and c.importe <> 0
          and c.estado = 1
          and to_char(coalesce(c.fecha_operacion, c.fecha), 'yyyymmddhh24mi') <= P_FECHA   --MODIFICAR FECHA
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.fc_saldo_alumno_antes_fecha (P_ID_PERSONA integer, P_FECHA text,P_ID_DEPTO text) FROM PUBLIC;
