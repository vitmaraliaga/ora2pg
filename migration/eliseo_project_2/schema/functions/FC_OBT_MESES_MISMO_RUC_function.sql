...
CREATE OR REPLACE FUNCTION eliseo.fc_obt_meses_mismo_ruc (P_ID_ENTIDAD bigint, P_ID_PERSONA bigint, FECHA_INI timestamp(0), FECHA_FIN timestamp(0)) RETURNS bigint AS $body$
DECLARE
...
BEGIN
		...
		IF l_contar > 0 THEN
			IF l_contar = 1 THEN
				SELECT MONTHS_BETWEEN(RECORDS.SALIDA + 1, RECORDS.INGRESO) INTO STRICT total_meses
				FROM (
			...
			END IF;
		END IF;
	RETURN date_trunc('day', mod(total_meses, 12));
END;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.fc_obt_meses_mismo_ruc (P_ID_ENTIDAD bigint, P_ID_PERSONA bigint, FECHA_INI timestamp(0), FECHA_FIN timestamp(0)) FROM PUBLIC;
