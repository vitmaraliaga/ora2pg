...
CREATE OR REPLACE FUNCTION eliseo.fc_nueva_glosa ( p_id_asiento bigint ) RETURNS varchar AS $body$
DECLARE
...
BEGIN
    ...
    RAISE NOTICE 'ESTADO:%    TIPO:%    SIGN:%', l_estado, l_id_tipo_origen, l_asign;
    ...
    ELSIF l_id_tipo_origen = '12' THEN  --RETENCION 
        IF l_asign = '1' THEN --D
            SELECT
                CASE WHEN cuenta                       || '/'                       || cuenta_cte='2130390/1' THEN  '1'  ELSE '0' END
            INTO STRICT l_estado
            FROM
                conta_asiento
            WHERE
                id_asiento = p_id_asiento;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.fc_nueva_glosa ( p_id_asiento bigint ) FROM PUBLIC;
