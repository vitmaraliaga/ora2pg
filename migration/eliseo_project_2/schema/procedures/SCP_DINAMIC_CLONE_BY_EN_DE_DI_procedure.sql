...
CREATE OR REPLACE PROCEDURE eliseo.scp_dinamic_clone_by_en_de_di ( v_ID_ENTIDAD_FROM bigint, v_ID_DEPTO_FROM text, v_ID_YEAR_FROM bigint, v_ID_DINAMICA_FROM bigint, v_ID_ENTIDAD_TO bigint, v_ID_DEPTO_TO text, v_ID_YEAR_TO bigint, v_ERROR OUT bigint, v_MSGERROR OUT text ) AS $body$
DECLARE
   ...
   l_id_almacen bigint;
    ...
    AND ID_DINAMICA = v_ID_DINAMICA_FROM
...
BEGIN
    ...
    IF L_ERROR = 0 THEN
        ...
        LOOP
                ...
                SELECT max(ID_CTABANCARIA) into STRICT l_id_ctabancaria FROM CAJA_CUENTA_BANCARIA
                WHERE ID_ENTIDAD = v_ID_ENTIDAD_TO
                AND ID_DEPTO = v_ID_DEPTO_TO;

                SELECT max(ID_ALMACEN) into STRICT l_id_almacen FROM INVENTARIO_ALMACEN
                WHERE ID_ENTIDAD = v_ID_ENTIDAD_TO
                AND ID_DEPTO = v_ID_DEPTO_TO;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.scp_dinamic_clone_by_en_de_di ( v_ID_ENTIDAD_FROM bigint, v_ID_DEPTO_FROM text, v_ID_YEAR_FROM bigint, v_ID_DINAMICA_FROM bigint, v_ID_ENTIDAD_TO bigint, v_ID_DEPTO_TO text, v_ID_YEAR_TO bigint, v_ERROR OUT bigint, v_MSGERROR OUT text ) FROM PUBLIC;
