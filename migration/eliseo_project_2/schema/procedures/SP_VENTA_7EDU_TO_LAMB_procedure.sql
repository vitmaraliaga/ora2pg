...
CREATE OR REPLACE PROCEDURE eliseo.sp_venta_7edu_to_lamb () AS $body$
DECLARE

    L_CANT bigint;
   	L_ID_CLIENTE_NEW bigint:= null;
   	L_ID_CLIENTE_LEGAL_NEW bigint:= null;
   	L_NO_EXISTE_PERSONA bigint;
   	L_ID_VOUCHER bigint;
   	L_ID_VENTA_NEW bigint;
   	L_ACTIVO varchar(1):='S';
    ...
    VENTAS CURSOR FOR	
    SELECT * FROM ELISEO.VENTA_TO_LAMB WHERE coalesce(ID_VENTA_LAMB::text, '') = '';
...
BEGIN
		...
		LOOP
			...
			L_CANT := 0;
			RAISE NOTICE 'List % row', venta_item.id_venta;
			IF L_CANT = 0 THEN
				L_NO_EXISTE_PERSONA := 0;
				SELECT COUNT(1) INTO STRICT L_CANT FROM moises.PERSONA_DOCUMENTO pd
				WHERE num_documento = venta_item.num_documento_cliente;
				IF L_CANT > 0 THEN
					SELECT ID_PERSONA INTO STRICT L_ID_CLIENTE_NEW FROM moises.PERSONA_DOCUMENTO pd 
					WHERE num_documento = venta_item.num_documento_cliente  LIMIT 1;
				ELSE
					SELECT COUNT(1) INTO STRICT L_CANT FROM moises.PERSONA_NATURAL pd 
					WHERE num_documento = venta_item.num_documento_cliente;
					IF L_CANT > 0 THEN
						SELECT ID_PERSONA INTO STRICT L_ID_CLIENTE_NEW FROM moises.PERSONA_NATURAL pd 
						WHERE num_documento = venta_item.num_documento_cliente  LIMIT 1;
						UPDATE ELISEO.VENTA_TO_LAMB SET NO_EXISTE_CLIENTE = 0 WHERE ID_VENTA = venta_item.ID_VENTA;
					ELSE
						SELECT COUNT(1) INTO STRICT L_CANT FROM moises.PERSONA_JURIDICA pd 
						WHERE id_ruc = venta_item.num_documento_cliente;
						IF L_CANT > 0 THEN
							SELECT ID_PERSONA INTO STRICT L_ID_CLIENTE_NEW FROM moises.PERSONA_JURIDICA pd 
							WHERE id_ruc = venta_item.num_documento_cliente  LIMIT 1;
							UPDATE ELISEO.VENTA_TO_LAMB SET NO_EXISTE_CLIENTE = 0 WHERE ID_VENTA = venta_item.ID_VENTA;
						ELSE
							--L_NO_EXISTE_PERSONA := 1;

							--UPDATE ELISEO.VENTA_TO_LAMB SET NO_EXISTE_CLIENTE = 1 WHERE ID_VENTA = venta_item.ID_VENTA;
							SELECT COUNT(1) INTO STRICT L_CANT FROM eliseo.venta_persona_to_lamb pd 
							WHERE id_persona = venta_item.id_cliente;
							IF L_CANT > 0 THEN
								SELECT nombre,paterno,materno,num_documento,id_ruc INTO STRICT L_NOMBRE,L_PATERNO,L_MATERNO,L_NUM_DOCUMENTO,L_ID_RUC 
								FROM eliseo.venta_persona_to_lamb pd 
								WHERE id_persona = venta_item.id_cliente  LIMIT 1;
								IF (L_NUM_DOCUMENTO IS NOT NULL AND L_NUM_DOCUMENTO::text <> '') THEN
								SELECT MAX(id_persona) +1 INTO STRICT L_ID_CLIENTE_NEW FROM moises.persona;
									INSERT INTO moises.persona(id_persona,nombre,paterno,materno) 
									VALUES (L_ID_CLIENTE_NEW,L_NOMBRE,L_PATERNO,L_MATERNO);
									INSERT INTO moises.persona_natural(id_persona,id_tipopais,id_tiporeligion,id_tipodocumento,num_documento,sexo,celular,fec_nacimiento,correo)
									...
									FROM eliseo.venta_persona_to_lamb WHERE id_persona = venta_item.id_cliente;
								ELSIF (L_ID_RUC IS NOT NULL AND L_ID_RUC::text <> '') THEN
									SELECT MAX(id_persona) +1 INTO STRICT L_ID_CLIENTE_NEW FROM moises.persona;
									INSERT INTO moises.persona(id_persona,nombre,paterno,materno)
									VALUES (L_ID_CLIENTE_NEW,L_NOMBRE,L_PATERNO,L_MATERNO);
									INSERT INTO moises.PERSONA_JURIDICA(id_ruc,id_persona,id_tipoestado,id_tipocondicion,id_tipocontribuyente,id_tipoactividadeconomica,id_tipopais)
				...
				IF L_CANT > 0 THEN
					SELECT ID_PERSONA INTO STRICT L_ID_CLIENTE_LEGAL_NEW FROM moises.PERSONA_DOCUMENTO pd 
					WHERE num_documento = venta_item.num_documento_cliente_legal  LIMIT 1;
				ELSE
					IF (venta_item.id_cliente_legal IS NOT NULL AND venta_item.id_cliente_legal::text <> '') THEN 
						SELECT COUNT(1) INTO STRICT L_CANT FROM moises.PERSONA_NATURAL pd 
						WHERE num_documento = venta_item.num_documento_cliente_legal;
						IF L_CANT > 0 THEN
							SELECT ID_PERSONA INTO STRICT L_ID_CLIENTE_LEGAL_NEW FROM moises.PERSONA_NATURAL pd 
							WHERE num_documento = venta_item.num_documento_cliente_legal  LIMIT 1;
						...
						ELSE
							...
							IF L_CANT > 0 THEN
								SELECT ID_PERSONA INTO STRICT L_ID_CLIENTE_LEGAL_NEW FROM moises.PERSONA_JURIDICA pd 
								WHERE id_ruc = venta_item.num_documento_cliente_legal  LIMIT 1;
								UPDATE ELISEO.VENTA_TO_LAMB SET NO_EXISTE_CLIENTE_LEGAL = 0 WHERE ID_VENTA = venta_item.ID_VENTA;
							ELSE
								--L_NO_EXISTE_PERSONA := 1;
								...
								IF L_CANT > 0 THEN
									SELECT nombre,paterno,materno,num_documento,id_ruc INTO STRICT L_NOMBRE,L_PATERNO,L_MATERNO,L_NUM_DOCUMENTO,L_ID_RUC 
									FROM eliseo.venta_persona_to_lamb pd 
									WHERE id_persona = venta_item.id_cliente_legal  LIMIT 1;
									IF (L_NUM_DOCUMENTO IS NOT NULL AND L_NUM_DOCUMENTO::text <> '') THEN
									SELECT MAX(id_persona) +1 INTO STRICT L_ID_CLIENTE_LEGAL_NEW FROM moises.persona;
										INSERT INTO moises.persona(id_persona,nombre,paterno,materno) 
				...
				IF L_NO_EXISTE_PERSONA = 0 THEN
					BEGIN ELISEO.CALL PKG_ACCOUNTING.SP_CREAR_VOUCHER_CARGO_CUOTA(
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.sp_venta_7edu_to_lamb () FROM PUBLIC;
