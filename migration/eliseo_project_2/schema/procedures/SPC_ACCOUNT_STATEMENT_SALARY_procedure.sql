...
CREATE OR REPLACE PROCEDURE eliseo.spc_account_statement_salary ( p_id_entidad integer, p_id_anho integer, p_id_mes integer, p_id_persona integer, p_id_cta_cte text, OUT CURSOR REFCURSOR ) AS $body$
DECLARE
...
;
    ...
    -- l_descuentos_total := l_descuentos + l_descuento_extraordinario + l_descuentos_ayudas + l_descuentos_viajes +l_descuentos_adelantos;
    l_descuentos_total := l_descuentos + l_descuento_extraordinario  +l_descuentos_adelantos;
    ...
    RAISE NOTICE '%', l_sueldo_depositado;
    INSERT INTO TT_TABLE_AUX(NUM_1, NUM_2, NUM_3)
    VALUES (l_ingresos, l_descuentos_total, l_sueldo_depositado);

    OPEN CURSOR FOR 
    SELECT 
      NUM_1 as l_ingresos,
      NUM_2 as l_descuentos_total,
      NUM_3 as l_sueldo_depositado
    from TT_TABLE_AUX 
;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.spc_account_statement_salary ( p_id_entidad integer, p_id_anho integer, p_id_mes integer, p_id_persona integer, p_id_cta_cte text, OUT CURSOR REFCURSOR ) FROM PUBLIC;
