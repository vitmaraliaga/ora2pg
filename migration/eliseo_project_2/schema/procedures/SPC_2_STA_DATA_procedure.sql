...
CREATE OR REPLACE PROCEDURE eliseo.spc_2_sta_data ( p_id_entidad integer, p_id_anho integer, p_id_mes integer, p_id_persona integer, p_id_cta_cte text, OUT CURSOR REFCURSOR ) AS $body$
DECLARE
...
-- AYUDAS SALDO FINAL
select 
	coalesce(sum(cos_valor),0) INTO STRICT l_saldo_ayudas
from VW_CONTA_DIARIO
where id_entidad = p_id_entidad
and ID_CUENTAAASI = 1135005
and ID_CTACTE = p_id_cta_cte
...
----------------------------------------------------------------
-- VIAJES INFORMADOS
SELECT abs(coalesce(sum(valor),0)) into STRICT l_ingresos_viajes FROM (
	/*
    SELECT A.COS_VALOR FROM VW_APS_PLANILLA A 
        INNER JOIN APS_CONCEPTO_PLANILLA B ON A.ID_CONCEPTOAPS = B.ID_CONCEPTOAPS 
		WHERE A.ID_ENTIDAD = p_id_entidad
...
-- VIAJES SALDO FINAL
select
	coalesce(sum(cos_valor),0) INTO STRICT l_saldo_viajes
from VW_CONTA_DIARIO
where id_entidad = p_id_entidad
and ID_CUENTAAASI = 1135010
and ID_CTACTE = p_id_cta_cte
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.spc_2_sta_data ( p_id_entidad integer, p_id_anho integer, p_id_mes integer, p_id_persona integer, p_id_cta_cte text, OUT CURSOR REFCURSOR ) FROM PUBLIC;
