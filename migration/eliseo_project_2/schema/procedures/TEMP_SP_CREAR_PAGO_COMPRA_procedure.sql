...
CREATE OR REPLACE PROCEDURE eliseo.temp_sp_crear_pago_compra (P_ID_PAGO bigint,P_ID_DINAMICA bigint,P_ID_PROVEEDOR bigint,P_ID_COMPRA bigint, P_IMPORTE bigint,P_IMPORTE_ME bigint, P_ID_PAGO_COMPRA INOUT bigint, P_ERROR out bigint, P_MSGERROR out text) AS $body$
DECLARE
...
BEGIN
            ...
            if l_error=0 then
                ...
                SELECT COUNT(1) INTO STRICT l_contar FROM COMPRA WHERE ID_COMPRA = P_ID_COMPRA;
                IF l_contar = 0 THEN
...
;
                ...
                CASE WHEN ID_MEDIOPAGO='001' THEN 'TLC' WHEN ID_MEDIOPAGO='007' THEN 'CHQ' WHEN ID_MEDIOPAGO='008' THEN 'EFEC' END ||'-'||NUMERO||' | '||TO_CHAR(FECHA,'DD/MM/YYYY')||' | '||PKG_CAJA.FC_CUENTA_BANCARIA(ID_CTABANCARIA)
                ...
                ELSE 
                    ...
                    IF coalesce(P_IMPORTE,0)+coalesce(P_IMPORTE_ME,0) > 0 THEN
                       ...
                       IF l_error = 0 THEN
                            SELECT COALESCE(MAX(ID_PCOMPRA),0)+1 into STRICT l_id_pcompra FROM CAJA_PAGO_COMPRA;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.temp_sp_crear_pago_compra (P_ID_PAGO bigint,P_ID_DINAMICA bigint,P_ID_PROVEEDOR bigint,P_ID_COMPRA bigint, P_IMPORTE bigint,P_IMPORTE_ME bigint, P_ID_PAGO_COMPRA INOUT bigint, P_ERROR out bigint, P_MSGERROR out text) FROM PUBLIC;
