...
CREATE OR REPLACE PROCEDURE eliseo.pkg_purchases_sp_compra_asiento_inven_upn (P_ID_COMPRA bigint,P_ID_DINAMICA bigint,P_ERROR OUT bigint,P_MSGERROR OUT text) AS $body$
DECLARE
        ...
        L_ID_MONEDA bigint;
        L_TIPOCAMBIO bigint;
        L_ID_COMPROBANTE_NC varchar(100);
       ...
       */
        ...
        cvdet CURSOR FOR
        SELECT BASE, IMPORTE, IGV, COSTO_VINCULADO, ID_ARTICULO, pkg_inventories_fc_articulo(ID_ARTICULO) AS NAME_ARTICULO, ID_ALMACEN 
        FROM COMPRA_DETALLE
...
BEGIN
        ...
        INTO STRICT L_ID_ENTIDAD,L_ID_DEPTO,L_SERIE,L_NUMERO,L_BASE, L_IGV, L_IMPORTE, L_IMPORTE_ME, L_ESTADO, L_ID_COMPROBANTE, L_ID_ANHO, L_ID_PROVEEDOR,
               ...
               FOR a_cvdet in cvdet
                ...
                BEGIN
                    ...
                    ELSIF (L_UNICOCTACTE='X') THEN
                        SELECT pkg_purchases_fc_ruc(L_ID_PROVEEDOR) into STRICT L_ID_CTACTE;
                    ELSIF L_UNICOCTACTE='A' THEN  -- Del artículo.
                        SELECT  COUNT(1) INTO STRICT L_CONT FROM INVENTARIO_ALMACEN_ARTICULO WHERE ID_ALMACEN=a_cvdet.ID_ALMACEN AND ID_ARTICULO=a_cvdet.ID_ARTICULO AND ID_ANHO=L_ID_ANHO;
                        IF L_CONT>0 THEN
                            SELECT ID_CTACTE INTO STRICT L_ID_CTACTE FROM INVENTARIO_ALMACEN_ARTICULO WHERE ID_ALMACEN=a_cvdet.ID_ALMACEN AND ID_ARTICULO=a_cvdet.ID_ARTICULO AND ID_ANHO=L_ID_ANHO;
                        END IF;
                    END IF;

                    
                    SELECT CASE L_ID_INDICADOR WHEN 'IMPORTE' THEN (a_cvdet.IMPORTE)--*(L_PORCENTAJE/100)
                          WHEN 'BASE' THEN (a_cvdet.BASE)--*(L_PORCENTAJE/100)
                          WHEN 'IGV' THEN (a_cvdet.IGV)--*(L_PORCENTAJE/100)
                          WHEN 'COSTO_ALM' THEN (a_cvdet.IMPORTE)--*(L_PORCENTAJE/100)
                          ELSE
                          0
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_purchases_sp_compra_asiento_inven_upn (P_ID_COMPRA bigint,P_ID_DINAMICA bigint,P_ERROR OUT bigint,P_MSGERROR OUT text) FROM PUBLIC;
