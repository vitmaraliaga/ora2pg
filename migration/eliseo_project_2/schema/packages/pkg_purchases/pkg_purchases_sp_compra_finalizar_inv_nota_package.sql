...
CREATE OR REPLACE PROCEDURE eliseo.pkg_purchases_sp_compra_finalizar_inv_nota ( P_ID_COMPRA bigint, P_ERROR OUT bigint, P_MSGERROR OUT text ) AS $body$
DECLARE
...
BEGIN
      	...
      	INTO STRICT L_COMPRA_SERIE, L_COMPRA_NUMERO, L_COMPRA_ID_VOUCHER, L_ESTADO,L_ID_ANHO, L_ID_COMPROBANTE
           	...
           	IF L_ID_COMPROBANTE = '07' OR L_ID_COMPROBANTE = '87' THEN  -- Nota de crédito
	            FOR detalle in CURSOR_CDETALLE
	                ...
	                BEGIN
	                  	...
	                  	CALL pkg_inventories_sp_articulo_stock(detalle.ID_ALMACEN,detalle.ID_ARTICULO,L_ID_ANHO,L_STOCK,L_COSTO_ST,L_MSN);
	        			CALL pkg_inventories_sp_articulo_stock_temp(detalle.ID_ALMACEN,detalle.ID_ARTICULO,L_ID_ANHO,L_STOCK_T,L_MSN);
	        		
	        			SELECT COALESCE(MAX(NOMBRE),'') INTO STRICT L_ARTICULO_NOMBRE FROM INVENTARIO_ARTICULO WHERE ID_ARTICULO = detalle.ID_ARTICULO;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_purchases_sp_compra_finalizar_inv_nota ( P_ID_COMPRA bigint, P_ERROR OUT bigint, P_MSGERROR OUT text ) FROM PUBLIC;
