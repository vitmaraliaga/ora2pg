...
CREATE OR REPLACE PROCEDURE eliseo.pkg_finances_students_sp_desc_mat_ingresante (P_ID_ALUMNO_CONTRATO bigint,P_ID_CRITERIO_SEMESTRE bigint,P_TIPO_ALUMNO text,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
BEGIN
    ...
    INTO STRICT l_id_plan_programa,l_id_semestre_programa
    FROM DAVID.ACAD_ALUMNO_CONTRATO
    WHERE ID_ALUMNO_CONTRATO = P_ID_ALUMNO_CONTRATO;

    select count(*) into STRICT l_contar from david.acad_plan_programa a,david.acad_plan b,david.acad_semestre c
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_finances_students_sp_desc_mat_ingresante (P_ID_ALUMNO_CONTRATO bigint,P_ID_CRITERIO_SEMESTRE bigint,P_TIPO_ALUMNO text,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
