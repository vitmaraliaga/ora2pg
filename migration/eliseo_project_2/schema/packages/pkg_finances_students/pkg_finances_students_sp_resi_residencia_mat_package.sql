...
CREATE OR REPLACE PROCEDURE eliseo.pkg_finances_students_sp_resi_residencia_mat (P_ID_ALUMNO_CONTRATO bigint,P_ID_CRITERIO_SEMESTRE bigint,P_TIPO_ALUMNO text,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
--       goto resi_resi_mat;
    ...
    if coalesce(l_id_criterio::text, '') = '' then
      l_error:=1;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_finances_students_sp_resi_residencia_mat (P_ID_ALUMNO_CONTRATO bigint,P_ID_CRITERIO_SEMESTRE bigint,P_TIPO_ALUMNO text,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
