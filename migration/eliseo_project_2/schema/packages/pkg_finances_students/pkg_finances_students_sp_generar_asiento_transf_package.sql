...
CREATE OR REPLACE PROCEDURE eliseo.pkg_finances_students_sp_generar_asiento_transf (P_ID_ALUMNO_CONTRATO bigint,P_ID_ENTIDAD bigint,P_ID_TRANSFERENCIA bigint,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE

     l_error bigint:=0;
      l_msgerror varchar(200):='';
      l_contar bigint;
      l_id_tipoorigen bigint:=2; --venta
...
BEGIN
      SELECT SERIE, NUMERO, FECHA,ID_VOUCHER, GLOSA, FC_NOMBRE_PERSONA(ID_CLIENTE) INTO STRICT l_serie,l_numero,l_fecha, l_voucher, l_glosa, l_nombre 
      ...
      INSERT INTO CONTA_ASIENTO(
        ...
        (CASE b.TIPO_DC WHEN 'D' THEN P_ID_ENTIDAD||'-'||'(Doc: '||l_serie||'-'||(l_numero)::numeric ||')-'||l_glosa||'-'||l_nombre
        ELSE
            P_ID_ENTIDAD||' - (Doc: '||l_serie||'-'||l_numero||')-'||CS.NOMBRE||' '||to_char(l_fecha,'DD/MM/YYYY')
        END),
        (l_id_tipoorigen::text || '-' || VD.ID_TDETALLE::text),
        l_voucher,
        NULL,
        CASE WHEN b.TIPO_DC='C' THEN 'N'  ELSE 'S' END 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_finances_students_sp_generar_asiento_transf (P_ID_ALUMNO_CONTRATO bigint,P_ID_ENTIDAD bigint,P_ID_TRANSFERENCIA bigint,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
