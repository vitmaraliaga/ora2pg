...
CREATE OR REPLACE PROCEDURE eliseo.pkg_finances_students_sp_execute_cuota (P_ID_ENTIDAD bigint,P_ID_DEPTO text,P_ID_SEMESTRE bigint,P_ID_PERSONA bigint,P_ID_MODO_CONTRATO bigint, P_ID_NIVEL_ENSENANZA bigint, P_ID_MODALIDAD_ESTUDIO bigint,P_ID_PROGRAMAS text,P_ID_SEDE bigint,P_ALUMNOS text,PLAN_PAGO bigint,P_CUOTA bigint,P_ID_PLANPAGO bigint,P_ERROR OUT bigint,P_MSGERROR OUT text) AS $body$
DECLARE
        ...
        SELECT A.ID_ALUMNO_CONTRATO,A.ID_PERSONA,A.TIPO_ALUMNO,coalesce(A.CREDITOSVAR,0)
...
BEGIN
                        ...
                        WHILE C_ALUMNOS%FOUND LOOP
                            ...
                            --IF L_CONTAR > 0 THEN
                                ...
                                IF L_PROCESA = 'S' THEN
                                    ...
                                    CALL pkg_finances_students_sp_cobrar_cuota(L_ID_ALUMNO_CONTRATO,P_ID_ENTIDAD,P_ID_DEPTO,L_ID_ANHO,L_ID_MES,P_ID_PERSONA,L_ES_VIRTUAL,L_ID_TIPOVENTA,L_CONINTERNADO,P_CUOTA,L_ID_VENTA,L_ERROR,L_MSGERROR);
                                    INSERT INTO TEST_CUOTA(ID_PERSONA,ID_VENTA,FECHA,VARIACION,ID_NIVEL_ENS,CUOTA,ID_SEMESTRE)VALUES (L_ID_CLIENTE,L_ID_VENTA,clock_timestamp(),'S',P_ID_NIVEL_ENSENANZA,5,P_ID_SEMESTRE);
                                    IF L_ERROR > 0 THEN
                                        INSERT INTO TEST_NO_CUOTA(ID_PERSONA,ID_ALUMNO_CONTRATO,ERROR,VARIACION,ID_NIVEL_ENS,CUOTA,FECHA,ID_SEMESTRE)VALUES (L_ID_CLIENTE,L_ID_ALUMNO_CONTRATO,L_MSGERROR,'S',P_ID_NIVEL_ENSENANZA,5,clock_timestamp(),P_ID_SEMESTRE);
                                        ...
                                        VALUES (P_ID_PERSONA,L_ID_CLIENTE,L_ID_ALUMNO_CONTRATO,P_ID_ENTIDAD,P_ID_DEPTO,P_ID_SEMESTRE,P_ID_NIVEL_ENSENANZA,ES_VARIACION,P_CUOTA,L_CODIGO||': '||L_MSGERROR,clock_timestamp());
                                    END IF;
                                END IF;
            ...
            IF L_CONTAR = 0 THEN
                ...
                WHERE ID_SEMESTRE_PROGRAMA  IN (SELECT ID_SEMESTRE_PROGRAMA FROM DAVID.ACAD_SEMESTRE_PROGRAMA WHERE ID_SEMESTRE = P_ID_SEMESTRE AND ID_PROGRAMA_ESTUDIO IN (SELECT ID_PROGRAMA_ESTUDIO FROM TT_PROGRAMAS WHERE ID_SEDE = P_ID_SEDE) ) )
                AND ORDEN = P_CUOTA-1;
            ELSE
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_finances_students_sp_execute_cuota (P_ID_ENTIDAD bigint,P_ID_DEPTO text,P_ID_SEMESTRE bigint,P_ID_PERSONA bigint,P_ID_MODO_CONTRATO bigint, P_ID_NIVEL_ENSENANZA bigint, P_ID_MODALIDAD_ESTUDIO bigint,P_ID_PROGRAMAS text,P_ID_SEDE bigint,P_ALUMNOS text,PLAN_PAGO bigint,P_CUOTA bigint,P_ID_PLANPAGO bigint,P_ERROR OUT bigint,P_MSGERROR OUT text) FROM PUBLIC;
