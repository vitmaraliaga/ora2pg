...
SET search_path = eliseo,pkg_sales_finances,public;
    ...
    --PROCEDURE SP_GENERAR_ASIENTO_CUOTA(P_ID_ALUMNO_CONTRATO numeric);
    --PROCEDURE SP_GENERAR_TRANSF_CUOTA_IDEC(P_ID_SEMESTRE numeric,P_ID_MODO_CONTRATO numeric, P_ID_NIVEL_ENSENANZA numeric, P_ID_MODALIDAD_ESTUDIO numeric,P_ERROR OUT numeric,P_MSGERROR OUT varchar);
    --PROCEDURE SP_GEN_ASTO_TRANS_CUOTA_IDEC(P_ID_ALUMNO_CONTRATO numeric);
CREATE OR REPLACE PROCEDURE eliseo.pkg_sales_finances_sp_dscto_covid (P_ID_ENTIDAD bigint,P_ID_DEPTO text,P_ID_SEMESTRE bigint,P_ID_MODO_CONTRATO bigint, P_ID_NIVEL_ENSENANZA bigint, P_ID_MODALIDAD_ESTUDIO bigint,P_ERROR OUT bigint,P_MSGERROR OUT text) AS $body$
DECLARE
...
--                         GOTO SALIDA_VENTA;
                    ...
                    OPEN C_ALUMNOS;
                        ...
                        WHILE C_ALUMNOS%FOUND LOOP
                            SELECT COUNT(1) INTO STRICT L_CONTAR FROM MAT_TEST WHERE ID_PERSONA = L_ID_CLIENTE;
                            ...
                            --IF L_CONTAR > 0 THEN
                                ...
                                IF L_IMP_DSCTO > 0 THEN
                                    ...
                                    IF L_CONTAR > 0 THEN  --SI LA MATRICULA FUE UNA VENTA
                                        ...
                                        AND A.GLOSA LIKE '5ta%2020-1'  LIMIT 1;
                                        IF L_CONTAR > 0 THEN  --SI LA ARMADA FUE UNA VENTA
                                            SELECT DISTINCT A.ID_VENTA,A.ID_COMPROBANTE,A.SERIE,A.NUMERO INTO STRICT L_ID_PARENT,L_ID_COMPROBANTE_REF,L_SERIE_REF,L_NUMERO_REF
                                            FROM VENTA A JOIN VENTA_DETALLE B
                                            ON A.ID_VENTA = B.ID_VENTA
                                            WHERE A.ID_CLIENTE = L_ID_CLIENTE
                                            AND A.TOTAL > 0
                                            ...
                                            AND A.GLOSA LIKE '5ta%2020-1'
                                            AND B.ID_ALUMNO_CONTRATO = L_ID_ALUMNO_CONTRATO  LIMIT 1;
                                        ELSE
                                            SELECT DISTINCT A.ID_VENTA,A.ID_COMPROBANTE,A.SERIE,A.NUMERO INTO STRICT L_ID_PARENT,L_ID_COMPROBANTE_REF,L_SERIE_REF,L_NUMERO_REF
                                            FROM VENTA A JOIN VENTA_DETALLE B
                                            ON A.ID_VENTA = B.ID_VENTA
                                        ...
                                        IF L_CONTAR > 0 THEN
                                            SELECT A.CREDITOSVAR,
                                                    ...
                                                    AND Y.TIPO IN ('E')
                                                    AND X.DC = 'C'
                                                    AND Y.TIENE_HIJO = 0 )/PP.CUOTAS,0))*0.1,0) AS DSCTO_COVID INTO STRICT L_CREDITOS_VAR,L_DSCTO_VAR
                                            FROM DAVID.ACAD_ALUMNO_CONTRATO A JOIN MOISES.PERSONA B ON A.ID_PERSONA = B.ID_PERSONA
                                            JOIN MOISES.PERSONA_NATURAL_ALUMNO C ON B.ID_PERSONA = C.ID_PERSONA
                                            JOIN DAVID.ACAD_SEMESTRE_PROGRAMA D ON A.ID_SEMESTRE_PROGRAMA = D.ID_SEMESTRE_PROGRAMA
                                            JOIN DAVID.ACAD_MATRICULA_DETALLE MD ON A.ID_MATRICULA_DETALLE = MD.ID_MATRICULA_DETALLE
                                            ...
                                            L_VAR := 'N';
                                        END IF;
                                        INSERT INTO FIN_CUOTA_TEST(ID_PERSONA,GLOSA,IMPORTE,VARR,NRO_CUOTA,LAMB,COVID_MAT,ID_MODALIDAD_ESTUDIO,FECHA)VALUES (L_ID_CLIENTE,L_GLOSA,L_IMP_DSCTO,L_VAR,5,'S','N',P_ID_MODALIDAD_ESTUDIO,clock_timestamp()); --2 = 2DA ARMADA
                                    ...
                                    ELSE
                                        ...
                                        INSERT INTO FIN_SIN_DSCTO(ID_PERSONA,IMPORTE,NRO_CUOTA,LAMB,COVID_MAT,ID_MODALIDAD_ESTUDIO,FECHA)VALUES (L_ID_CLIENTE,L_IMP_DSCTO,5,'S','N',P_ID_MODALIDAD_ESTUDIO,clock_timestamp());--2 ARMADA 2
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_finances_sp_dscto_covid (P_ID_ENTIDAD bigint,P_ID_DEPTO text,P_ID_SEMESTRE bigint,P_ID_MODO_CONTRATO bigint, P_ID_NIVEL_ENSENANZA bigint, P_ID_MODALIDAD_ESTUDIO bigint,P_ERROR OUT bigint,P_MSGERROR OUT text) FROM PUBLIC;
