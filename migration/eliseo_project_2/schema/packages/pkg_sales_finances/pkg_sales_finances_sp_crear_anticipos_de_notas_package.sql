...
--                     GOTO salida_rapida;
            ...
            IF L_ERROR = 0 THEN
                ...
                IF P_DC = 'C' THEN
                    ...
                    L_ES_ANTICIPO := 'N';
                ELSE
                    SELECT COUNT(1) INTO STRICT L_CONT FROM VENTA WHERE ID_VENTA = P_ID_VENTA;--GLOSA DE LA NOTA DE CREDITO
                    IF L_CONT  > 0  THEN
                        SELECT GLOSA INTO STRICT L_DET FROM VENTA WHERE ID_VENTA = P_ID_VENTA;
                ...
                VALUES (P_ID_ENTIDAD,P_ID_DEPTO,P_ID_ANHO,P_ID_MES,P_ID_PERSONA,P_ID_CLIENTE,L_ID_TIPOTRANSACCION,L_ID_DINAMICA,L_ID_VOUCHER,L_ID_TIPOORIGEN,L_ID_MONEDA,L_TC,L_SERIE,L_NUMERO,clock_timestamp(),L_GLOSA,ABS(P_IMPORTE),'1',L_ES_ANTICIPO,L_ID_TIPOVENTA)
                RETURNING ID_TRANSFERENCIA INTO L_ID_TRANSFERENCIA;

                IF P_DC = 'C' THEN  --RELACIONA EL ANTRICIPO A UNA VENTA
                    --INSERT INTO VENTA_TRANSFERENCIA_DETALLE(ID_VENTA,ID_TRANSFERENCIA,DC,IMPORTE,DETALLE,ID_TIPOORIGEN)--P_TIPO varchar DEFAULT %DEFAULT1%
                    ...
                    ELSE
                        ...
                        END IF;
                    END IF;
                    INSERT INTO VENTA_TRANSFERENCIA_DETALLE(ID_VENTA,ID_TRANSFERENCIA,DC,IMPORTE,DETALLE,ID_TIPOORIGEN,ID_TRANSFERENCIA_P,ID_SALDO)--P_TIPO varchar DEFAULT 'V'
                    ...
                    --pkg_accounting_sp_asiento_contable(L_ID_DINAMICA,L_ID_TIPOORIGEN,L_ID_TRANSFERENCIA,P_IMPORTE,L_GLOSA,L_ID_VOUCHER,0 ,L_ERROR,L_MSN);
                    
                    --PARA VALIDAR LOS ANNTICIPOS
                    SELECT COUNT(1) INTO STRICT L_CANT FROM VENTA_ANTICIPO WHERE ID_TIPOORIGEN = 1 AND ID_ORIGEN = P_ID_VENTA;
                    IF L_CANT > 0 THEN
                        INSERT INTO VENTA_ANTICIPO(ID_TIPOORIGEN,ID_ORIGEN,ID_DESTINO,FECHA)VALUES (1,P_ID_VENTA,L_ID_TRANSFERENCIA,clock_timestamp());
                    ELSE
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_finances_sp_crear_anticipos_de_notas (P_ID_ENTIDAD bigint,P_ID_DEPTO text,P_ID_ANHO bigint,P_ID_MES bigint,P_ID_CLIENTE bigint,P_ID_PERSONA bigint,P_ID_VENTA bigint,P_IMPORTE bigint,P_DC text,P_CODIGO text,P_ERROR OUT bigint, P_MSN OUT text,P_TIPO text DEFAULT 'V') FROM PUBLIC;
