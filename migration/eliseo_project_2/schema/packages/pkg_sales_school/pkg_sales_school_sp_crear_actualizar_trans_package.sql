...
CREATE OR REPLACE PROCEDURE eliseo.pkg_sales_school_sp_crear_actualizar_trans (P_ID_ENTIDAD bigint,P_ID_DEPTO text,P_ID_ANHO bigint,P_ID_MES bigint,P_ID_PERSONA bigint, P_ID_EMPLEADO bigint,P_ID_CLIENTE bigint,P_ID_VENTA bigint,P_ID_VDETALLE bigint,P_IMPORTE bigint,P_DC text, P_ID_MONEDA bigint, P_ID_TIPOTRANSACCION bigint,P_ID_DINAMICA bigint,P_GLOSA text, P_ES_ENTRECLIENTES text, P_ID_ARTICULO bigint,P_ID_ARTICULO_DESTINO bigint, P_ID_VENTA_DESTINO bigint, P_ID_TRANSFERENCIA INOUT bigint,P_ERROR OUT bigint, P_MSGERROR OUT text) AS $body$
DECLARE
        ...
        l_id_articulo_destino bigint;
        l_id_venta_destino bigint;
        l_id_cliente_destino bigint;
        l_es_entreclientes varchar(5);
        ...
        l_id_comprobante_transf varchar(3);
...
BEGIN
			...
			SELECT ID_COMPROBANTE INTO STRICT l_id_comprobante_transf FROM TIPO_COMPROBANTE WHERE ID_TIPOPAIS = l_id_tipopais AND CODIGO = '99'; --Agregado por Ulices
            SELECT pkg_sales_fc_documento_impresion_user(P_ID_PERSONA,l_id_comprobante_transf,P_ID_ENTIDAD,P_ID_DEPTO) INTO STRICT L_DOCUMENTO;
...
--                 GOTO salida_rapida;
            ...
            l_contar := pkg_sales_fc_verificar_serie_numero(P_ID_PERSONA,l_id_comprobante_transf,P_ID_ENTIDAD, P_ID_DEPTO);
            if l_contar < 1 then
                l_error := 1;
...
--                 GOTO salida_rapida;
            ...
            SELECT ID_TIPOORIGEN, ID_ARTICULO INTO STRICT l_id_tipoorigen_vdetalle, l_id_articulo_vdetalle FROM VENTA_DETALLE WHERE ID_VDETALLE=P_ID_VDETALLE;
            SELECT count(*) into STRICT l_contar FROM CONTA_ASIENTO WHERE ID_TIPOORIGEN = l_id_tipoorigen_vdetalle AND ID_ORIGEN=P_ID_VDETALLE AND PRIMARIO='S';
            if l_contar < 1 then
                l_error := 1;
...
--                 GOTO salida_rapida;
            ...
            IF P_ES_ENTRECLIENTES = 'S' THEN 
                    select max(id_cliente) into STRICT l_id_cliente_destino from venta where id_venta = P_ID_VENTA_DESTINO;
            ...
            IF l_error = 0 then
                ...
                INSERT INTO VENTA_TRANSFERENCIA(
                    ...
                    P_ID_TIPOTRANSACCION,P_ID_DINAMICA,l_id_moneda,l_tc,'-','-',clock_timestamp(),P_GLOSA,0, l_es_entreclientes)
                ...
                pkg_sales_school_sp_obtener_serie_numero(P_ID_PERSONA,l_id_comprobante_transf,P_ID_ENTIDAD
               	 ,P_ID_DEPTO
                ...
                pkg_sales_school_sp_generar_asiento_trans(L_ID_TRANSFERENCIA,l_error,l_msgerror);

                if P_ID_ENTIDAD <> 7124 then 
                 pkg_sales_school_sp_generar_asiento_inter_trs(L_ID_TRANSFERENCIA);
                end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_school_sp_crear_actualizar_trans (P_ID_ENTIDAD bigint,P_ID_DEPTO text,P_ID_ANHO bigint,P_ID_MES bigint,P_ID_PERSONA bigint, P_ID_EMPLEADO bigint,P_ID_CLIENTE bigint,P_ID_VENTA bigint,P_ID_VDETALLE bigint,P_IMPORTE bigint,P_DC text, P_ID_MONEDA bigint, P_ID_TIPOTRANSACCION bigint,P_ID_DINAMICA bigint,P_GLOSA text, P_ES_ENTRECLIENTES text, P_ID_ARTICULO bigint,P_ID_ARTICULO_DESTINO bigint, P_ID_VENTA_DESTINO bigint, P_ID_TRANSFERENCIA INOUT bigint,P_ERROR OUT bigint, P_MSGERROR OUT text) FROM PUBLIC;
