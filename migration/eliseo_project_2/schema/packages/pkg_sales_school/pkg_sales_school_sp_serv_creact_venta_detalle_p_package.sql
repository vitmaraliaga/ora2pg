...
CREATE OR REPLACE PROCEDURE eliseo.pkg_sales_school_sp_serv_creact_venta_detalle_p (P_ID_VENTA bigint,P_ID_SERVICIO bigint,P_CANTIDAD bigint,P_PRECIO bigint,P_ID_VDETALLE INOUT bigint, P_ERROR OUT bigint,P_MSGERROR OUT text) AS $body$
DECLARE
...
BEGIN
        ...
        select COALESCE(PRECIO,0), ID_TIPOIGV, ID_ARTICULO, ID_DINAMICA, SUBSTR(GLOSA, 0,50), precio_editable
                into STRICT l_precio_servicio, l_id_tipoigv, l_id_articulo, l_id_dinamica, l_glosa, l_precio_editable from CAJA_SERVICIO where ID_SERVICIO=P_ID_SERVICIO;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_school_sp_serv_creact_venta_detalle_p (P_ID_VENTA bigint,P_ID_SERVICIO bigint,P_CANTIDAD bigint,P_PRECIO bigint,P_ID_VDETALLE INOUT bigint, P_ERROR OUT bigint,P_MSGERROR OUT text) FROM PUBLIC;
