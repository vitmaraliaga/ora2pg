...
CREATE OR REPLACE PROCEDURE eliseo.pkg_presupuesto_sp_pregrado_proceso (P_ID_ENTIDAD bigint,P_ID_DEPTO_PADRE text,P_ID_ANHO bigint,P_ID_AUXILIAR bigint,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
BEGIN 
    ...
    TIPO
     ...
     SELECT 
      ...
      CASE WHEN coalesce(P.CONVOCA::text, '') = '' THEN IMPORTE_I ELSE IMPORTE_BECA_I END AS IMPORTE_I,
      CASE WHEN coalesce(P.CONVOCA::text, '') = '' THEN IMPORTE_II ELSE IMPORTE_BECA_II END AS IMPORTE_II,
      (CASE WHEN coalesce(P.CONVOCA::text, '') = '' THEN IMPORTE_I ELSE IMPORTE_BECA_I END)*TOTAL_ALUMNO_I AS TOTAL_I,
      (CASE WHEN coalesce(P.CONVOCA::text, '') = '' THEN IMPORTE_II ELSE IMPORTE_BECA_II END)*TOTAL_ALUMNO_II AS TOTAL_II,
      C.TIPO
      ...
      and (CASE WHEN coalesce(P.CONVOCA::text, '') = '' THEN IMPORTE_I ELSE IMPORTE_BECA_I END +
      CASE WHEN coalesce(P.CONVOCA::text, '') = '' THEN IMPORTE_II ELSE IMPORTE_BECA_II END +
      (CASE WHEN coalesce(P.CONVOCA::text, '') = '' THEN IMPORTE_I ELSE IMPORTE_BECA_I END)*TOTAL_ALUMNO_I +
      (CASE WHEN coalesce(P.CONVOCA::text, '') = '' THEN IMPORTE_II ELSE IMPORTE_BECA_II END)*TOTAL_ALUMNO_II )>0 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_presupuesto_sp_pregrado_proceso (P_ID_ENTIDAD bigint,P_ID_DEPTO_PADRE text,P_ID_ANHO bigint,P_ID_AUXILIAR bigint,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
