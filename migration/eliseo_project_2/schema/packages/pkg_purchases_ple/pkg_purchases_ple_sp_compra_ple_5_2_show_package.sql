...
CREATE OR REPLACE PROCEDURE eliseo.pkg_purchases_ple_sp_compra_ple_5_2_show ( P_ID_EMPRESA bigint, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_ID_MES bigint, P_ID_ANHO bigint, OUT CURSOR REFCURSOR ) AS $body$
BEGIN
    ...
    SELECT
					...
					trim(both isc) as campo21,
					trim(both '0.00') as campo22,
					trim(both baseimp5) as campo23,
					trim(both importe) as campo24,
					trim(both moneda) as campo25,  --moneda
					trim(both tipo_cambio) as campo26,  --tipo de cambio
					trim(both fecha_doc_modifica) as campo27,  --fecha de emision comprobante que modifica
					trim(both tipo_doc_modifica) as campo28, --tipo de comprobante de pago que modifica
					trim(both serie_modifica) as campo29, --numero de serie del comprobante de pago que se modifica
					'' as campo30,  --codigo de la dependencia aduanera
					trim(both numdoc_modifica) as campo31, --numero de comprobante de pago que modifica
					trim(both fechadetraccion) as campo32,
					trim(both numerodetraccion) as campo33,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_purchases_ple_sp_compra_ple_5_2_show ( P_ID_EMPRESA bigint, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_ID_MES bigint, P_ID_ANHO bigint, OUT CURSOR REFCURSOR ) FROM PUBLIC;
