...
CREATE OR REPLACE FUNCTION eliseo.pkg_sales_industry_fc_venta_exportacion (P_ID_VENTA bigint) RETURNS text AS $body$
DECLARE
		...
		-- FACTURA
	        ...
	        open VENTA FOR  
		  	SELECT distinct'CF'||
	            ...
	            '|'||REGEXP_REPLACE(COALESCE(A.RAZON_SOCIAL_CLI,'Cliente Varios'),'|','', 'g')||' INCOTERM '||COALESCE(A.ID_INCOTERM,'-')||
		        ...
		        '|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0)+coalesce(A.IGV,0) = 0 THEN '0.00' WHEN coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0)+coalesce(A.IGV,0) BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0)+coalesce(A.IGV,0),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(coalesce(A.GRAVADA,0)+coalesce(A.INAFECTA,0)+coalesce(A.EXONERADA,0)+coalesce(A.VALOR_EXPORTACION,0)+coalesce(A.IGV,0),'9999999990D99')),',','.') END)  ELSE (CASE  WHEN coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME+A.IGV_ME,0) = 0 THEN '0.00' WHEN coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME+A.IGV_ME,0) BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME+A.IGV_ME,0),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(coalesce(A.GRAVADA_ME+A.INAFECTA_ME+A.EXONERADA_ME+A.VALOR_EXPORTACION_ME+A.IGV_ME,0),'9999999990D99')),',','.') END) END || 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.pkg_sales_industry_fc_venta_exportacion (P_ID_VENTA bigint) FROM PUBLIC;
