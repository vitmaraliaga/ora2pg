...
CREATE OR REPLACE PROCEDURE eliseo.pkg_sire_sp_sire_rce_show ( P_ID_EMPRESA bigint, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_ID_MES bigint, P_ID_ANHO bigint, OUT CURSOR REFCURSOR ) AS $body$
DECLARE
...
BEGIN
    ...
    SELECT
                    ...
                    voucher as voucher,
                    -- Se agrega
                    trim(both l_empresa_ruc) AS campo1,
                    trim(both l_empresa_rs) AS campo2,
                    trim(both periodo) AS campo3,
                    '' as campo4,
                    ...
                    -- Se mantiene
                    trim(both fecha_doc) as campo5,
                    trim(both fecha_venc) as campo6,
					trim(both tipo_doc) as campo7,
					trim(both serie) as campo8,
					'0' as campo9, 
					trim(both numdoc) as campo10,
					'' as campo11,
                    trim(both tipo_doc_identidad) as campo12,
					trim(both ruc) as campo13,
					trim(both nombre) as campo14,
					trim(both baseimp1) as campo15, 
					trim(both igv1) as campo16,
					trim(both baseimp2) as campo17,  
					trim(both igv2) as campo18,
					trim(both baseimp3) as campo19, 
					trim(both igv3) as campo20,
					trim(both baseimp4) as campo21, 
					trim(both isc) as campo22,
					trim(both '0.00') as campo23,
					trim(both baseimp5) as campo24,
					trim(both importe) as campo25,
					trim(both moneda) as campo26,  --moneda
					trim(both tipo_cambio) as campo27,  --tipo de cambio
					trim(both fecha_doc_modifica) as campo28,  --fecha de emision comprobante que modifica
					trim(both tipo_doc_modifica) as campo29, --tipo de comprobante de pago que modifica
					trim(both serie_modifica) as campo30, --numero de serie del comprobante de pago que se modifica
					'' as campo31,  --codigo de la dependencia aduanera
					trim(both numdoc_modifica) as campo32, --numero de comprobante de pago que modifica
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sire_sp_sire_rce_show ( P_ID_EMPRESA bigint, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_ID_MES bigint, P_ID_ANHO bigint, OUT CURSOR REFCURSOR ) FROM PUBLIC;
