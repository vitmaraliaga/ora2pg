...
CREATE OR REPLACE PROCEDURE eliseo.pkg_pay_sp_ventas_academicas ( P_ID_PERSONA bigint, P_OPERACION text, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_ID_ANHO bigint, P_ID_MES bigint, P_IMPORTE bigint, P_IP text, P_ID_REGISTRO text, P_ID_CLIENTE bigint, P_ID_COMPROBANTE text, P_ID_TIPOVENTA bigint, P_ID_MONEDA bigint, P_VENTAS OUT text, P_VENTAS_IMP out text, P_ERROR OUT bigint, P_MSGERROR out text ) AS $body$
DECLARE
    ...
    WHERE A.ID_REGISTRO = (P_ID_REGISTRO)::numeric;
...
BEGIN
            ...
            select cos_denominacional into STRICT l_tc from ELISEO.TIPO_CAMBIO
            where id_moneda=9 
            and  fecha =(
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_pay_sp_ventas_academicas ( P_ID_PERSONA bigint, P_OPERACION text, P_ID_ENTIDAD bigint, P_ID_DEPTO text, P_ID_ANHO bigint, P_ID_MES bigint, P_IMPORTE bigint, P_IP text, P_ID_REGISTRO text, P_ID_CLIENTE bigint, P_ID_COMPROBANTE text, P_ID_TIPOVENTA bigint, P_ID_MONEDA bigint, P_VENTAS OUT text, P_VENTAS_IMP out text, P_ERROR OUT bigint, P_MSGERROR out text ) FROM PUBLIC;
