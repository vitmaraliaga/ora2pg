...
CREATE OR REPLACE PROCEDURE eliseo.pkg_pay_sp_deposito_alumnoead_visa ( P_ID_ORDEN bigint, P_IMPORTE bigint, P_IP text, P_ID OUT text, P_ERROR OUT bigint, P_MSGERROR out text ) AS $body$
DECLARE
...
--               GOTO salida_depalumniddni;
        ...
        if P_ID_COMPROBANTE<>'00' then
          CALL pkg_pay_sp_ventas_alumnoead(
                   P_ID_ORDEN,
                   P_ID_PERSONA,	
                   P_OPERACION,
                   P_ID_ENTIDAD,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_pay_sp_deposito_alumnoead_visa ( P_ID_ORDEN bigint, P_IMPORTE bigint, P_IP text, P_ID OUT text, P_ERROR OUT bigint, P_MSGERROR out text ) FROM PUBLIC;
