...
CREATE OR REPLACE PROCEDURE eliseo.pkg_pay_sp_deposito_alumno_upeu_visa ( P_ID_ORDEN bigint, P_IMPORTE bigint, P_IP text, P_ID OUT text, P_ERROR OUT bigint, P_MSGERROR out text ) AS $body$
DECLARE
...
BEGIN
        ...
        IF coalesce(l_id_moneda::text, '') = '' THEN
          l_id_moneda:=7;
        END IF;
...
--             GOTO salida_dep;
        ...
        CALL pkg_pay_sp_ventas_depositar(
                P_ID_ENTIDAD,
                P_ID_DEPTO,
                l_id_anho,
                P_ID_PERSONA,
        ...
        CALL pkg_pay_sp_crear_deposito_alum_visa(P_ID_ENTIDAD,P_ID_DEPTO,l_id_anho,l_id_mes,l_id_mediopago,l_id_user,
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_pay_sp_deposito_alumno_upeu_visa ( P_ID_ORDEN bigint, P_IMPORTE bigint, P_IP text, P_ID OUT text, P_ERROR OUT bigint, P_MSGERROR out text ) FROM PUBLIC;
