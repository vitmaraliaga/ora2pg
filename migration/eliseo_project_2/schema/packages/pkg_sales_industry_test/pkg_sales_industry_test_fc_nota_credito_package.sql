...
CREATE OR REPLACE FUNCTION eliseo.pkg_sales_industry_test_fc_nota_credito (P_ID_VENTA bigint) RETURNS varchar AS $body$
DECLARE
...
UNION ALL
 ...
 SELECT  'DC'||	'|'||ROWNUM||
	    ...
	    '|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(B.PRECIO_BASE,0) = 0 THEN '0.00' WHEN B.PRECIO_BASE BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(B.PRECIO_BASE,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.PRECIO_BASE,'9999999999D99')),',','.') END)  ELSE (CASE  WHEN B.PRECIO_BASE_ME = 0 THEN '0.00' WHEN B.PRECIO_BASE_ME BETWEEN 0.01 AND 0.99 THEN REPLACE(trim(both TO_CHAR(B.PRECIO_BASE_ME,'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.PRECIO_BASE_ME,'9999999999D99')),',','.') END) END ||
            ...
            '|'||CASE WHEN A.ID_MONEDA='7' THEN (CASE  WHEN coalesce(B.PRECIO,0) = 0 THEN '0.00' WHEN B.PRECIO-(coalesce(B.DESCUENTO,0)/B.CANTIDAD) BETWEEN 0.01 AND 0.99 THEN REPLACE(TRIM(TO_CHAR(B.PRECIO-(coalesce(B.DESCUENTO,0)/B.CANTIDAD),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.PRECIO-(coalesce(B.DESCUENTO,0)/B.CANTIDAD),'9999990D99999')),',','.') END)  ELSE (CASE  WHEN B.PRECIO_ME = 0 THEN '0.00' WHEN B.PRECIO_ME-(coalesce(B.DESCUENTO_ME,0)/B.CANTIDAD) BETWEEN 0.01 AND 0.99 THEN REPLACE(TRIM(TO_CHAR(B.PRECIO_ME-(coalesce(B.DESCUENTO_ME,0)/B.CANTIDAD),'90D99')),',','.') ELSE REPLACE(LTRIM(TO_CHAR(B.PRECIO_ME-(coalesce(B.DESCUENTO_ME,0)/B.CANTIDAD),'9999990D99999')),',','.') END) END ||
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eliseo.pkg_sales_industry_test_fc_nota_credito (P_ID_VENTA bigint) FROM PUBLIC;
