...
DECLARE
        ...
        L_ERROR bigint :=0;
        L_MSN varchar(200);
        L_ID_TASIENTO bigint;
        L_GLOSA varchar(100);
        L_GLOSA1 varchar(100);
        L_ID_CLIENTE bigint;
...
BEGIN
            SELECT ID_ENTIDAD||'-'||ID_DEPTO||'-'||GLOSA,ID_ENTIDAD||'-'||ID_DEPTO,ID_CLIENTE INTO STRICT L_GLOSA,L_GLOSA1,L_ID_CLIENTE FROM VENTA_TRANSFERENCIA WHERE ID_TRANSFERENCIA = P_ID_TRANSFERENCIA;
            L_ERROR :=0;
            IF P_MODO = '1' THEN
                ...
                FETCH casi INTO L_ID_ASIENTO,L_ID_RESTRICCION,L_ID_CUENTAAASI,L_DC,L_ID_INDICADOR,L_UNICO,L_UNICOCTACTE,L_PORCENTAJE,L_DESCRIPCION,L_AGRUPA,L_ID_FONDO;
                    WHILE casi%FOUND LOOP
                        ...
                        IF P_IMPORTE<>0 THEN
                            P_ERROR :=0;
                            SELECT COALESCE(MAX(ID_TASIENTO),0)+1 INTO STRICT L_ID_TASIENTO FROM VENTA_TRANSFERENCIA_ASIENTO;
                ...
                END LOOP;
                CLOSE casi;
                SELECT COUNT(1) INTO STRICT L_CONT FROM VENTA_TRANSFERENCIA_ASIENTO
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_sp_crear_transferencia_asiento (P_ID_TRANSFERENCIA bigint,P_ID_DINAMICA bigint,P_ID_CUENTAAASI text,P_ID_RESTRICCION text,P_ID_CTACTE text,P_ID_FONDO bigint,P_ID_DEPTO text,P_IMPORTE bigint,P_IMPORTE_ME bigint,P_DESCRIPCION text, P_EDITABLE text, P_DC text,P_AGRUPA text,P_MODO text,P_ERROR OUT bigint, P_MSN OUT text) FROM PUBLIC;
