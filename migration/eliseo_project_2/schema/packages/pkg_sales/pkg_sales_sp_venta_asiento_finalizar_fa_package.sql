...
CREATE OR REPLACE PROCEDURE eliseo.pkg_sales_sp_venta_asiento_finalizar_fa (P_ID_VENTA bigint,P_ERROR OUT bigint,P_MSGERROR OUT text) AS $body$
DECLARE
...
BEGIN
        SELECT SUM(IMPORTE) INTO STRICT L_IMPORTE_DET FROM VENTA_DETALLE WHERE ID_VENTA = P_ID_VENTA;
        --- Validar que haya contrapartida
        SELECT coalesce(SUM(IMPORTE),0),coalesce(SUM(CASE WHEN DC='D' THEN IMPORTE  ELSE 0 END ),0), COUNT(1) INTO STRICT L_TOTALIZAR_IMPORTE,L_DEBITOS, L_CANTIDAD_ASIENTOS  FROM VENTA_ASIENTO WHERE ID_VENTA = P_ID_VENTA;
        ...
        IF L_ERROR <> 1 THEN
            SELECT ID_ENTIDAD||'-'||ID_DEPTO||'-'||SERIE||'-'||NUMERO,SERIE, NUMERO, ID_VOUCHER, TOTAL,DESCUENTO INTO STRICT L_GLOSA, L_VENTA_SERIE, L_VENTA_NUMERO, L_VENTA_ID_VOUCHER, L_TOTAL, L_DESCUENTO FROM VENTA WHERE ID_VENTA = P_ID_VENTA;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_sales_sp_venta_asiento_finalizar_fa (P_ID_VENTA bigint,P_ERROR OUT bigint,P_MSGERROR OUT text) FROM PUBLIC;
