...
CREATE OR REPLACE PROCEDURE eliseo.pkg_caja_sp_finalizar_devolucion_venta (P_ID_PAGO bigint,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
UNION ALL
        ...
        IF l_error=0 AND L_ENTIDAD_PAGO <> 0 THEN
            ...
            IF L_IMPORTE+L_IMPORTE_ME > 0 THEN
                ...
                WHERE ID_PAGO = P_ID_PAGO;
                if l_error=0  then
                    CALL pkg_caja_sp_generar_asiento_dev_venta(P_ID_PAGO,l_error,l_msgerror);
                end if;
            ELSE
                l_error := 1;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_caja_sp_finalizar_devolucion_venta (P_ID_PAGO bigint,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
