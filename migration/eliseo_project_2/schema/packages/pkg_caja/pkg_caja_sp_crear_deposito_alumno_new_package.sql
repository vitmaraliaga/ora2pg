...
DECLARE
        ...
        l_ventas_tmp varchar(100):=null;
        l_ventas_imp_tmp varchar(100):=null;
        l_tipo_tmp varchar(100):=null;
...
;
       ...
       ---obtiene los documentos 
        ...
        select FC_SPLIT(P_TIPO_VENTAS,'|') into STRICT s_tipo;
        select FC_SPLIT(P_NRO_CUOTA_VENTAS,'|') into STRICT s_nro_cuota;
        ...
        begin
          ...
          ID_VENTA_CONT,IMPORTE,TIPO,NRO_CUOTA
            ...
            (s_ventas(i))::numeric ,
            (s_importes(i))::numeric ,
            s_tipo(i),
            (s_nro_cuota(i))::numeric 
          );
        end;
        end LOOP;

        select 
          string_agg(ID_VENTA_CONT,'|' ORDER BY ID_VENTA_CONT) as VENTAS,
          string_agg(IMPORTE,'|' ORDER BY ID_VENTA_CONT) as VENTAS_IMP,
          string_agg(TIPO,'|' ORDER BY ID_VENTA_CONT) as TIPO
          into STRICT L_VENTAS_D,L_VENTAS_IMP_D,L_TIPO_D
        from ELISEO.TT_VENTAS_DEPOSITO
        where TIPO<>'P';
        ...
        BEGIN
          CALL pkg_finances_students_sp_cobrar_cuota_x_contrato(
            cur.ID_VENTA_CONT,
            P_ID_ENTIDAD,
            P_ID_DEPTO,
            P_ID_PERSONA,
...
--               GOTO SALIDA_RAPIDA;
          ...
          if l_contar>0 then
            select id_venta,total,'V' into STRICT l_ventas_tmp,l_ventas_imp_tmp,l_tipo_tmp
            from VENTA where id_venta = l_id_venta;
            if coalesce(L_VENTAS_D::text, '') = '' then
        ...
        /*IF COALESCE(LENGTH(L_VENTAS_D),0)>0 AND L_ERROR=0 THEN --VALIDA VENTAS POR PAGAR Y LA DIFERENCIA LO ENVIA A ANTICIPOS
            ...
            IF L_IMPORTE_DEP>L_IMPORTEDET THEN
                ...
                pkg_sales_sp_crear_anticipos_clientes(P_ID_ENTIDAD,P_ID_DEPTO,P_ID_ANHO,P_ID_MES,P_ID_CLIENTE,P_ID_PERSONA,L_DEPOSITO,L_IMPORTE_DEP-L_IMPORTEDET,'D',L_COD_DINAMICA_ANT,L_ERROR,L_MSGERROR);
                --genera asiento
                IF P_ID_MEDIOPAGO = '008' THEN
                    pkg_caja_sp_generar_asiento_deposito(L_DEPOSITO,L_ID_DINAMICA,P_ID_CTABANCARIA,L_IMPORTE_DEP-L_IMPORTEDET,0,0,L_GLOSA,L_ERROR,L_MSGERROR);
                ELSE
                    pkg_caja_sp_generar_asiento_deposito(L_DEPOSITO,L_ID_DINAMICA,P_ID_CTABANCARIA,0,L_IMPORTE_DEP-L_IMPORTEDET,0,L_GLOSA,L_ERROR,L_MSGERROR);
                END IF;
        ...
        ELSE-- TODO EL IMPORTE VA A ANTICIPOS
            ...
            --insert Anticipo
            L_COD_DINAMICA_ANT:='AN';-- anticipo a favor del cliente
            pkg_sales_sp_crear_anticipos_clientes(P_ID_ENTIDAD,P_ID_DEPTO,P_ID_ANHO,P_ID_MES,P_ID_CLIENTE,P_ID_PERSONA,L_DEPOSITO,L_IMPORTE_DEP,'D',L_COD_DINAMICA_ANT,L_ERROR,L_MSGERROR);
            --genera asiento
            IF P_ID_MEDIOPAGO = '008' THEN
                pkg_caja_sp_generar_asiento_deposito(L_DEPOSITO,L_ID_DINAMICA,P_ID_CTABANCARIA,L_IMPORTE_DEP,0,0,L_GLOSA,L_ERROR,L_MSGERROR);
            ELSE
                pkg_caja_sp_generar_asiento_deposito(L_DEPOSITO,L_ID_DINAMICA,P_ID_CTABANCARIA,0,L_IMPORTE_DEP,0,L_GLOSA,L_ERROR,L_MSGERROR);
            END IF;
        END IF;
        */
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_caja_sp_crear_deposito_alumno_new (P_ID_ENTIDAD bigint,P_ID_DEPTO text,P_ID_ANHO bigint,P_ID_MES bigint,P_ID_MEDIOPAGO text,P_ID_PERSONA bigint,P_ID_CLIENTE bigint,P_AUTOMATICO text,P_VENTAS text,P_IMP_VENTAS text,P_ID_TIPOTRANSACCION bigint, P_ID_MONEDA bigint,P_ID_DINAMICA bigint,P_ID_TIPOTARJETA bigint,P_ID_CTABANCARIA bigint,P_OPERACION text,P_FECHA_OP timestamp(0),P_IMPORTE bigint,P_IMPORTE_TARJETA bigint,P_IMPORTE_ME bigint, P_TIPOCAMBIO bigint,P_GLOSA text,P_NOMBRE_DEP text,P_DOCUMENTO_DEP text,P_ID_TIPOASIENTO text,P_TIPO_VENTAS text,P_NRO_CUOTA_VENTAS text,P_ID_CANAL integer DEFAULT NULL,P_ERROR OUT bigint,P_MSGERROR OUT text, P_ID_DEPOSITO OUT bigint) FROM PUBLIC;
