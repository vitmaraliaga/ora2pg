...
CREATE OR REPLACE PROCEDURE eliseo.pkg_caja_sp_crear_deposito_detalle_aces (P_ID_DEPOSITO bigint,P_VENTAS text,P_IMP_VENTAS text,P_TIPO text,P_ID_MONEDA bigint,P_TIPO_CAMBIO bigint,P_ID_DINAMICA bigint,P_ERROR out bigint, P_MSGERROR out text) AS $body$
DECLARE
...
BEGIN
      ...
      LOOP
          ...
          select id_entidad,id_cliente into STRICT l_id_entidad,l_id_cliente from caja_deposito
          where id_deposito = P_ID_DEPOSITO;

            SELECT  TIPO INTO STRICT s_tipo
            FROM VW_SALES_MOV
         ...
         select count(1) into STRICT l_contar from VENTA_DETALLE A,CONTA_DINAMICA B, CONTA_DINAMICA_ASIENTO C
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE eliseo.pkg_caja_sp_crear_deposito_detalle_aces (P_ID_DEPOSITO bigint,P_VENTAS text,P_IMP_VENTAS text,P_TIPO text,P_ID_MONEDA bigint,P_TIPO_CAMBIO bigint,P_ID_DINAMICA bigint,P_ERROR out bigint, P_MSGERROR out text) FROM PUBLIC;
