...
DECLARE
P_ORIGEN VARCHAR2(50);
P_ID_INFORME NUMBER;
BEGIN
	P_ORIGEN:=:NEW.ORIGEN;
	IF P_ORIGEN ='informe_ayuda' THEN 
		SELECT ID_INFORME INTO P_ID_INFORME FROM ELISEO.INFORME_AYUDA WHERE ID_IAYUDA =:NEW.ID_ORIGEN;
	...
	ELSE
		IF P_ORIGEN ='informe_gasto' THEN 
			SELECT ID_INFORME INTO P_ID_INFORME FROM ELISEO.INFORME_GASTO WHERE ID_IGASTO =:NEW.ID_ORIGEN;
			SELECT DECODE(ESTADO_ACTUAL,'R', 'V', 'V', 'A', 'A') INTO :NEW.ESTADO FROM ELISEO.INFORME WHERE ID_INFORME =P_ID_INFORME;
		ELSIF P_ORIGEN ='compra' THEN
			SELECT ID_INFORME INTO P_ID_INFORME FROM ELISEO.COMPRA WHERE ID_COMPRA = :NEW.ID_ORIGEN;
			SELECT DECODE(ESTADO_ACTUAL,'R', 'V', 'V', 'A', 'A') INTO :NEW.ESTADO FROM ELISEO.INFORME WHERE ID_INFORME =P_ID_INFORME;
		END IF;
	END IF;
END
