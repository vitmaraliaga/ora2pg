...
PROCEDURE        iudp_Activo_Movimiento (P_Modo IN int, P_ID_ENTIDAD IN int, P_COD_ACTIVO IN varchar2, P_ID_TIPOACTIVOMOVIMIENTO IN int, P_FECHA IN varchar2, P_VALOR IN varchar2, P_ES_CONTABILIZADO IN boolean,P_NUM_LOTE IN VARCHAR2 ) AS
	S_COD_ACTIVO          VARCHAR2(60);
	S_NUM_LOTE            VARCHAR2(60);
 ...
 BEGIN
    ...
    IF P_Modo = 0 THEN
        ...
        if not P_ES_CONTABILIZADO is null 
        then 
        ...
        IF S_CANTIDAD > 0 THEN
            ...
            SELECT ID_ACTIVO, FEC_COMPRA INTO S_ID_ACTIVO,S_FEC_COMPRA
            FROM CONTA_ACTIVO 
            WHERE ID_ENTIDAD = P_ID_ENTIDAD
            AND COD_ACTIVO = P_COD_ACTIVO;
            
            SELECT COUNT(ID_ACTIVO) INTO S_CANT 
