...
PROCEDURE        iudp_Entidad_Departamento (
P_Modo IN int, P_ID_ENTIDAD INT, P_ID_DEPARTAMENTO IN varchar2, P_ID_PARENT IN varchar2, P_NOMBRE IN varchar2, P_ES_ACTIVO IN varchar2
) AS
 
  S_ID_PARENT           varchar2(60);
  S_ES_ACTIVO           varchar2(60);
  S_CANTIDAD            INT;  
  S_ID_DEPARTAMENTO     varchar2(60);
...
IF P_Modo = 0 THEN
	...
	IF COALESCE(S_ID_PARENT, '') = '' THEN
	    S_CANTIDAD := 0;
	    SELECT COUNT(*) INTO S_CANTIDAD FROM CONTA_ENTIDAD_DEPTO WHERE ID_ENTIDAD = P_ID_ENTIDAD AND ID_DEPTO = S_ID_PARENT; 
	    IF S_CANTIDAD = 0 THEN
	    	iudp_Entidad_Departamento (P_Modo, P_ID_ENTIDAD, S_ID_PARENT, NULL, P_NOMBRE, S_ES_ACTIVO);
	    END IF;
    END IF;
    
    S_CANTIDAD := 0;
    SELECT COUNT(*) INTO S_CANTIDAD FROM CONTA_ENTIDAD_DEPTO WHERE ID_ENTIDAD = P_ID_ENTIDAD AND ID_DEPTO = S_ID_DEPARTAMENTO; 
