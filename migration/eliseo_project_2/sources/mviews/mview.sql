...
CREATE MATERIALIZED VIEW VWM_CONTA_DIARIO_TOTALES
BUILD IMMEDIATE
REFRESH FORCE ON DEMAND
AS SELECT ID_CUENTAAASI, ID_ANHO, ID_MES, SUM(COS_VALOR) COS_VALOR  FROM VW_CONTA_DIARIO WHERE ID_DEPTO IS NOT NULL --AND ID_ANHO=2021 AND ID_MES<=12 
	AND ID_FONDO IS NOT NULL AND ID_TIPOASIENTO<>'EA'
 GROUP BY ID_CUENTAAASI, ID_ANHO, ID_MES USING NO INDEX;



DROP MATERIALIZED VIEW IF EXISTS VWM_SALES_DETAIL_MOV_COL;
CREATE MATERIALIZED VIEW VWM_SALES_DETAIL_MOV_COL
BUILD IMMEDIATE
REFRESH FORCE ON DEMAND
AS SELECT col.* FROM VW_SALES_DETAIL_MOV_COL_DIARIO col 
INNER JOIN jose.SCHOOL_INSTITUCION si ON si.ID_CAMPO = col.ID_ENTIDAD AND si.ID_DEPTO = col.ID_DEPTO USING NO INDEX;



DROP MATERIALIZED VIEW IF EXISTS VWM_SALES_DETAIL_MOV_COL_D;
CREATE MATERIALIZED VIEW VWM_SALES_DETAIL_MOV_COL_D
BUILD IMMEDIATE
REFRESH FORCE ON DEMAND
AS SELECT "ID_ENTIDAD","ID_DEPTO","ID_ANHO","ID_MES","ID_VOUCHER","ID_VENTA","ID_ARTICULO","ID_CLIENTE","ID_SUCURSAL","ID_COMPROBANTE","ID_MONEDA","SERIE","NUMERO","DOCUMENTO_VENTA","SERIE_MOV","NUMERO_MOV","DOCUMENTO_MOV","FECHA","GLOSA","DETALLE","IMPORTE","IMPORTE_ME","ID_TIPOTRANSACCION","ID_DEPTO_CCOSTO" FROM (  
  SELECT 	  A.ID_ENTIDAD,
          A.ID_DEPTO,
          A.ID_ANHO,
          A.ID_MES,
...
UNION ALL 
    SELECT A.ID_ENTIDAD,
          ...
          CASE WHEN COALESCE(C.TOTAL_ME,0) = 0 THEN 
          	0 ELSE 
          	(D.IMPORTE_ME/C.TOTAL_ME*B.IMPORTE_ME) * -1
          	END
          ELSE 
          	B.IMPORTE_ME *-1
          END,
    ...
    SELECT A.ID_ENTIDAD,
          ...
          A.GLOSA,
          D.DETALLE,
          DECODE(D.DC,'C',-1,1)*D.IMPORTE AS TOTAL,
          DECODE(D.DC,'C',-1,1)*D.IMPORTE_ME AS TOTAL_ME,
          A.ID_TIPOTRANSACCION,
          A.ID_DEPTO_CCOSTO
   FROM VENTA A 
...
CREATE MATERIALIZED VIEW VWM_SALES_MOV
BUILD IMMEDIATE
REFRESH FORCE ON DEMAND
AS SELECT "ID_ENTIDAD","ID_DEPTO","ID_ANHO","ID_MES","ID_VENTA","ID_CLIENTE","ID_SUCURSAL","ID_COMPROBANTE","ID_MONEDA","SERIE","NUMERO","FECHA","GLOSA","TOTAL","TOTAL_ME","ID_TIPOTRANSACCION","ID_TIPOVENTA","TIPO","MOV","VOUCHER","LOTE","TIPO_DOCUMENTO","NUMERO_LEGAL"
FROM (  
SELECT A.ID_ENTIDAD,
          A.ID_DEPTO,
          A.ID_ANHO,
   ...
   SELECT A.ID_ENTIDAD,
          ...
          A.SERIE || '-' || A.NUMERO AS NUMERO_LEGAL
     FROM VENTA_SALDO A
    WHERE ID_VENTA IS NULL AND A.TOTAL > 0
   ...
   SELECT DISTINCT A.ID_ENTIDAD,
    ...
    WHERE     A.ES_ANTICIPO = 'N'
          AND B.ID_VENTA IS NULL
          AND B.ID_SALDO IS NULL
          AND B.ID_TRANSFERENCIA_P IS NULL
          AND A.ESTADO = 1
          AND A.IMPORTE <> 0
...
CREATE MATERIALIZED VIEW VWM_SALES_MOV_SSI
BUILD IMMEDIATE
REFRESH FORCE ON DEMAND
AS SELECT "ID_ENTIDAD","ID_DEPTO","ID_ANHO","ID_MES","ID_VENTA","ID_CLIENTE","ID_SUCURSAL","ID_COMPROBANTE","ID_MONEDA","SERIE","NUMERO","FECHA","GLOSA","TOTAL","TOTAL_ME","ID_TIPOTRANSACCION","ID_TIPOVENTA","TIPO","MOV","VOUCHER","LOTE","TIPO_DOCUMENTO","NUMERO_LEGAL"
FROM (  
