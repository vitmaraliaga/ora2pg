...
FUNCTION FC_SALDO_ALUMNO(P_ID_PERSONA INTEGER, P_ANIO INTEGER,P_ID_DEPTO VARCHAR2)
    RETURN NUMBER IS
    SALDO	  NUMBER(10,2);		 
    ...
    BEGIN
        SELECT CODIGO INTO V_CODIGO FROM MOISES.PERSONA WHERE ID_PERSONA = P_ID_PERSONA;

        SELECT --DECODE(LENGTH(SIGN(NVL(SUM(TOTAL),0))),2,'',0)||SIGN(NVL(SUM(TOTAL),0))||'-'||NVL(ABS(SUM(TOTAL)),0) 
                NVL(SUM(TOTAL),0) INTO SALDO
        FROM (
            SELECT TOTAL
