...
CREATE OR REPLACE VIEW vw_conta_saldos (id_empresa, id_entidad, id_anho, id_mes, id_depto, id_fondo, id_tipoplan, id_cuentaaasi, id_restriccion, id_ctacte, debe, haber, saldo) AS SELECT
  ...
  SUM(CASE WHEN CONTA_DIARIO_DETALLE.COS_VALOR > 0 THEN ABS(ROUND(CONTA_DIARIO_DETALLE.COS_VALOR, 2)) ELSE 0 END) - SUM(CASE WHEN CONTA_DIARIO_DETALLE.COS_VALOR < 0 THEN ABS(ROUND(CONTA_DIARIO_DETALLE.COS_VALOR, 2)) ELSE 0 END) AS SALDO
FROM CONTA_ENTIDAD
