...
CREATE OR REPLACE VIEW vw_sales_detail_mov_col_dtemp (id_entidad, id_depto, id_anho, id_mes, id_voucher, id_venta, id_articulo, id_cliente, id_sucursal, id_comprobante, id_moneda, serie, numero, documento_venta, serie_mov, numero_mov, documento_mov, fecha, glosa, detalle, importe, importe_me, id_tipotransaccion, id_depto_ccosto) AS SELECT ID_ENTIDAD,ID_DEPTO,ID_ANHO,ID_MES,ID_VOUCHER,ID_VENTA,ID_ARTICULO,ID_CLIENTE,ID_SUCURSAL,ID_COMPROBANTE,ID_MONEDA,SERIE,NUMERO,DOCUMENTO_VENTA,SERIE_MOV,NUMERO_MOV,DOCUMENTO_MOV,FECHA,GLOSA,DETALLE,IMPORTE,IMPORTE_ME,ID_TIPOTRANSACCION,ID_DEPTO_CCOSTO FROM (  
  SELECT 	  A.ID_ENTIDAD,
          A.ID_DEPTO,
          A.ID_ANHO,
          A.ID_MES,
