...
PROCEDURE       pcd_f_evaluacion_docente(p_id_semestre integer) IS
...
BEGIN
	...
	DELETE FROM david.evaluacion_grupo_focal WHERE id_semestre=p_id_semestre;
     ...
     INSERT INTO david.EVALUACION_ACTIVIDAD_DOCENTE 
		...
		pe.id_persona,
		avg(nvl(PROMEDIO_ASISTENCIA_DOCENTE,0)) * 0.1 AS PROMEDIO_ASISTENCIA_DOCENTE,
		avg(nvl(promedio_asistencia, 0))*0.1 AS promedio_asistencia,
		avg(nvl(promedio_notas_semana,0))*0.3 AS promedio_notas_semana,
		avg(nvl(canvas,0))*0.5 AS canvas,
		1  AS id_ope
...
vape.id_programa_estudio,
pe.id_persona,
avg(nvl(redm.capacitacion,0)) * 0.1 AS capacitacion,
...
vape.id_programa_estudio,
pe.id_persona,
avg(nvl(redm.capacitacion,0)) AS capacitacion, -- * 0.1 AS capacitacion,
...
ic.consolidado, ic.id_cuestionario,
  nvl(ins_seccion_n('34341', ip.IDENTIFICADOR,ip.id_ejecucion), 0) AS dimension_1,
nvl(ins_seccion_n('34347', ip.IDENTIFICADOR,ip.id_ejecucion), 0) AS dimension_2,
nvl(ins_seccion_n('34354', ip.IDENTIFICADOR,ip.id_ejecucion), 0) AS dimension_3,
nvl(ins_seccion_n('34361', ip.IDENTIFICADOR,ip.id_ejecucion), 0) AS dimension_4
