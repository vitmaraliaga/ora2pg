...
PROCEDURE pcd_temporal_carga_docente(P_ID_SEMESTRE NUMBER)
IS
  l_contar number;
  l_error number:=0;
  l_msgerror varchar2(200):='ok';
  l_id_carga_curso_docente number;
  ...
  inner join acad_modulo_detalle amd on amd.id_modulo_detalle=accd.id_modulo_detalle/*(select asp.id_semestre,to_date('01/'||min(to_char(Amd.Fecha_Inicio,'MM'))||'/'||max(to_char(Amd.Fecha_Inicio,'YYYY')),'DD/MM/YYYY') fecha_inicio,
              TRUNC(LAST_DAY(max(Amd.Fecha_fin))) fecha_fin
              from acad_semestre_programa asp inner join acad_modulo_detalle amd on Amd.Id_Semestre_Programa=Asp.Id_Semestre_Programa
              inner join acad_carga_curso_docente accd on Accd.Id_Modulo_Detalle=Amd.Id_Modulo_Detalle
              group by asp.id_semestre) amd on amd.id_semestre=aca.id_semestre*/
...
BEGIN
  ...
  delete david.tb_pcd_horas_dia_docente where ID_SEMESTRE=P_ID_SEMESTRE;
  commit;
  delete david.tb_pcd_carga_docente;
  commit;
  delete tb_pcd_horas_mes_ccd where ID_SEMESTRE=P_ID_SEMESTRE;
  commit;
                  ...
                  while l_i<=l_dias LOOP
                    ...
                    if l_hora = '1' then
                      ...
                      if l_fecha>=curD.fecha_inicio and l_fecha <=curD.fecha_fin then
                        INSERT INTO DAVID.TB_PCD_CARGA_SEMANA(DIA,HORA,ID_TIPO_HORARIO,FECHA) VALUES(l_i,TO_NUMBER(l_hora),curD.ID_TIPO_HORARIO, l_fecha);
                        commit;
                     end if;
                    end if;
                    l_fecha:= l_fecha + 1;
  ...
  delete tb_pcd_fecha_hora_cur_doc where id_semestre=P_ID_SEMESTRE;
  
  commit;

  insert into tb_pcd_fecha_hora_cur_doc 
  select distinct a.id_semestre,acd.id_carga_curso_docente,A.Fecha,trim(to_char(a.Fecha,'DAY')) dia,
  FT_CURSO_DOCENTE_HORARIO(DIA_HORA_CURSO_N(a.horario,''||trim(to_char(a.Fecha,'DAY')),Asp.Id_Horario),to_char(a.fecha,'D'),Asp.Id_Horario,A.Horas,'I') HORA_INICIO,
  FT_CURSO_DOCENTE_HORARIO(DIA_HORA_CURSO_N(a.horario,''||trim(to_char(a.Fecha,'DAY')),Asp.Id_Horario),to_char(a.fecha,'D'),Asp.Id_Horario,A.Horas,'F') HORA_FIN
...
END pcd_temporal_carga_docente;
