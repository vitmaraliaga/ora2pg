...
procedure pcd_act_matriculas_comp
as


begin
  EXECUTE IMMEDIATE 'truncate table aatemp_matriculas_verificar';
    commit; 
  ...
  delete aatemp_matriculas_verificar where semestre in (select semestre from acad_semestre where ESTADO_ADVISER=1);
  commit;
  
  delete aatemp_matriculas_verificar where semestre like '%V%';
  
  delete aatemp_matriculas_verificar where (semestre,id_persona,id_escuela)  
  not in (SELECT max(semestre),id_persona,id_escuela FROM aatemp_matriculas_verificar group by id_persona,id_escuela);
  commit;
  
  delete aatemp_matriculas_verificar where (id_persona,id_escuela)  
