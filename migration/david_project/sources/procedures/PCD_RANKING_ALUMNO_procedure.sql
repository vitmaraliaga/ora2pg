...
procedure pcd_ranking_alumno(p_id_semestre in integer)
as
...
begin
  ...
  pcd_alumno_ciclo(p_id_semestre);
    COMMIT;
  ...
  from (select distinct aac.id_persona,aac.id_plan_programa,aac.id_semestre,aac.id_programa_estudio,Aac.Ciclo,
        nvl(ft_n_ponderado_general_plan(aac.id_persona,aac.id_plan_programa),0) ponderado_general,
        nvl(ft_ponderado_ciclo(aac.id_persona,aac.id_semestre,aac.id_nivel_ensenanza),0) ponderado_ciclo
  ...
  where (id_persona,id_plan_programa,id_semestre,id_programa_estudio,ciclo) not in 
        (select id_persona,id_plan_programa,id_semestre,id_programa_estudio,ciclo from aatemp_poderados_semestre);
  commit;
  
  delete aatemp_ranking_semestre where id_semestre=p_id_semestre;
