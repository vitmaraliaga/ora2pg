...
procedure PCD_ACT_AVANCE_SILABO2024( pval in integer) AS 
...
begin 
    ...
    DELETE AATEMP_AVANCE_SILABO2024 WHERE ID_SEMESTRE=p_id_semestre;
    commit; 
    ...
    nvl(cs.sumilla,0) sumilla,nvl(cc.competencia,0) competencia,nvl(cra.resultado_aprendizaje,0) resultado_aprendizaje,
    nvl(cu.unidad,0) unidades,nvl(CT.CANTIDAD_TEMAS,0)CANTIDAD_TEMAS,nvl(CT.TIENE_TEMAS,0) TIENE_TEMAS,
    nvl(ct.temas,'NO') temas,nvl(cta.tema_anexo,0) tema_anexo,
    nvl(cev.TIENE_EVALUACION,0) TIENE_EVALUACION, nvl(cev.evaluaciones,'NO') evaluaciones,NVL(CEV.PESO,0) PESO,
    nvl(ce.estrategias,0) estrategias,
    nvl(cr.recursos,0) recursos,nvl(cb.bibliografia,0) bibliografia
    ...
    left join (select ce.id_carga_curso, sum(peso) PESO, 1 TIENE_EVALUACION,
                (case when sum(peso)=100 then 'COMPLETO'
                      ...
                      from ( select ce.id_carga_curso,Ce.Id_Carga_Evaluacion, peso peso from Acad_Carga_Evaluacion ce 
                            ...
                            where ce.estado='1' and ce.id_carga_competencia is not null) ce
