...
FUNCTION DIA_HORA_CURSO(HORARIO VARCHAR2)
RETURN VARCHAR2 IS
  ...
  IF LENGTH(TRIM(dias)) >0 THEN
    FOR y IN 1..length(TRIM(dias)) LOOP
      ...
      CASE
       ...
       WHEN var = '3' THEN IF y<length(TRIM(dias)) THEN dias2 :=CONCAT(dias2, CONCAT('Ma-',CONCAT(turno2,'->')));ELSE dias2 :=CONCAT(dias2, CONCAT('Ma-',turno2)); END IF ;
       WHEN var = '4' THEN IF y<length(TRIM(dias)) THEN dias2 :=CONCAT(dias2, CONCAT('Mi-',CONCAT(turno2,'->')));ELSE dias2 :=CONCAT(dias2, CONCAT('Mi-',turno2)); END IF ;
