...
function ft_calcular_promedio_act(v_id_carga_curso number, v_id_persona number, v_id_programa_estudio number)
return number
is
...
begin
    ...
    if (v_id_programa_estudio = 1) then
        select round(round((sum(aae.nota*ace.ponderado)/nvl(sum(ace.ponderado),1)),2),0) into promedio_ponderado
        ...
        select round(round((sum(aae.nota*ace.ponderado)/nvl(sum(ace.ponderado),1)),2),0) into promedio_final
        from acad_alumno_evaluacion aae
        inner join acad_carga_evaluacion ace on ace.id_carga_evaluacion = aae.id_carga_evaluacion
        where aae.id_carga_curso = v_id_carga_curso and aae.id_persona = v_id_persona;    
    end if;
