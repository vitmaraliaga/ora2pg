...
FUNCTION FT_CURSO_APROBADO(V_ID_PERSONA INTEGER,V_ID_CURSO_DETALLE in integer)
RETURN VARCHAR2 AS
P_RESULTADO varchar2(500);
BEGIN

  SELECT DISTINCT 'S' INTO P_RESULTADO
  FROM(
  SELECT NVL(APC.ID_CURSO_DETALLE,ACA.ID_CURSO_DETALLE) ID_CURSO_DETALLE
  ...
  FROM ACAD_CURSO_ALUMNO ACA WHERE ACA.ID_PERSONA=V_ID_PERSONA AND ACA.ID_TIPO_CONDICION=1 AND ID_PLAN_PROGRAMA IS NULL
  ) APC
