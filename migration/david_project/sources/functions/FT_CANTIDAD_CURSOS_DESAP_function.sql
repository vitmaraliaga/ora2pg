...
function ft_cantidad_cursos_desap(v_id_persona integer, v_id_plan_programa integer)
RETURN number IS
  p_res	  number;	
  p_area	  number;
BEGIN

    select id_escuela into p_area from Vw_Plan_Programa where Id_Plan_Programa=v_id_plan_programa;

    SELECT Distinct 1 INTO p_res
    FROM(
          ...
          AND NVL(NVL(FT_CURSO_APROBADO_NOM(v_id_persona,ACA.NOMBRE_CURSO),FT_CURSO_APROBADO(v_id_persona,ACA.ID_CURSO_DETALLE)),'N')='N'
