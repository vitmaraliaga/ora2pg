...
function ft_nivel_riesgo_ant(v_ponderado number, v_desaprobados number, v_tipo varchar2)
RETURN varchar2 IS
  p_res	  varchar2(500);		 
...
BEGIN
    ...
    if(v_tipo='R') then
      ...
      select (case when round(v_ponderado,0)<13 then 'MA' 
      ...
      select (case when round(v_ponderado,0)<13 then 'Muy Alto' 
              when round(v_ponderado,0)>=13 
