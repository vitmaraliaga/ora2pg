...
function       ft_new_users_dni(v_id_persona number) RETURN VARCHAR2 AS
    ...
    CONCAT(
        sintilde_con_punto(
            decode(
                        ...
                        decode(
                            sintilde_con_punto(
                                decode(
                                    sintilde_con_punto(
                                        lower(
                                            decode(INSTR(p.nombres,' '),'0',p.nombres,substr(p.nombres,0,INSTR(p.nombres,' ')-1))
                                            ||'.'||p.paterno
                                        )
                                    ), 
                                    u.email,
                                    lower(
                                        decode(INSTR(p.nombres,' '),'0',p.nombres,substr(p.nombres,INSTR(p.nombres,' ')+1))
                                        ||'.'||p.paterno
                                    ),
                                    sintilde_con_punto(
                                        lower(
                ...
                u3.email,
                lower(replace(p.nombres,' ')||'.'||p.paterno||'.'||substr(p.materno,0,1)),
                sintilde_con_punto(
                    replace(
                        decode(
                            decode(
