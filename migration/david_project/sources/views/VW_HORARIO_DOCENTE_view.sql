...
inner join (select ah.Id_Horario,Tn.Codigo,hd.Periodo,
              (CASE WHEN to_number(SUBSTR(hd.Hora_Inicio,0,2))<6 AND hd.Id_Tipo_Turno=1 
                  then to_number(SUBSTR(hd.Hora_Inicio,0,2))+12||SUBSTR(Hora_Inicio,-3)
                when to_number(SUBSTR(hd.Hora_Inicio,0,2))<12 AND hd.Id_Tipo_Turno>1  
                  then to_number(SUBSTR(hd.Hora_Inicio,0,2))+12||SUBSTR(hd.Hora_Inicio,-3)
                else hd.Hora_Inicio 
        ...
        ON hi.Id_Horario=asp.Id_Horario 
            and hi.Codigo=substr(DIA_HORA_CURSO3(HORARIO,asp.id_horario),INSTR(DIA_HORA_CURSO3(HORARIO,asp.id_horario),ds.codigo)+1,1) 
            and hi.Periodo=substr(DIA_HORA_CURSO3(HORARIO,asp.id_horario),INSTR(DIA_HORA_CURSO3(HORARIO,asp.id_horario),ds.codigo)+2,1)
inner join (select ah.Id_Horario,
                    (CASE WHEN to_number(SUBSTR(hd.hora_fin,0,2))<6 AND hd.Id_Tipo_Turno=1 
                        then to_number(SUBSTR(hd.hora_fin,0,2))+12||SUBSTR(hd.hora_fin,-3)
                      when to_number(SUBSTR(hd.hora_fin,0,2))<12 AND hd.Id_Tipo_Turno>1 
                        then to_number(SUBSTR(hd.hora_fin,0,2))+12||SUBSTR(hd.hora_fin,-3)
                      else hd.hora_fin 
