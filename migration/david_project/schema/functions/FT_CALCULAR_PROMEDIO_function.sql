...
CREATE OR REPLACE FUNCTION david.ft_calcular_promedio (v_id_carga_curso bigint, v_id_persona bigint, v_id_programa_estudio bigint) RETURNS bigint AS $body$
DECLARE
...
BEGIN
    ...
    if (v_id_programa_estudio = 1) then
        ...
        select round((round((sum(aae.nota*ace.ponderado/100))::numeric,2))::numeric,0) into STRICT promedio_final
        from acad_alumno_evaluacion aae
        inner join acad_carga_evaluacion ace on ace.id_carga_evaluacion = aae.id_carga_evaluacion
        where aae.id_carga_curso = v_id_carga_curso and aae.id_persona = v_id_persona;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.ft_calcular_promedio (v_id_carga_curso bigint, v_id_persona bigint, v_id_programa_estudio bigint) FROM PUBLIC;
