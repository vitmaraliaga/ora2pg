...
CREATE OR REPLACE FUNCTION david.ft_calcular_likert ( v_id_ejecucion bigint, v_id_programa_estudio bigint, v_valor_respuesta bigint, v_id_pregunta bigint ) RETURNS bigint AS $body$
DECLARE
...
BEGIN
		...
		SELECT coalesce(contador,0) AS contador INTO STRICT   valor_calculado
		FROM ( 
		SELECT vaac.sede, 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.ft_calcular_likert ( v_id_ejecucion bigint, v_id_programa_estudio bigint, v_valor_respuesta bigint, v_id_pregunta bigint ) FROM PUBLIC;
