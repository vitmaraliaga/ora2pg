...
CREATE OR REPLACE FUNCTION david.ft_ciclo_plan_alumno_v2 (p_id_persona integer,p_id_plan_programa integer) RETURNS bigint AS $body$
DECLARE
...
BEGIN
  ...
  select coalesce((select sum(acp.credito)
                ...
                inner join(select aca.Id_Plan_Programa,aca.promedio,aca.id_tipo_condicion,
                            ...
                            where aca.id_persona=p_id_persona  and aca.estado='1' and aca.Id_Tipo_Condicion not in (3,4,5,7,6,15) 
                            ) aca
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.ft_ciclo_plan_alumno_v2 (p_id_persona integer,p_id_plan_programa integer) FROM PUBLIC;
