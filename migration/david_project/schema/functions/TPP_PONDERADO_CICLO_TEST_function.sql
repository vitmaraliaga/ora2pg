...
CREATE OR REPLACE FUNCTION david.tpp_ponderado_ciclo_test ( v_id_persona integer, v_id_plan_programa integer, v_id_semestre integer, v_tipo text) RETURNS varchar AS $body$
DECLARE
...
BEGIN
    ...
    sum(20*ace.ponderado)/100 ponderado_a_alcanzar,round(round((sum(Aae.Nota*coalesce(ace.ponderado,(SELECT sum(aced.peso)from david.acad_carga_evaluacion_detalle aced where aced.id_carga_evaluacion = aae.id_carga_evaluacion and aced.id_tipo_evaluacion = 2)))/sum(coalesce(ace.ponderado,(select sum(aced.peso)from david.acad_carga_evaluacion_detalle aced where aced.id_carga_evaluacion = aae.id_carga_evaluacion and aced.id_tipo_evaluacion = 2)))),2)) ponderado_fecha ,credito 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.tpp_ponderado_ciclo_test ( v_id_persona integer, v_id_plan_programa integer, v_id_semestre integer, v_tipo text) FROM PUBLIC;
