...
CREATE OR REPLACE FUNCTION david.ft_facultad_alumno_nd (P_NUM_DOC text) RETURNS varchar AS $body$
DECLARE
...
IF CANTIDAD>0 THEN
	SELECT
	DISTINCT fac_ads INTO STRICT P_FACULTAD
FROM (
	SELECT
		apc.campus_ads,
		apc.fac_ads,
...
SELECT C.NOMBRE INTO STRICT P_FACULTAD
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.ft_facultad_alumno_nd (P_NUM_DOC text) FROM PUBLIC;
