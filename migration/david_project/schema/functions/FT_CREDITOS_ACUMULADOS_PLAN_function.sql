...
CREATE OR REPLACE FUNCTION david.ft_creditos_acumulados_plan (p_id_persona integer,p_id_plan_programa integer) RETURNS bigint AS $body$
DECLARE
...
BEGIN
  ...
  select sum(coalesce(Acpe.Credito,Acp.Credito)) into STRICT p_creditos
                    ...
                    from (select id_carga_curso,id_curso_detalle,promedio,id_tipo_condicion,min(fecha) fecha
                        from (
                                ...
                                from acad_curso_alumno aca inner join acad_plan_programa app
                                                        ...
                                                        and aca.id_persona=p_id_persona and aca.id_tipo_condicion =1 and aca.estado='1'
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.ft_creditos_acumulados_plan (p_id_persona integer,p_id_plan_programa integer) FROM PUBLIC;
