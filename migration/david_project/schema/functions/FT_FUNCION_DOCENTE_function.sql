...
CREATE OR REPLACE FUNCTION david.ft_funcion_docente ( p_id_persona bigint, p_id_semestre bigint, p_id_nivel_ensenanza bigint, p_id_tipo_contrato bigint, p_id_unidad_academica bigint, p_id_programa_estudio bigint, p_id_sede bigint, p_bimestre bigint ) RETURNS bigint AS $body$
DECLARE
...
BEGIN
    ...
    SELECT avg(FUNCION_DOCENTE) * 0.20 / 100 * 100 AS FUNCION_DOCENTE INTO STRICT r_puntaje  FROM (
...
SELECT 
    ...
    bimestre, num_documento,
     (coalesce(sum( 0  + promedio_asistencia_docente + promedio_evaluacion + promedio_Recursos + promedio_asistencia),0) * 100 / 20) AS funcion_docente
            FROM (
                    select  
            ...
            materno,
            nombre,
            num_documento ) alias18 
           
            GROUP BY  programa_estudio,
            unidad_academica,sede,
            id_persona ,bimestre, paterno,materno,nombre,
            num_documento ) alias19;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.ft_funcion_docente ( p_id_persona bigint, p_id_semestre bigint, p_id_nivel_ensenanza bigint, p_id_tipo_contrato bigint, p_id_unidad_academica bigint, p_id_programa_estudio bigint, p_id_sede bigint, p_bimestre bigint ) FROM PUBLIC;
