...
CREATE OR REPLACE FUNCTION david.ft_campus_alumno_nd (P_NUM_DOC text) RETURNS varchar AS $body$
DECLARE
...
FROM (
	SELECT
		...
		PD.MATERNO,
		PD.NOMBRE,
		CASE WHEN PD.CODIGO_SEXO='F' THEN  'Femenino' WHEN PD.CODIGO_SEXO='M' THEN  'Masculino' END  AS GENERO,
		Pd.Tipo_Doc,
		Pd.Num_Documento,
	...
	LEFT JOIN(
		SELECT
			...
			apc.id_persona,
			apc.id_semestre,
			CASE WHEN Apc.Feligresia='S' THEN  'SI'  ELSE 'NO' END  ADVENTISTA,
...
UNION ALL
	...
	SELECT
		...
		pnc.MATERNO,
		pnc.NOMBRE,
		CASE WHEN pnc.CODIGO_SEXO='F' THEN  'Femenino' WHEN pnc.CODIGO_SEXO='M' THEN  'Masculino' END  AS GENERO,
		pnc.Tipo_Doc,
		pnc.Num_Documento,
		ttt.nombre regimen_dedicacion
	FROM
...
SELECT
	DISTINCT CAMPUS_ADS INTO STRICT P_CAMPUS
FROM (
	SELECT
		apc.campus_ads,
		apc.fac_ads,
...
SELECT CASE WHEN B.ID_SEDE='1' THEN  'Sede Lima' WHEN B.ID_SEDE='2' THEN  'Sede Juliaca' WHEN B.ID_SEDE='3' THEN  'Sede Tarapoto' END  INTO STRICT P_CAMPUS
...
AND A.ID_PERSONA IN (SELECT ID_PERSONA FROM MOISES.PERSONA_DOCUMENTO WHERE NUM_DOCUMENTO = P_NUM_DOC)   LIMIT 1;

END IF;

/*
  SELECT SEDE INTO P_CAMPUS
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.ft_campus_alumno_nd (P_NUM_DOC text) FROM PUBLIC;
