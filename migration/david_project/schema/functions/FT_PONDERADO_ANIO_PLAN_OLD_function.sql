...
CREATE OR REPLACE FUNCTION david.ft_ponderado_anio_plan_old (p_id_persona integer,p_id_plan_programa integer,p_anio integer) RETURNS bigint AS $body$
DECLARE
...
union
                                ...
                                left join(select accd.id_carga_curso,max(amd.fecha_fin) fecha_fin
                                            from acad_carga_curso_docente accd inner join acad_modulo_detalle amd on amd.id_modulo_detalle=accd.id_modulo_detalle
                                            group by accd.id_carga_curso) accd on accd.id_carga_curso=aca.id_carga_curso
                                where (apce.id_curso_detalle IS NOT NULL AND apce.id_curso_detalle::text <> '') ) alias19 
...
union
                        ...
                        where aca.id_persona=p_id_persona and aca.id_tipo_condicion=1 and coalesce(id_plan_programa::text, '') = '' and aca.estado='1'
                        and id_curso_detalle not in (select vce.id_curso_detalle from acad_curso_alumno aca inner join vw_curso_equivalente vce on vce.curso_equiv=aca.id_curso_detalle
                                                        where id_persona=p_id_persona and id_tipo_condicion=1 and (id_plan_programa IS NOT NULL AND id_plan_programa::text <> '') ) ) Apc
        ...
        where id_plan=p_plan and coalesce(Acp.Id_Plan_Curso_Electivo::text, '') = '' ) alias26
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.ft_ponderado_anio_plan_old (p_id_persona integer,p_id_plan_programa integer,p_anio integer) FROM PUBLIC;
