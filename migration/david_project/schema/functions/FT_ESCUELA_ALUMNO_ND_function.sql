...
CREATE OR REPLACE FUNCTION david.ft_escuela_alumno_nd (P_NUM_DOC text) RETURNS varchar AS $body$
DECLARE
...
BEGIN
	 ...
	 SELECT
	DISTINCT eap_ads INTO STRICT P_ESCUELA
FROM (
	SELECT
		apc.campus_ads,
		apc.fac_ads,
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION david.ft_escuela_alumno_nd (P_NUM_DOC text) FROM PUBLIC;
