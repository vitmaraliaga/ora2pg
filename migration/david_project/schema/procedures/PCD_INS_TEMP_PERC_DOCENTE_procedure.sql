...
CREATE OR REPLACE PROCEDURE david.pcd_ins_temp_perc_docente ( pval integer) AS $body$
BEGIN

  EXECUTE 'truncate table aatemp_percepcion_docente';
    commit;
  ...
  INSERT INTO aatemp_percepcion_docente
    ...
    FROM (
        ...
        AND vaca.ORIGEN ='O'
        WHERE (ir.VALOR_RESPUESTA IS NOT NULL AND ir.VALOR_RESPUESTA::text <> '')
        AND vaca.SEMESTRE ='2023-2'
        and vaca.id_sede = coalesce('',vaca.id_sede)
        and vaca.id_nivel_ensenanza = coalesce('',vaca.id_nivel_ensenanza)
        and vaca.id_tipo_contrato = coalesce('',vaca.id_tipo_contrato)
        and vaca.id_unidad_academica = coalesce('',vaca.id_unidad_academica)
        and vaca.id_programa_estudio = coalesce('',vaca.id_programa_estudio)
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE david.pcd_ins_temp_perc_docente ( pval integer) FROM PUBLIC;
