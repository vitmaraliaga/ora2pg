...
CREATE OR REPLACE PROCEDURE david.pcd_update_req_alum ( v_id_persona integer, v_id_usuario integer, v_id_plan_programa integer, v_id_solicitud_mat_alum integer, v_id_tipo_requisito_beca integer, v_renovacion text ) AS $body$
DECLARE
...
;
   ...
   if (req.codigo='G14') then
    ...
    set estado=p_estado , observacion=p_obs ,id_usuario_act=v_id_usuario , fecha_actualizacion=clock_timestamp()
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE david.pcd_update_req_alum ( v_id_persona integer, v_id_usuario integer, v_id_plan_programa integer, v_id_solicitud_mat_alum integer, v_id_tipo_requisito_beca integer, v_renovacion text ) FROM PUBLIC;
