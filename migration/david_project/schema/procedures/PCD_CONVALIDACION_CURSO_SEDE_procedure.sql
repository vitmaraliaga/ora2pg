...
CREATE OR REPLACE PROCEDURE david.pcd_convalidacion_curso_sede (p_id_persona integer,p_id_plan_programa_a integer,p_id_plan_programa_n integer, p_id_usuario_reg integer) AS $body$
DECLARE
...
BEGIN
  ...
  for cc in cursos loop
    ...
    values (cc.ID_PERSONA,p_id_plan_programa_n,cc.ID_CURSO_DETALLE,cc.ID_TIPO_TRAMITE,cc.ID_TIPO_CONDICION,cc.ID_PLAN_CURSO,cc.PROMEDIO,cc.ALCANZADO,cc.ESTADO,p_id_usuario_reg,clock_timestamp(),cc.FECHA_FIN_CURSO);
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE david.pcd_convalidacion_curso_sede (p_id_persona integer,p_id_plan_programa_a integer,p_id_plan_programa_n integer, p_id_usuario_reg integer) FROM PUBLIC;
