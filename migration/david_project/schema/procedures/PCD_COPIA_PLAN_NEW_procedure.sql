...
CREATE OR REPLACE PROCEDURE david.pcd_copia_plan_new ( p_id_plan_ant integer, p_id_plan_new integer, p_id_usuario_reg integer ) AS $body$
BEGIN
  ...
  ESTADO,p_id_usuario_reg ID_USUARIO_REG,clock_timestamp() FECHA_REGISTRO,HTP,HTV,HPP,HPV,MODO,HORA_DIVIDIDA
  from acad_plan_curso where Id_Plan=p_id_plan_ant and coalesce(Id_Plan_Curso_Electivo::text, '') = '' 
  ...
  pc.CICLO,pc.CODIGO,
  pc.ESTADO,p_id_usuario_reg ID_USUARIO_REG,clock_timestamp() FECHA_REGISTRO,pc.HTP,pc.HTV,pc.HPP,pc.HPV,pc.MODO,pc.HORA_DIVIDIDA 
  ...
  where pc.Id_Plan=p_id_plan_ant and (pc.Id_Plan_Curso_Electivo IS NOT NULL AND pc.Id_Plan_Curso_Electivo::text <> '') 
  ...
  pr.ESTADO,p_id_usuario_reg ID_USUARIO_REG,clock_timestamp() FECHA_REGISTRO
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE david.pcd_copia_plan_new ( p_id_plan_ant integer, p_id_plan_new integer, p_id_usuario_reg integer ) FROM PUBLIC;
