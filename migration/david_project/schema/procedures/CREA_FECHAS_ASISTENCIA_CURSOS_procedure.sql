...
CREATE OR REPLACE PROCEDURE david.crea_fechas_asistencia_cursos ( p_val integer, p_id_nivel_ensenanza integer) AS $body$
DECLARE
...
BEGIN
  ...
  delete FROM acad_doc_asistencia where id_semestre=p_id_semestre and id_nivel_ensenanza=p_id_nivel_ensenanza and no_medicina=1;
  commit;
  ...
  FOR c_horario IN cursos_carga LOOP
       ...
       IF LENGTH(trim(both dias)) >0 THEN
          FOR y IN 1..length(trim(both dias)) LOOP
            ...
            Insert into acad_doc_asistencia Values (c_horario.id_carga_curso_docente,p_id_semestre,p_id_nivel_ensenanza,1,fecha_inicio_curso,c_horario.TIPO,'0',hini,hfin);
            commit;
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE david.crea_fechas_asistencia_cursos ( p_val integer, p_id_nivel_ensenanza integer) FROM PUBLIC;
