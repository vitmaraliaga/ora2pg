...
CREATE OR REPLACE PROCEDURE david.pcd_reg_candado_teologia_sem (v_id_semestre integer) AS $body$
BEGIN

  insert into acad_candado(ID_PERSONA,ID_TIPO_CANDADO,ID_SEMESTRE,DESCRIPCION,ESTADO,ID_USUARIO_REG,FECHA_REGISTRO)
  SELECT id_persona,67,v_id_semestre,'Bloqueo por cese de estudias por 2 ciclos académicos',1,247620,clock_timestamp()
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE david.pcd_reg_candado_teologia_sem (v_id_semestre integer) FROM PUBLIC;
