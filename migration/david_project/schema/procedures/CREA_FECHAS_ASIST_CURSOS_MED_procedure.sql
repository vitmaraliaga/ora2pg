...
CREATE OR REPLACE PROCEDURE david.crea_fechas_asist_cursos_med ( p_id_semestre integer, p_id_nivel_ensenanza integer) AS $body$
DECLARE
  ...
  where id_semestre=p_id_semestre and aca.origen='O' and Aca.Id_Nivel_Ensenanza=p_id_nivel_ensenanza 
  and Aca.Id_Programa_Estudio=1 and aca.id_curso_modo=1;


BEGIN

  delete FROM acad_doc_asistencia where id_semestre=p_id_semestre and id_nivel_ensenanza=p_id_nivel_ensenanza and no_medicina=0;
  commit;
  ...
  FOR c_horario IN cursos_carga LOOP
       ...
       IF LENGTH(trim(both dias)) >0 THEN
          FOR y IN 1..length(trim(both dias)) LOOP
            ...
            Insert into acad_doc_asistencia Values (c_horario.id_carga_curso_docente,p_id_semestre,p_id_nivel_ensenanza,0,fecha_inicio_curso,c_horario.TIPO,'0',hini,hfin);
            commit;
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE david.crea_fechas_asist_cursos_med ( p_id_semestre integer, p_id_nivel_ensenanza integer) FROM PUBLIC;
