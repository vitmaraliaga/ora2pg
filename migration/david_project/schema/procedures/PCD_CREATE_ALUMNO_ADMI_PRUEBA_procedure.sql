...
CREATE OR REPLACE PROCEDURE david.pcd_create_alumno_admi_prueba ( V_ID_TIPO_DOCUMENTO integer, V_NUM_DOCUMENTO text, V_NOMBRE text, V_PATERNO text, V_MATERNO text, V_ESTADO_CIVIL text, V_NACIONALIDAD text, V_UBIGEO integer, V_COD_RELIGION text, V_SEXO text, V_FECHA_NACIMIENTO timestamp(0), V_CELULAR text, V_CORREO text, V_ID_PLAN_PROGRAMA integer, V_ID_TIPO_BECA_ESTATAL integer ) AS $body$
DECLARE
...
if (p_id_persona=0) then
  ...
  select coalesce((select id_persona from moises.persona where upper(sintilde(nombre||' '||paterno||' '||materno))=upper(sintilde(v_nombre||' '||v_paterno||' '||v_materno)) ),0) into STRICT p_id_persona;
...
if (p_res_id_persona=0) then
  ...
  if (p_res_persona_natural=0) then
    ...
    if (p_res_persona_alumno=0) then
      ...
      select coalesce((select IS_NUMBER(codigo) from moises.persona_natural_alumno where id_persona=p_id_persona),0) into STRICT p_res_codigo;
      ...
      where estado='1' and (id_persona,id_plan_programa) in (SELECT id_persona,id_plan_programa from Vw_Alumno_Plan_Programa where id_persona=p_id_persona and Id_Nivel_Ensenanza=p_id_nivel_ensenanza);
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE david.pcd_create_alumno_admi_prueba ( V_ID_TIPO_DOCUMENTO integer, V_NUM_DOCUMENTO text, V_NOMBRE text, V_PATERNO text, V_MATERNO text, V_ESTADO_CIVIL text, V_NACIONALIDAD text, V_UBIGEO integer, V_COD_RELIGION text, V_SEXO text, V_FECHA_NACIMIENTO timestamp(0), V_CELULAR text, V_CORREO text, V_ID_PLAN_PROGRAMA integer, V_ID_TIPO_BECA_ESTATAL integer ) FROM PUBLIC;
