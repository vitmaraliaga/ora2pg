...
CREATE OR REPLACE PROCEDURE david.pcd_act_retorno_est () AS $body$
BEGIN
  EXECUTE 'truncate table aatemp_ultima_matricula_act';
    commit;

  insert into aatemp_ultima_matricula_act
  SELECT A.*,clock_timestamp() FECHA_REGISTRO
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE david.pcd_act_retorno_est () FROM PUBLIC;
