...
CREATE OR REPLACE VIEW vw_ii_24 (id_equipo_taller, id_evento_participante, id_evento, evento, disciplina, institucion, jugadores, delegado, num_documento_delegado, jugador, documento_jugador, id_jugador, photo, id_iglesia, otra_iglesia, nombre_iglesia, nombre_iglesia_campo_dtto, nombre_iglesia_campo, nombre_iglesia_union) AS SELECT
            ...
            CASE WHEN coalesce(ei.NOMBRE::text, '') = '' THEN eep.OTRA_INSTITUCION WHEN  coalesce(ei.CODIGO  , '-' ) = 'OTRO' THEN eep.OTRA_INSTITUCION ELSE ei.NOMBRE END AS institucion ,
            ...
            coalesce(ej2.NOMBRE, '') ||' ' || coalesce(ej2.PATERNO , '') ||' ' || coalesce(ej2.MATERNO , '') AS JUGADOR,
            coalesce(ej2.NUMERO_DOCUMENTO,' ') AS documento_jugador,
            ej2.ID_JUGADOR ,
            ej2.PHOTO,
            case WHEN coalesce(ei2.ID_IGLESIA::text, '') = '' THEN 0 ELSE ei2.ID_IGLESIA end AS id_iglesia ,
            eu.sigla || ' - ' || ec.sigla || ' - ' || ecd.NOMBRE || ' - ' ||   eep.OTRA_IGLESIA AS otra_iglesia,
    		CASE WHEN coalesce(eep.IGLESIA::text, '') = '' THEN eep.OTRA_IGLESIA
