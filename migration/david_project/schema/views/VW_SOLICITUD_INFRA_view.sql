...
oa.Sigla sort1,coalesce(Ape.codigo_sunedu2,ape.nombre_corto) sort2,asp.fecha_expiracion dateexpiry,'ESTUDI' categorycode,
coalesce(aac.Fecha_Actualizacion,aac.Fecha_Registro) fecha_de_matricula
...
pa.Codigo_Sexo sex, pa.Celular phone, pa.Fec_Nacimiento dateofbirth, pa.Direccion address, 
max(replace(replace(coalesce(ape.Sede,apec.Sede),'Sede ',''),'Filial ','')) state,
tp.Gentilicio B_country, Pna.Codigo cardnumber, pa.Num_Documento borrowernotes,
max(coalesce(oa.Sigla,oac.Sigla)) sort1,max(coalesce(Ape2.Codigo_Sunedu2,coalesce(Ape.Nombre_Corto,apec.Nombre_Corto))) sort2,max(Amd.Fecha_Fin) dateexpiry,'DOCEN' categorycode,
