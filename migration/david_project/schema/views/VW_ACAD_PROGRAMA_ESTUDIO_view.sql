...
CREATE OR REPLACE VIEW vw_acad_programa_estudio (id_programa_estudio, nombre_original, duracion, nombre_corto, nombre_certificado, min_cred_requeridos, max_cred_requeridos, nombre, id_sedearea, id_sede, sede, fac_sigla, id_facultad, nombre_facultad, nombre_escuela_cs, id_escuela, nombre_escuela, escuela, id_escuela_profesional, escuela_profesional, id_modalidad_estudio, modalidad_estudio, id_tipo_contrato, id_nivel_ensenanza, id_unidad_academica, id_semipresencial, id_sede_sem, id_area, id_parent, codigo, estado, alias, nombre_certificado_sunedu, codigo_nacional) AS select ape.id_programa_estudio,ape.nombre, ape.duracion, ape.nombre_corto,ape.NOMBRE_CERTIFICADO,ape.min_cred_requeridos, ape.max_cred_requeridos,
 CASE WHEN ape.ID_TIPO_CONTRATO =24 THEN  ape.Nombre_Certificado ||CASE WHEN coalesce(asmp.id_semipresencial::text, '') = '' THEN ''  ELSE ' - '||asmp.nombre END  || ', ' || tme.nombre  ELSE ape.Nombre_Certificado||CASE WHEN coalesce(asmp.id_semipresencial::text, '') = '' THEN ''  ELSE ' - '||asmp.nombre END  || ', ' || tme.nombre END  AS  nombre,
  -- ape.nombre,
  osa.id_sedearea, os.id_sede, os.nombre sede ,op.sigla fac_sigla,
coalesce(CASE WHEN op.id_parent=8 THEN 8  ELSE null END , op.id_area) id_facultad, CASE WHEN CASE WHEN op.id_parent=8 THEN 8  ELSE null coalesce(END::text, '') = '' THEN op.nombre  ELSE opp.nombre END  nombre_facultad,
oa.nombre||CASE WHEN coalesce(asmp.id_semipresencial::text, '') = '' THEN ''  ELSE ' - '||asmp.nombre END  nombre_escuela_cs,
oa.id_area id_escuela , oa.nombre||CASE WHEN coalesce(asmp.id_semipresencial::text, '') = '' THEN ''  ELSE ' - '||asmp.codigo END  nombre_escuela,oa.nombre escuela,
ape.ID_ESCUELA_PROFESIONAL,coalesce(coalesce(nep.Nombre,pp.nombre),oa.Nombre) escuela_profesional
