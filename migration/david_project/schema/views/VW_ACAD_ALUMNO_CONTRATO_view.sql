...
coalesce(tr.Nombre,'Otro') RELIGION,Ft_Calcular_Creditos_Semestre(asm.semestre,aac.id_persona,aac.Id_Plan_Programa) creditos,
Rth.Id_Resid_Tipo_Habitacion, coalesce(Rth.Nombre,'Externo') RESIDENCIA,
coalesce(CASE WHEN aac.Tipo_Alumno='B18' THEN tbe.Nombre END ,'Ninguna') BECA,
coalesce(CASE WHEN aac.Tipo_Alumno='B18' THEN tbea.Nombre END ,'Ninguna') BECA_ant,
CASE WHEN Ft_Calcular_Ciclo_Programa(asm.semestre,aac.id_persona,aac.Id_Plan_Programa)=1 THEN 'Ingresante'  ELSE 'Antiguo' END  INGRESANTE,
...
CASE WHEN aac.Estado='1' THEN 'Matriculados' WHEN aac.Estado='0' THEN 'En Proceso' WHEN aac.Estado='M' THEN 'Confirmado' END  situacion,
...
aac.ID_PLAN_PROGRAMA,coalesce(aac.Id_Tipo_Resp_Financiero,3) id_tipo_resp_financiero,coalesce(aac.id_resp_financiero,aac.id_persona) id_resp_financiero,
Trf.Nombre tipo_resp_financiero,
CASE WHEN aac.ID_NIVEL_ENSENANZA=9 THEN  genesis.ft_resp_financiero_nombre(pn.num_documento)  ELSE pr.Nombre||' '||pr.Paterno||' '||pr.Materno END  as Resp_financiero,
CASE WHEN aac.ID_NIVEL_ENSENANZA=9 THEN  genesis.ft_resp_financiero_celular(pn.num_documento)  ELSE pnr.celular END   celular_resp,
CASE WHEN aac.ID_NIVEL_ENSENANZA=9 THEN  genesis.ft_resp_financiero_correo(pn.num_documento)  ELSE pnr.correo END   correo_resp,
...
left join Acad_Alumno_Beca aab on Aab.Id_Persona=aac.Id_Persona and Aab.Estado='1' and (Aab.Id_Tipo_Beca_Estatal IS NOT NULL AND Aab.Id_Tipo_Beca_Estatal::text <> '')
left join Tipo_Beca_Estatal tbe on tbe.Id_Tipo_Beca_Estatal=Aab.Id_Tipo_Beca_Estatal
left join Acad_Alumno_Beca aaba on Aaba.Id_Persona=aac.Id_Persona and (Aaba.Id_Tipo_Beca_Estatal IS NOT NULL AND Aaba.Id_Tipo_Beca_Estatal::text <> '')
...
left join Residencia_Tipo_Habitacion rth on Rth.Id_Resid_Tipo_Habitacion=coalesce(rh.Id_Resid_Tipo_Habitacion,aac.Id_Resid_Tipo_Habitacion)
left join moises.persona pm on pm.Id_Persona=coalesce(aac.Id_Usuario_Act,aac.Id_Usuario_Reg)
left join moises.persona pr on pr.Id_Persona=coalesce(aac.Id_Resp_Financiero,aac.id_persona)
...
left join Tipo_Resp_Financiero trf on Trf.Id_Tipo_Resp_Financiero=coalesce(aac.Id_Tipo_Resp_Financiero,3)
