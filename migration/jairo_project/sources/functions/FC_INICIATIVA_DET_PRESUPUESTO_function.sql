...
FUNCTION        FC_INICIATIVA_DET_PRESUPUESTO(
P_ID_INICIATIVA_DETALLE IN INTEGER )
RETURN NUMBER IS R_PRESUPUESTO NUMBER(10, 2);
BEGIN 
   SELECT SUM(cp.cos_valor) into R_PRESUPUESTO
            ...
            INNER JOIN ( select sic.id_iniciativa_cuenta, sic.id_iniciativa, sic.id_cuentaaasi,decode(sic.tiene_ctacte,'S',sicc.cta_cte,'X') cta_cte ,sic.tiene_ctacte from jairo.sge_iniciativa_cuenta sic 
            left join jairo.sge_iniciativa_cuenta_cte sicc on sic.id_iniciativa_cuenta = sicc.id_iniciativa_cuenta
             ) tsic on tsic.id_iniciativa = smi.id_iniciativa AND tsic.id_cuentaaasi =cp.id_cuentaaasi   and tsic.cta_cte = decode(tsic.tiene_ctacte,'S',cp.id_ctacte,'X')
            WHERE SMID.ID_INICIATIVA_DETALLE = P_ID_INICIATIVA_DETALLE
            AND (
                (SMO.ID_ANHO IS NULL AND cp.ID_ANHO = SMO.ID_ANHO)
                OR (SMO.ID_ANHO IS NOT NULL AND cp.ID_ANHO = SMO.ID_ANHO)
            )
