...
CREATE OR REPLACE FUNCTION enoc.fc_motivo_asistencia (P_ID_TRABAJADOR bigint,P_ID_TIPO_HORARIO bigint,P_ID_SEDEAREA bigint, P_FECHA timestamp(0)) RETURNS varchar AS $body$
DECLARE
...
--        GOTO SALIDA;
   ...
   select count(*) into STRICT l_contar_fer from enoc.plla_feriados
   where  fecha = l_fecha
...
--        GOTO SALIDA;
    ...
    WHERE ID_SEDEAREA=P_ID_SEDEAREA;
     ...
     select count(*) into STRICT l_contar_fer_ins from enoc.plla_feriados
     where fecha = l_fecha
...
--        GOTO SALIDA;
    ...
    if l_contar=1 then
      SELECT A.ID_TIPO_HORARIO into STRICT l_id_tipo_horario FROM ENOC.PLLA_TIPO_HORARIO A,ENOC.PLLA_TIPO_HORARIO_TRAB B
   ...
   if l_codigo ='HM' then
    ...
    select count(*) into STRICT l_contar from enoc.plla_horario_mensual m ,  enoc.plla_horario_mensual_det d
    ...
    select count(*) into STRICT l_contar from enoc.plla_horario_detalle a
   ...
   IF l_dato='A' then
    SELECT count(*) into STRICT l_contar FROM ENOC.PLLA_TIPO_HORARIO A,ENOC.PLLA_TIPO_HORARIO_TRAB B ,ENOC.PLLA_TIPO_CONFIG_HORARIO c 
    WHERE A.ID_TIPO_HORARIO=B.ID_TIPO_HORARIO 
    and  c.ID_TIPO_CONFIG_HORARIO=a.ID_TIPO_CONFIG_HORARIO
    AND B.ID_TRABAJADOR=P_ID_TRABAJADOR
    AND c.codigo='SH'
    AND l_fecha BETWEEN B.DESDE AND B.HASTA;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION enoc.fc_motivo_asistencia (P_ID_TRABAJADOR bigint,P_ID_TIPO_HORARIO bigint,P_ID_SEDEAREA bigint, P_FECHA timestamp(0)) FROM PUBLIC;
