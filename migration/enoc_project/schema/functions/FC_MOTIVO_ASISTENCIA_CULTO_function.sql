...
CREATE OR REPLACE FUNCTION enoc.fc_motivo_asistencia_culto (P_ID_TRABAJADOR bigint,P_ID_TIPO_HORARIO bigint,P_ID_SEDEAREA bigint, P_FECHA timestamp(0)) RETURNS varchar AS $body$
DECLARE
...
--        GOTO SALIDA;
   ...
   SELECT count(*) into STRICT l_contar FROM ENOC.PLLA_TIPO_HORARIO A,ENOC.PLLA_TIPO_HORARIO_TRAB B 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION enoc.fc_motivo_asistencia_culto (P_ID_TRABAJADOR bigint,P_ID_TIPO_HORARIO bigint,P_ID_SEDEAREA bigint, P_FECHA timestamp(0)) FROM PUBLIC;
