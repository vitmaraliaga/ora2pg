...
CREATE OR REPLACE FUNCTION enoc.fc_paso_req_cand (P_ID_SOLIC_REQUE bigint,P_ID_PERSONA bigint) RETURNS bigint AS $body$
DECLARE

l_contar bigint:=0;
l_id_paso_req bigint:=null;
l_id_modalidad_req bigint;
l_esdocente bigint;
l_id_entidad bigint;
l_codigo_mod varchar(20);
l_codigo_paso varchar(20);
l_id_situacion_trabajador varchar(10);
l_extint varchar(5):='E';

BEGIN

    select id_modalidad_req, coalesce(esdocente,0),id_entidad
    into STRICT l_id_modalidad_req,l_esdocente,l_id_entidad
    ...
    if (l_codigo_paso IS NOT NULL AND l_codigo_paso::text <> '') then
      select ID_PASO_REQUE into STRICT l_id_paso_req from ENOC.PLLA_PASO_REQUE where codigo=l_codigo_paso;
    end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION enoc.fc_paso_req_cand (P_ID_SOLIC_REQUE bigint,P_ID_PERSONA bigint) FROM PUBLIC;
