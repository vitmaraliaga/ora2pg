...
CREATE OR REPLACE PROCEDURE enoc.pkg_planilla_proc_sp_d_donacion_diezmo_autor (P_ID_PLANILLA_TRAB text,P_ID_CONFIG_PLANILLA_CONCEPTO bigint, P_IMPORTE OUT bigint,P_ERROR OUT bigint,P_MSGERROR out text) AS $body$
DECLARE
...
BEGIN
    ...
    if l_descuentodiezmo = 'S' then
      ...
      select count(*) into STRICT  l_contar from ENOC.VW_PLLA_PARAMETROS_VALOR where id_entidad= l_id_entidad and codigo='PARAM_DIEZMO' and id_anho=l_id_anho;
      if l_contar=0 then
        l_error:=1;
...
--         goto salida;
      ...
      if l_contar>0 then
        select importe into STRICT l_porc from ENOC.VW_PLLA_PARAMETROS_VALOR where id_entidad= l_id_entidad and codigo='PARAM_DIEZMO' and id_anho=l_id_anho;
      end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE enoc.pkg_planilla_proc_sp_d_donacion_diezmo_autor (P_ID_PLANILLA_TRAB text,P_ID_CONFIG_PLANILLA_CONCEPTO bigint, P_IMPORTE OUT bigint,P_ERROR OUT bigint,P_MSGERROR out text) FROM PUBLIC;
