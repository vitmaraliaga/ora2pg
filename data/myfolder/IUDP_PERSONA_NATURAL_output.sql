...
CREATE OR REPLACE PROCEDURE moises.iudp_persona_natural ( P_Modo integer, P_NUM_DOCUMENTO text, P_ID_TIPODOCUMENTO integer, P_NOMBRE text, P_PATERNO text, P_MATERNO text, P_ID_ESTADOCIVIL integer, P_ID_TIPOTRATAMIENTO integer, P_ID_PAIS integer, P_ID_NACIONALIDAD integer, P_ID_TIPOSANGRE integer, P_SEXO text, P_FEC_NACIMIENTO text, P_FEC_DEFUNCION text, P_ID_PERSONA OUT integer ) AS $body$
DECLARE
      ...
      END IF;
		...
		IF S_ID_PERSONANATURAL = 0 THEN
		      ...
		      --DBMS_OUTPUT.PUT_LINE(S_ID_PAIS); 
		        ...
		        VALUES (P_ID_PERSONA, P_ID_ESTADOCIVIL, coalesce(P_ID_TIPOTRATAMIENTO, 1), NULLIF(S_ID_PAIS, 0), NULLIF(S_ID_NACIONALIDAD, 0), NULLIF(P_ID_TIPOSANGRE, 0), P_SEXO, to_date(P_FEC_NACIMIENTO,'yyyy-mm-dd'), to_date(P_FEC_DEFUNCION,'yyyy-mm-dd'));
		      END IF;
	      ELSE
		      UPDATE moises.Persona_Natural SET
		        ...
		        ID_TIPOPAIS            = coalesce(NULLIF(S_ID_PAIS, 0), ID_TIPOPAIS),
		        ID_NACIONALIDAD    = coalesce(NULLIF(S_ID_NACIONALIDAD, 0), ID_NACIONALIDAD),
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE moises.iudp_persona_natural ( P_Modo integer, P_NUM_DOCUMENTO text, P_ID_TIPODOCUMENTO integer, P_NOMBRE text, P_PATERNO text, P_MATERNO text, P_ID_ESTADOCIVIL integer, P_ID_TIPOTRATAMIENTO integer, P_ID_PAIS integer, P_ID_NACIONALIDAD integer, P_ID_TIPOSANGRE integer, P_SEXO text, P_FEC_NACIMIENTO text, P_FEC_DEFUNCION text, P_ID_PERSONA OUT integer ) FROM PUBLIC;
