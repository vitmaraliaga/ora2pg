...
(select estado from moises.verificacion v where v.tipo='C' and v.id_persona=p.id_persona and v.valor=pt.num_telefono) solicito_codigo_celular,
(select estado from moises.verificacion v where v.tipo='V' and v.id_persona=p.id_persona and v.valor=pv.direccion) solicito_codigo_correo,
UPPER(coalesce(P.PATERNO || ' ', '')|| coalesce(P.MATERNO || ', ', ''))|| INITCAP(P.NOMBRE) AS NOM_PERSONA
from(select id_persona,
      (select id_telefono from moises.persona_telefono where es_activo = 1 and id_tipotelefono = 5 and id_persona=pn.id_persona  LIMIT 1) id_telefono,
      (select id_virtual from moises.persona_virtual where es_activo = 1 and id_tipovirtual = 1 and id_persona=pn.id_persona  LIMIT 1) id_virtual,
      (select id_direccion from moises.persona_direccion where es_activo = 1 and id_tipodireccion = 4 and id_persona=pn.id_persona  LIMIT 1) id_direccion,
      (select num_documento from moises.persona_documento where id_persona=pn.id_persona  LIMIT 1) num_documento,
      ...
      fec_nacimiento,fec_defuncion,vive,fec_caducidad,telefono,correo_inst,iglesia,fecha_bautizo,tiene_cargo,cargo_iglesia,nombre_autoridad,
      tlf_autoridad,id_nacionalidad
...
--where pn.id_persona=7427
--where pn.id_persona=8345
--where pn.num_documento='70000395'
;
