...
CREATE OR REPLACE PROCEDURE registro_persona.sp_registro_alumno (P_ID_PERSONA bigint,P_NOMBRE text, P_PATERNO text, P_MATERNO text, P_TIPO text, P_CODIGO out text,P_EMAIL out text, P_REG_ALUMNO out bigint) AS $body$
DECLARE
...
BEGIN
        ...
        select count(*) into STRICT has_cod from moises.persona where id_persona = P_ID_PERSONA and codigo is not null;
        ...
        select count(*) into STRICT has_uni from moises.persona_natural_alumno where id_persona = P_ID_PERSONA and codigo is not null;
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE registro_persona.sp_registro_alumno (P_ID_PERSONA bigint,P_NOMBRE text, P_PATERNO text, P_MATERNO text, P_TIPO text, P_CODIGO out text,P_EMAIL out text, P_REG_ALUMNO out bigint) FROM PUBLIC;
